...
static char ksm_sleep_millisecs[5] = "";
static char ksm_pages_to_scan[5] = "" ;
...
static int set_ksm_param(uint16_t __unused unused)
{
   ...
   if(is_ksm_supported == 0)
   {
       ...
       FWRITE_STR(ksm_param_sleeptime, sleep_time, 3, rc);
       FWRITE_STR(ksm_param_pages_to_scan, scan_page, 3, rc);
   }
   return rc;
}
...
static int reset_ksm_param(uint16_t __unused unused)
{
   ...
   if(is_ksm_supported == 0)
   {
      FWRITE_STR(ksm_param_sleeptime, ksm_sleep_millisecs, 3, rc);
      FWRITE_STR(ksm_param_pages_to_scan, ksm_pages_to_scan, 3, rc);
   }
   return rc;
}
...
#   define ITS(x)          its[cpu_prefix_len+x]
    ...
    setup_cpu_freq_mappings();
     ...
     //target based initiaizations
    if (SOCID_8939 == get_soc_id())
    {
    ...
    if(is_ksm_supported == 0)
    {
        FREAD_STR(ksm_param_sleeptime,ksm_sleep_millisecs,5,rc);
        FREAD_STR(ksm_param_pages_to_scan,ksm_pages_to_scan,5,rc);
    }
