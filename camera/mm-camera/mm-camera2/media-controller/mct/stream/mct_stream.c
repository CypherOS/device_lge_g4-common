...
static boolean mct_stream_link_preview(mct_stream_t *stream)
{
  boolean              ret = FALSE;
  ...
  if (!sensor || !iface ||!pproc || !imglib) {
    CLOGE(CAM_MCT_MODULE, "Null: %p %p %p %p", sensor, iface, pproc, imglib);
    return FALSE;
  }
  ...
  if (pipeline->session_data.sensor_format != FORMAT_YCBCR) {
    ...
    if (!isp || !stats) {
      CLOGE(CAM_MCT_MODULE, "Null: %p %p ", isp, stats);
      return FALSE;
    }

    isp->set_mod(isp, MCT_MODULE_FLAG_INDEXABLE, stream->streaminfo.identity);
    stats->set_mod(stats, MCT_MODULE_FLAG_SINK, stream->streaminfo.identity);
    ...
    ret = mct_stream_link_branch_modules(stream, isp, stats, NULL);
    if (ret == FALSE) {
      CLOGE(CAM_MCT_MODULE, "link failed");
      return FALSE;
    }
  } else {
    CLOGE(CAM_MCT_MODULE, "YUV sensor attached. Not linking stats module");
    ret = mct_stream_link_modules(stream, sensor, iface, pproc, imglib,
      NULL);
    if (ret == FALSE) {
      CLOGE(CAM_MCT_MODULE, "link failed");
      return FALSE;
...
static boolean mct_stream_link_preview_secure(mct_stream_t *stream)
{
  boolean              ret = FALSE;
  mct_list_t           *modules =
    MCT_PIPELINE_MODULES(MCT_OBJECT_PARENT(stream)->data);
  ...
  if (!sensor || !iface || !isp) {
    CLOGE(CAM_MCT_MODULE, "Null: %p %p %p ", sensor, iface, isp);
    return FALSE;
  }
  sensor->set_mod(sensor, MCT_MODULE_FLAG_SOURCE,
    stream->streaminfo.identity);
...
static boolean mct_stream_start_link(mct_stream_t *stream)
{
  ...
  CLOGD(CAM_MCT_MODULE, "stream_info->is_secure :%d", stream_info->is_secure);
  ...
  case CAM_STREAM_TYPE_PREVIEW: {
    ...
    if(CAM_STREAM_TYPE_PREVIEW == stream->streaminfo.stream_type) {
      CLOGH(CAM_MCT_MODULE, "Start linking preview stream 0x%x",
        stream->streaminfo.identity);
    } else {
      CLOGH(CAM_MCT_MODULE, "Start linking postview stream 0x%x",
        stream->streaminfo.identity);
    }

    if(SECURE == stream_info->is_secure) {
      CLOGH(CAM_MCT_MODULE, "secure stream linking");
      ret = mct_stream_link_preview_secure(stream);
      if (ret == FALSE) {
        CLOGE(CAM_MCT_MODULE, "link failed");
        return FALSE;
      }
    } else {
      CLOGH(CAM_MCT_MODULE, "non secure stream linking");
      ret = mct_stream_link_preview(stream);
      if (ret == FALSE) {
        CLOGE(CAM_MCT_MODULE, "link failed");
        return FALSE;
      }
  ...
  case CAM_STREAM_TYPE_PARM: {
    CLOGH(CAM_MCT_MODULE, "Start linking Session-stream 0x%x",
      stream->streaminfo.identity);
    sensor = mct_stream_get_module(modules, "sensor");
    iface  = mct_stream_get_module(modules, "iface");
    isp    = mct_stream_get_module(modules, "isp");
    ...
    if (!sensor || !iface || !isp || !stats || !pproc || !imglib) {
      CLOGE(CAM_MCT_MODULE, "Null: %p %p %p %p %p %p",
  ...
  case CAM_STREAM_TYPE_SNAPSHOT: {
    ...
    if (CAM_STREAM_TYPE_CALLBACK == stream->streaminfo.stream_type) {
      CLOGH(CAM_MCT_MODULE, "Start linking callback stream 0x%x",
        stream->streaminfo.identity);
    } else {
      CLOGH(CAM_MCT_MODULE, "Start linking snapshot stream 0x%x",
        stream->streaminfo.identity);
    }
    sensor = mct_stream_get_module(modules, "sensor");
    iface  = mct_stream_get_module(modules, "iface");
...
/*  Regular snapshot where PPROC is part of module link */
    ...
    if (stream->streaminfo.pp_config.feature_mask &
      ...
      if (pipeline->session_data.sensor_format != FORMAT_YCBCR) {
       ...
       ret = mct_stream_link_modules(stream, sensor, iface, pproc, NULL);
      }
    }
/*  ZSL Snapshot case, where PPROC is part of Offline stream*/
  ...
  case CAM_STREAM_TYPE_VIDEO: {
    CLOGH(CAM_MCT_MODULE, "Start linking video stream 0x%x",
      stream->streaminfo.identity);
    sensor = mct_stream_get_module(modules, "sensor");
    iface  = mct_stream_get_module(modules, "iface");
    isp    = mct_stream_get_module(modules, "isp");
  ...
  case CAM_STREAM_TYPE_RAW: {
    CLOGH(CAM_MCT_MODULE, "Start linking raw stream 0x%x",
      stream->streaminfo.identity);
    sensor = mct_stream_get_module(modules, "sensor");
    iface  = mct_stream_get_module(modules, "iface");
    isp    = mct_stream_get_module(modules, "isp");
    ...
    if (pipeline->session_data.sensor_format != FORMAT_YCBCR) {
      ...
      ret = mct_stream_link_modules(stream, sensor, iface, NULL);
      if (ret == FALSE) {
        CLOGE(CAM_MCT_MODULE, "link failed");
        return FALSE;
      }
  ...
  case CAM_STREAM_TYPE_ANALYSIS: {
    CLOGH(CAM_MCT_MODULE, "Starting Analysis stream linking");
    ...
    if (!sensor || !iface || !isp || !imglib || !pproc) {
      CLOGE(CAM_MCT_MODULE, "Null: %p %p %p %p %p",
        sensor, iface, isp, imglib, pproc);
      return FALSE;
    }
  ...
  case CAM_STREAM_TYPE_METADATA:
    CLOGH(CAM_MCT_MODULE, "Link Metadata stream 0x%x: do nothing",
      stream->streaminfo.identity);
    ret = TRUE;
    break;
  ...
  case CAM_STREAM_TYPE_OFFLINE_PROC: {
    ...
    CLOGH(CAM_MCT_MODULE, "Start linking offline stream 0x%x",
      stream->streaminfo.identity);
    ...
    if (stream->streaminfo.reprocess_config.pp_feature_config.feature_mask &
      ...
      CLOGH(CAM_MCT_MODULE, "Metadata collection for offline stream");
      return TRUE;
    }
   ...
   /*Check if PPROC module is needed based on feature mask*/
    ...
    if (!iface || !isp || !pproc || !imglib){
      CLOGH(CAM_MCT_MODULE, "iface=%p, isp=%p, pproc=%p, imglib=%p",
    ...
    if (iface){
      ...
      if (pproc){
        if (imglib){
          ...
          imglib->set_mod(imglib,MCT_MODULE_FLAG_SINK,
            stream->streaminfo.identity);
          CLOGH(CAM_MCT_MODULE, "Linking iface->isp->pproc->imglib");
        ...
        pproc->set_mod(pproc,MCT_MODULE_FLAG_SINK,
          stream->streaminfo.identity);
        CLOGH(CAM_MCT_MODULE, "Linking iface->isp->pproc");
        ret = mct_stream_link_modules(stream, iface, isp, pproc, NULL);
      } else {
        isp->set_mod(isp, MCT_MODULE_FLAG_SINK,
          stream->streaminfo.identity);
        CLOGH(CAM_MCT_MODULE, "Linking iface->isp");
      ...
      /* We have only one module to the stream
        ...
        CLOGH(CAM_MCT_MODULE, "Linking single module");
        if (imglib)
        ...
        single_module->set_mod(single_module, MCT_MODULE_FLAG_SOURCE,
          stream->streaminfo.identity);
        CLOGH(CAM_MCT_MODULE, "single_module=%p", single_module);
 ...
 *  Arguments/Fields:
 *    @pipeline: Container for frame_id<->frame_num match list
 *    @super_event structure of mct_event_super_control_parm_t
...
boolean mct_stream_map_frame_number(mct_pipeline_t *pipeline,
  mct_event_super_control_parm_t *super_event, uint32_t frame_id)
{
  uint32_t frame_num = super_event->frame_number;
  ...
  void *user_data __unused)
{
  ...
  CLOGH(CAM_MCT_MODULE, "stream_type = %d stream state = %d",
  ...
  if (MCT_ST_STATE_PENDING_RESTART == stream->state) {
    ...
    CLOGH(CAM_MCT_MODULE, "Stream state %d. Skipping", stream->state);
  }
  ...
  mct_stream_session_metadata_info *local)
{
  ...
  case MCT_BUS_MSG_TRIGGER_REGION:
   add_metadata_entry(CAM_INTF_META_ASF_TRIGGER_REGION,
                      sizeof(cam_asf_trigger_regions_t),
     psrc, pdst);
   break;
#endif
  ...
  default:
    CLOGE(CAM_MCT_MODULE, "Unsupported message type %d", type);
      /* fall through */
  }
  return TRUE;
}
  ...
  metadata_buffer_t **hal_buffer)
{
  ...
  if (stream->state != MCT_ST_STATE_RUNNING) {
    CLOGH(CAM_MCT_MODULE, "Metadata stream state: %d. Returning",
      stream->state);
    return FALSE;
  }
...
offline_meta_thread_exit:
  CLOGH(CAM_MCT_MODULE, "Exiting offline metadata thread");
    ...
    if ((mct_module_find_type(src_module, event->identity)
      ...
      if (sinkports) {
        ...
        if (port && port->event_func) {
          ...
          CLOGH(CAM_MCT_MODULE, "NULL ptr detected.");
        }
      } else {
        CLOGH(CAM_MCT_MODULE, "No sinkport found.");
      }
    }
  }
  ...
  mct_serv_ds_msg_t *msg)
{
  ...
  if (buf_holder->num_planes == 1) {
    if ((SECURE == stream_info->is_secure) &&
      ...
      CLOGH(CAM_MCT_MODULE, "Not going to map secure buffer");
      ...
      if (buf_holder->buf_planes[0].buf == MAP_FAILED) {
        CLOGE(CAM_MCT_MODULE, "Mapping failed with error %s",
          strerror(errno));
        goto error1;
      }
    }
    ...
    if (msg->plane_idx == -1) { /*same fd is used for all buffers*/
      ...
      if(SECURE == stream_info->is_secure) {
        CLOGH(CAM_MCT_MODULE, "Not going to map buffer");
          buf_holder->buf_planes[0].buf = NULL;
      } else {
        buf_holder->buf_planes[0].buf = mmap(NULL, msg->size,
          PROT_READ | PROT_WRITE, MAP_SHARED, msg->fd, 0);
...
static boolean mct_stream_streamoff(void *data, void *user_data)
{
  ...
  if (stream->state == MCT_ST_STATE_RUNNING) {
    ...
    event_data.type = MCT_EVENT_CONTROL_STREAMOFF;
    event_data.control_event_data = (void *)&stream->streaminfo;
    CLOGH(CAM_MCT_MODULE, "STREAMING OFF stream 0x%x",
      stream->streaminfo.identity);
    ret = mct_pipeline_pack_event(MCT_EVENT_CONTROL_CMD,
    (pack_identity(MCT_PIPELINE_SESSION(pipeline), stream->streamid)),
    MCT_EVENT_DOWNSTREAM, &event_data, &cmd_event);
    ...
    ret = stream->send_event(stream, &cmd_event);
    if (ret) {
      CLOGH(CAM_MCT_MODULE, "Stream 0x%x successfully streamed off.",
        stream->streaminfo.identity);
      stream->state = MCT_ST_STATE_IDLE;
      if ((stream->streaminfo.stream_type != CAM_STREAM_TYPE_METADATA) &&
...
static boolean mct_stream_del_offline_stream(void *data, void *user_data)
{
  ...
  ret = stream->send_event(stream, &cmd_event);
  /*set state to NONE even if downstream modules fails to handle DEL_STREAM
    notification */
  stream->state = MCT_ST_STATE_IDLE;
  return ret;
}
...
boolean mct_stream_destroy(mct_stream_t *stream)
{
  ...
  if (MCT_OBJECT_CHILDREN(stream))
  {
    ...
    else if (stream->streaminfo.stream_type == CAM_STREAM_TYPE_OFFLINE_PROC)
    {
      ...
      mct_stream_del_offline_stream(stream, pipeline);
    }
