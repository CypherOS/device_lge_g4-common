...
typedef boolean (* mct_module_set_session_data)
  (mct_module_t *module, void *set_buf,
  unsigned int sessionid);

typedef boolean (* mct_module_get_session_data)
  (mct_module_t *module, void *get_buf,
  unsigned int sessionid);
...
#define MCT_MODULE_LOCK(mod)          (MCT_OBJECT_LOCK(mod))
#define MCT_MODULE_UNLOCK(mod)        (MCT_OBJECT_UNLOCK(mod))
...
#define mct_module_set_session_data_func(mod,f)  \
  MCT_MODULE_CAST(mod)->set_session_data = f

#define mct_module_get_session_data_func(mod,f)  \
  MCT_MODULE_CAST(mod)->get_session_data = f
