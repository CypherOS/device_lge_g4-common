...
boolean mct_module_post_bus_msg(mct_module_t *module, mct_bus_msg_t *bus_msg)
{
  ...
  if (!meta_stream) {
    CLOGE(CAM_MCT_MODULE, "Not valid metadata in pipeline for session id %d",
      pipeline->session);
    return FALSE;
  }
  ...
  switch (bus_msg->type){
    ...
    case MCT_BUS_MSG_ISP_RD_DONE:
      ...
      break;
    default:
      CLOGD(CAM_MCT_MODULE, "Not a priority bus message! bus_msg=%d",
        bus_msg->type);
     break;
  }

  /* If the metadata stream is running then only fill the metadata buffer */
