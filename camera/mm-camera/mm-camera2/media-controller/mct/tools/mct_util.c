 ...
 *    @enforce_positive_diff: Compare only if ref_table.fps >= src_fps,
 *                                  thus always ensuring that diff is positive.
 *
 *  Return: index of closest matching entry from reference fps table
 *             that is smaller than src_fps.
 **/
uint8_t mct_util_find_closest_fps (
    float src_fps, cam_fps_range_t* ref_table, uint8_t num_entries)
{
  ...
  for (index = 0; (index < num_entries && index < MAX_SIZES_CNT); index++) {
    if (ref_table[index].max_fps <= src_fps) {
