...
void eztune_server_start ()
{
    ...
    if (tuning_lib == NULL) {
        ...
        CDBG_HIGH("Tuning lib not opened, eztune disabled: %s", dlerror());
        goto end;
    }
    ...
    if(!tuning_control_handle && !tuning_preview_handle) {
        ...
        if(!port || !pipeline ||
           pipeline->query_data.sensor_cap.sensor_format == FORMAT_YCBCR) {
            eztune_server_stop(module_eztune); //this clear the module_eztune ptr
            CDBG_HIGH("EZTune: server not starting as eztune is disabled or sensor is YUV");
        } else {
            eztune_server_start();
            if(!(tuning_control_handle && tuning_preview_handle)) {
                CDBG_ERROR("EZTune: error starting server");
            }
        }
    }
...
mct_pipeline_t *eztune_get_pipeline()
{
    ...
    if (!module_eztune || !MCT_OBJECT_PRIVATE(module_eztune)) {
        CDBG_ERROR("module is NULL, pipeline not open %p", module_eztune);
        return NULL;
    }
    ...
    if (!module || !MCT_MODULE_PARENT(module)) {
        CDBG_ERROR("module/stream is NULL, pipeline not yet initialized %p", module);
        return NULL;
    }
    stream = MCT_STREAM_CAST((MCT_MODULE_PARENT(module))->data);
    if(!stream || !MCT_STREAM_PARENT(stream)) {
        CDBG_ERROR("stream/pipeline is NULL, pipeline not yet initialized %p", stream);
        return NULL;
    }
    pipeline = MCT_PIPELINE_CAST((MCT_STREAM_PARENT(stream))->data);
    if(!pipeline) {
        CDBG_ERROR("pipeline is NULL, pipeline not yet initialized %p", pipeline);
        return NULL;
    }
