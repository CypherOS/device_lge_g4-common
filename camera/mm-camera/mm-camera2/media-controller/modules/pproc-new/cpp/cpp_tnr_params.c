  ...
  float isp_scale_ratio, float cpp_scale_ratio)
{
  ...
  for ( i = 0 ; i < MAX_LIGHT_TYPES_FOR_SPATIAL ; i ++ ) {
    if(trigger_start[i] > trigger_end[i]) {
      CPP_ERR("Strat trigger point is bigger than end trigger point");
      continue;
    }
  ...
  if (interp_ratio > 1.0) {
    CPP_ERR("Invalid interpolation ratio %f", interp_ratio);
    interp_ratio = 1.0;
  }
  ...
  cpp_hardware_params_t *hw_params, cpp_params_aec_trigger_info_t *aec_trigger)
{
  ...
  if (hw_params->tnr_denoise_lock) {
    CPP_ERR("Temporal wavelet denoise is locked by Chromatix");
    return 0;
  }
      ...
      if (session_params->stream_params[i]) {
        ...
        if (session_params->stream_params[i]->hw_params.tnr_mask) {
          ...
          case CAM_STREAM_TYPE_OFFLINE_PROC:
            ...
            break;
          default:
            CPP_ERR("TNR not supported for stream %d",
  ...
  uint32_t identity)
{
  ...
  if (!stream_params->hw_params.tnr_mask) {
    CPP_ERR("TNR feature mask not enabled %d for stream %d iden %x",
  ...
  if (rc < 0) {
    CPP_ERR("Buffer allocation failed\n");
    rc =  -EFAULT;
    goto end;
  }
  ...
  if (rc < 0) {
    CPP_ERR("fill_hw_buff_list error %d\n", rc);
    goto tnr_buf_list_error;
  }
  ...
  uint32_t identity)
{
  ...
  if (!stream_params->hw_params.tnr_mask) {
    CPP_ERR("TNR feature mask not enabled %d",
      stream_params->hw_params.tnr_mask);
    rc = 0;
    goto end;
  }
  ...
  for (j = 0; j < CPP_TNR_SCRATCH_BUFF_COUNT; j++) {
    ...
    if (rc < 0) {
      CPP_ERR("Fail to put buffer index %d fd %d",buff_holder->index,
        buff_holder->fd);
      continue;
    }
    buff_holder->fd = 0;
