...
#undef CPP_LOW
#define CPP_LOW(fmt, args...) do {} while(0)
...
void set_start_of_frame_parameters (struct cpp_plane_info_t *in_ary)
{
  ...
  CPP_LOW("Aligned block height: %d", in_ary->dst_height_block_aligned);
  ...
  struct cpp_striping_algorithm_info *stripe_info, uint32_t num_stripes)
{
  ...
  CPP_DBG("num_stripes %d", num_stripes);
...
void cpp_debug_bf_info(struct cpp_bf_info *bf_info __unused)
{
  ...
  for (i = 0; i < 4; i++) {
    ...
    CPP_DENOISE_LOW("bilateral_scale(bf cfg0)[%d]: %f\n",
      i, bf_info->bilateral_scale[i]);
    CPP_DENOISE_LOW("noise_threshold[%d]: %f\n", i, bf_info->noise_threshold[i]);
    CPP_DENOISE_LOW("weight[%d]: %f\n", i, bf_info->weight[i]);
    CPP_DENOISE_LOW("bf cfg1: 0x%x\n", (uint32_t)
...
#ifdef LDS_ENABLE
  if (lds_enable) {
    int32_t start_idx = NUM_GRAY_PATCHES;
    if (aec_trigger_params->aec_trigger_input <=
      ...
      CPP_ERR("LDS:invalid input: should not reach here.\n");
    }
  ...
  int32_t region_trg_num)
{
  ...
  if (region_trg_num < 1 || region_trg_num > CPP_PARAM_ASF_TRIGGER_MAX) {
    CPP_ERR("failed, region_trg_num=%d", region_trg_num);
    return -EINVAL;
  }
...
#endif
  default:
    CPP_ERR("invalid asf_region=%d", asf_region);
    rc = -EINVAL;
    break;
  }
  return rc;
  ...
  float effect_factor)
{
  ...
  if (interpolate) {
    if (interpolate_factor == 0.0f) {
      CPP_ERR("interpolate_factor=0.0f");
      return -EINVAL;
    }
    CPP_LOW("asf interpolation=yes factor=%f, region=(%d, %d)",
    ...
    CPP_LOW("asf interpolation=no region=%d", reg1);
  ...
  cpp_params_aec_trigger_t *aec_trigger_params)
{
  ...
  if (!module_chromatix || !module_chromatix->chromatixCppPtr) {
    CPP_ERR("failed, chromatix NULL, using default params");
    return -EINVAL;
  }
  ...
  cpp_params_aec_trigger_info_t *trigger_input)
{
  ...
  if (hw_params->asf_lock) {
    CPP_ERR("ASF is locked by Chromatix");
    return 0;
  }
  ...
  if (chromatix_ASF_7x7->control_asf_7x7 == 0) {
    ...
    for (i=0; i<CPP_PARAM_ASF_TRIGGER_MAX; i++) {
    ...
    /* Error in chromatix */
    CPP_ERR("invalid chromatix control type");
    return -EINVAL;
  }
  for (i=0; i<CPP_PARAM_ASF_TRIGGER_MAX; i++) {
    CPP_DBG("start[%d]=%f, end[%d]=%f", i,
  ...
  cpp_params_asf_region_t asf_region)
{
  ...
  case CPP_PARAM_ASF_LOW_LIGHT_INTERPOLATE:
    interpolate_factor = cpp_hw_params_calculate_interpolate_factor(
      ...
      aec_trigger_params->aec_trigger_input);
    if (interpolate_factor == 0.0f) {
      CPP_LOW("interpolate_factor zero");
      ret = -EINVAL;
      break;
    }
  ...
  cpp_params_aec_trigger_t *aec_trigger_params)
{
  ...
  if (hw_params->asf_mode == CPP_PARAM_ASF_DUAL_FILTER || hw_params->scene_mode_on) {
    ...
    CPP_LOW("min_ds %f max_ds %f max_factor %f",
      asf_info->sharp_min_ds_factor, asf_info->sharp_max_ds_factor,
      asf_info->sharp_max_factor);
  ...
  cpp_params_asf_region_t asf_region)
{
  ...
  /* Kernel coefficients are not interpolated across regions */
  switch (asf_region) {
  case CPP_PARAM_ASF_REGION_1:
  case CPP_PARAM_ASF_REGION_12_INTERPOLATE:
    region_num = 1;
  ...
  case CPP_PARAM_ASF_REGION_78_INTERPOLATE:
    region_num = 7;
    CPP_ASF_DBG("LDS: region_num %d", region_num);
    break;
  case CPP_PARAM_ASF_REGION_8:
    region_num = 8;
    CPP_ASF_DBG("LDS: region_num %d ", region_num);
  ...
  float scale_adj, float effect_factor, float asf_hdr_adj_factor)
{
  ...
  if (asf_info->info_1_6_x.cpp_1_12_ext.is_valid == true) {
    cpp_hw_params_conv_float_lut64_to_fixed_lut256(
      gain_neg_lut, asf_info->info_1_6_x.cpp_1_12_ext.gain_neg_lut, 8,
      FALSE, thres1, thres2);
  }
  ...
  struct asf_1_12_rnr_output_t *output_rnr_cfg)
{
  ...
  if (output_rnr_cfg == NULL) {
    CPP_ERR("invalid ptr output_rnr_cfg = %p", output_rnr_cfg);
    return false;
  }
  ...
  /*todo: do we need to consider sensor crop???*/
  output_rnr_cfg->rnr_start_offset_h = 0;
  output_rnr_cfg->rnr_start_offset_v = 0;
...
static int32_t cpp_hw_params_fill_lut_params_1_6_x(
  ...
  cpp_params_aec_trigger_t *aec_trigger_params,
  cpp_params_asf_region_t asf_region, float scale_adj,
  float effect_factor, float asf_hdr_adj_factor)
{
  float   interpolate_factor = 0.0f;
  boolean interpolate=FALSE;
  int32_t reg1, reg2;
  ...
  case CPP_PARAM_ASF_REGION_7:
    reg1 = 7;
    interpolate = FALSE;
    CPP_ASF_DBG("LDS: reg %d, interpolate %d", reg1, interpolate);
    break;
  case CPP_PARAM_ASF_REGION_78_INTERPOLATE:
    reg1 = 7; reg2 = 8;
    interpolate = TRUE;
    CPP_ASF_DBG("LDS: reg [%d %d], interpolate %d", reg1, reg2, interpolate);
    interpolate_factor = cpp_hw_params_calculate_interpolate_factor(
      aec_trigger_params->trigger_start[6],
      aec_trigger_params->trigger_end[6],
      aec_trigger_params->aec_trigger_input);
  ...
  if (asf_info->info_1_6_x.cpp_1_12_ext.is_valid == true) {
    ...
    if (ret == false) {
      CPP_ERR("failed: cpp_hw_param_calc_rnr_param failed!");
      return -EINVAL;
    }
  }
...
static void cpp_hw_params_get_aec_trigger_params(
  cpp_hardware_params_t *hw_params __unused, chromatix_cpp_type *chromatix_cpp,
  cpp_params_aec_trigger_info_t *trigger_input,
  cpp_params_aec_trigger_t *aec_trigger_params)
{
  uint32_t i = 0;
  ...
  chromatix_asf = &(chromatix_cpp->chromatix_ASF_9x9);
  GET_ASF_POINTER (chromatix_cpp, hw_params, chromatix_asf);

  CPP_LOW("control_type=%d", chromatix_asf->control_asf_9x9);
  switch (chromatix_asf->control_asf_9x9) {
  case CPP_ASF_CONTROL_LUX_BASED: {
      for (i=0; i<CPP_PARAM_ASF_TRIGGER_MAX; i++) {
        aec_trigger_params->trigger_start[i] =
          chromatix_asf->asf_9_9[i].asf_9x9_trigger.lux_index_start;
        aec_trigger_params->trigger_end[i] =
          chromatix_asf->asf_9_9[i].asf_9x9_trigger.lux_index_end;
      }
      aec_trigger_params->aec_trigger_input = trigger_input->lux_idx;
      break;
  }
  case CPP_ASF_CONTROL_GAIN_BASED: {
      for (i=0; i<CPP_PARAM_ASF_TRIGGER_MAX; i++) {
        aec_trigger_params->trigger_start[i] =
          chromatix_asf->asf_9_9[i].asf_9x9_trigger.gain_start;
        aec_trigger_params->trigger_end[i] =
          chromatix_asf->asf_9_9[i].asf_9x9_trigger.gain_end;
      }
      aec_trigger_params->aec_trigger_input = trigger_input->gain;
      break;
  }
  default:
    CPP_ERR("invalid control_asf_9x9=%d", chromatix_asf->control_asf_9x9);
  }

  for (i=0; i<CPP_PARAM_ASF_TRIGGER_MAX; i++) {
    CPP_LOW("start[%d]=%f, end[%d]=%f", i,
      aec_trigger_params->trigger_start[i], i,
      aec_trigger_params->trigger_end[i]);
  }
  ...
  cpp_params_aec_trigger_info_t *aec_trigger)
{
  ...
  if (!hdr_asf_adj_type_ptr) {
    CPP_ERR("failed: No ASF HDR chromatix!!");
    return -1;
  }
  ...
  cpp_params_aec_trigger_info_t *trigger_input)
{
  ...
  cpp_hw_params_get_aec_trigger_params( hw_params, chromatix_cpp, trigger_input,
    &aec_trigger_params);
  CPP_LOW("aec_trigger_input=%f", aec_trigger_params.aec_trigger_input);

  /* Fill ASF parameters in frame_params */
  if (!chromatix_asf->asf_9x9_enable ||
    aec_trigger_params.aec_trigger_input <= 0.0f) {
    /* Use default */
    CPP_HIGH("using default asf params, asf_en=%d, trig_in=%f",
      chromatix_asf->asf_9x9_enable, aec_trigger_params.aec_trigger_input);
    cpp_hw_params_update_default_asf_params_1_6_x(hw_params);
    return ret;
  }
  CPP_LOW("asf_mode = %d", hw_params->asf_mode);
  switch (hw_params->asf_mode) {
  case CPP_PARAM_ASF_OFF:
      break;
  case CPP_PARAM_ASF_BEAUTYSHOT:
  ...
  case CPP_PARAM_ASF_DUAL_FILTER:
      ...
      CPP_LOW("asf_region=%d", asf_region);
      asf_info->region = asf_region;

      /* update non-interpolated params */
      ret = cpp_hw_params_fill_asf_kernel_1_6_x(asf_9_9, asf_info, asf_region);
      if (ret < 0) {
        CPP_ERR("failed");
        return ret;
      }
      ...
      if (ASF_RNR) {
        asf_info->info_1_6_x.cpp_1_12_ext.asf_1_12_rnr_input.h_center =
          ASF_RNR_CENTER_H(chromatix_asf);
        asf_info->info_1_6_x.cpp_1_12_ext.asf_1_12_rnr_input.v_center =
          ASF_RNR_CENTER_V(chromatix_asf);
        for (i = 0; i < ASF_RNR_IN_ENTRIES; i++) {
      ...
      ret = cpp_hw_params_fill_lut_params_1_6_x(asf_9_9, asf_info,
              &aec_trigger_params, asf_region, hw_params->asf_scalor_adj,
              effect_factor, hw_params->asf_hdr_adj);
      if (ret < 0) {
        CPP_ERR("failed");
        return ret;
      }
  ...
  case CPP_PARAM_ASF_NEON:
      ...
      break;
  default:
      CPP_ERR("invalid asf_mode=%d", hw_params->asf_mode);
      return -EINVAL;
  }
  return 0;
}
  ...
  cpp_params_aec_trigger_info_t *trigger_input )
{
 ...
 !defined (CHROMATIX_308E) && !defined (CHROMATIX_309) && !defined (CHROMATIX_310)
  ...
  CPP_HIGH("chromatix 303 not available, using default ASF params");
  ...
  cpp_params_aec_trigger_info_t *trigger_input)
{
  if (!cpphw || !hw_params || !module_chromatix || !trigger_input) {
    CPP_ERR("failed,cpphw=%p, hw_params=%p, chromatix_ptr=%p, trigger_input=%p",
      cpphw, hw_params, module_chromatix, trigger_input);
    return -EINVAL;
  }
  cpp_firmware_version_t fw_version =
    cpp_hardware_get_fw_version(cpphw);
  CPP_LOW("fw_version = %x", fw_version);
  switch (fw_version) {
  case CPP_FW_VERSION_1_2_0:
  case CPP_FW_VERSION_1_4_0:
  ...
  case CPP_FW_VERSION_1_12_0:
    ...
    return cpp_hw_params_asf_interpolate_1_6_x(hw_params,
             module_chromatix, trigger_input);
  default:
    CPP_ERR("failed, unsupported fw version: 0x%x", fw_version);
    return -EINVAL;
  }
  return 0;
}
  ...
  void *ref_noise_profile_iplus1)
{
  ...
  if (IS_CPP_ERROR(cpp_wnr_params_rnr_update(
    ...
    ref_noise_profile_iplus1, interpolation_factor))) {
  ...
  *reg_strt_interp_ratio = 0.0f ;
  ...
  for ( i = 0 ; i < DOWN_SCALING_LEVELS ; i ++ ) {
    ...
    if ( scale_strt >= scale_end ) {
      CPP_LOW("WNR Down Scaling Ratio Settings are not in correct order");
  ...
  float *reg_strt_interp_ratio, uint32_t scalor __unused)
{
  ...
  *reg_strt_interp_ratio = 0.0f ;
  for ( i = 0 ; i < UP_SCALING_LEVELS ; i ++ ) {
    ...
    if ( scale_strt <= scale_end ) {
      CPP_LOW("WNR Up Scaling Ratio Settings are not in correct order");
      *reg_strt_idx_start = i;
      *reg_strt_idx_end = i;
      return;
    }
  ...
  cpp_params_aec_trigger_info_t *aec_trigger __unused)
{
  ...
  chromatix_cpp = (chromatix_cpp_type *)module_chromatix->chromatixCppPtr;
  if (!chromatix_cpp) {
    CPP_ERR("failed: No CPP chromatix!!");
    return -1;
  }
  ...
  cpp_params_aec_trigger_info_t *aec_trigger)
{
  ...
  if(GET_ADRC_WNR_ADJ_CONTROL_TYPE(adrc_wnr_adj_type_ptr) == CONTROL_AEC_EXP_SENSITIVITY_RATIO){
    //CONTROL_AEC_EXP_SENSITIVITY_RATIO
    if (aec_trigger->total_adrc_gain <= 0.0f){
      CPP_ERR("total_adrc_gain less then zero!!");
      return -1;
    }
    trigger_input = aec_trigger->aec_sensitivity_ratio;
  ...
  for (i = 0; i < end_idx; i++) {
    if(GET_ADRC_WNR_ADJ_CONTROL_TYPE(adrc_wnr_adj_type_ptr) == CONTROL_AEC_EXP_SENSITIVITY_RATIO){
      //CONTROL_AEC_EXP_SENSITIVITY_RATIO
      CPP_DENOISE_DBG("zzadrc exp sens ratio!");
      trigger_start =
  ...
  cpp_hardware_params_t *hw_params, cpp_params_aec_trigger_info_t *aec_trigger)
{
  ...
  CPP_LOW("cpp_hw_params_update_wnr_adj_params");
  ...
  cpp_hardware_params_t *hw_params, cpp_params_aec_trigger_info_t *aec_trigger)
{
  ...
  if (hw_params->denoise_lock) {
    CPP_ERR("Wavelet denoise is locked by Chromatix");
    return 0;
  }
  ...
  if (!chromatix_cpp) {
    ...
    if (trigger_input <= 0.0f) {
      CPP_ERR("invalid trigger input %f", trigger_input);
      return 0;
    }
...
int32_t cpp_params_calculate_crop(cpp_hardware_params_t *hw_params)
{
  ...
  if (!F_EQUAL(cpp_scale_ratio, 1.0f)) {
    ...
    hw_params->crop_info.process_window_first_line *=
      (float)(cpp_scale_ratio/hw_params->cpp_scale_ratio);
    CPP_ERR("Fatal, FOV compromised, scale ratio limit %f, corrected scale %f,"
 ...
 /* Hack to equalize the snpashots in ZSL and NON-ZSl mode when flip is not 0 */
  ...
  for(i = 0; i < plane_count; i++) {
      ...
      if (UBWC_ENABLE(hw_params->output_info.plane_fmt)) {
        ...
        plane_info[i].tile_byte_width = LUMA_TILE_WIDTH;
  ...
  CPP_LOW("bf_enable:%d", hw_params->denoise_enable);
  return 0;
}
