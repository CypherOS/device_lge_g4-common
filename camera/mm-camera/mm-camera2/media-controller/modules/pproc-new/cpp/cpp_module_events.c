  ...
  uint32_t identity, uint32_t div_identity,uint32_t duplicate_identity)
{
  ...
  CPP_LOW("proc_identity 0x%x proc_div_identity 0x%x proc_div_required %d\n",
  ...
  cpp_module_session_params_t *session_params, uint32_t frame_id)
{
  ...
  if (stream_params->hfr_skip_info.skip_required &&
    ...
    if ((cpp_decide_hfr_skip(frame_id -
      ...
      /* Skip this frame */
      CPP_LOW("skipping frame_id=%d for identity=0x%x",
        frame_id, stream_params->identity);
      CPP_LOW("skip_count=%d, offset=%d",
  ...
  cpp_module_process_list_t *hw_process_list)
{
  ...
  CPP_DBG("====dump CPP reprocessing list====");
  CPP_DBG("num_hw_process = %d", hw_process_list->num_hw_process);
  for (i = 0; i < hw_process_list->num_hw_process; i++) {
    ...
    if (hw_proc_info->process_stream != NULL &&
      ...
      CPP_DBG(" hw_proc_list[%d]: stream id %x, dup identity %x", i,
  ...
  cpp_module_process_list_t *hw_process_list)
{
  ...
  if (!module || !stream_list) {
    CPP_ERR("null ptr %p %p", module, stream_list);
    return FALSE;
  }

  if (num_streams > CPP_MODULE_MAX_STREAMS) {
    CPP_ERR("num_stream %d > max supported %d",
  ...
  for (i = 0; i < num_streams; i++) {
    ...
    if(!stream_params1 || stream_params1->identity == 0) {
      CPP_LOW("can not find valid stream\n");
      continue;
    }
  ...
  cpp_module_process_list_t *hw_process_list)
{
  ...
  if (stream_params == NULL || hw_process_list == NULL) {
    CPP_ERR(" null pointer %p %p", stream_params, hw_process_list);
    return -1;
  }
 ...
 /* linked stream case */
  ...
  if (stream_list_size == 0 && stream_list_size > CPP_MODULE_MAX_STREAMS) {
    CPP_ERR("num stream to handle = %d error!!\n",
      stream_list_size);
    return -1;
  }
  ...
  isp_buf_divert_t *isp_buf)
{
  ...
  if (!module || !session_params || !process_stream_params || !isp_buf) {
    CPP_ERR("failed! null ptr %p %p %p %p",
  ...
  uint32_t identity, isp_buf_divert_t *isp_buf)
{
  ...
  if(!module || !isp_buf) {
    CPP_ERR("failed, module=%p, isp_buf=%p\n", module, isp_buf);
    return -EINVAL;
  }
  ...
  CPP_DBG("pp_dbg  num_hw process %d", hw_process_list.num_hw_process);
  ...
  for (j = 0; j < hw_process_list.num_hw_process; j++) {
    ...
    CPP_DBG("cur stream _param %p process stream param %p",
    ...
    if (ret < 0) {
      CPP_ERR("cpp_module_prepare_hw_parm error!");
  ...
  if (ctrl->cpp_thread_started) {
    if (num_events != 0) {
      ...
      isp_buf->ack_flag = TRUE;
      isp_buf->is_buf_dirty = 1;
      CPP_DBG("buffer event received with no divert config\n");
      PTHREAD_MUTEX_UNLOCK(&(ctrl->cpp_mutex));
      MCT_OBJECT_UNLOCK(stream_port);
      return 0;
    }
    ...
    isp_buf->ack_flag = TRUE;
    isp_buf->is_buf_dirty = 1;
    CPP_DBG("Thread not started, piggy back ack\n");
    PTHREAD_MUTEX_UNLOCK(&(ctrl->cpp_mutex));
    MCT_OBJECT_UNLOCK(stream_port);
    return -EINVAL;
  }
  ...
  CPP_LOW("downstream event time = %6ld us",
  ...
  mct_event_t* event)
{
  ...
  if (session_params->dis_enable == 0) {
    ...
    if ((stream_params->stream_type == CAM_STREAM_TYPE_PREVIEW) ||
      ...
      CPP_BUF_DBG("live snapshot send %d for processing\n",
        isp_buf->buffer.sequence);
      cpp_module_send_buf_divert_event(module, event->identity, isp_buf);
    }
  } else {
     CPP_BUF_DBG("DIS crop event not sent yet %d for processing\n",
       isp_buf->buffer.sequence);
    /* DIS is disabled. Send frame for processing */
    cpp_module_send_buf_divert_event(module, event->identity, isp_buf);
  ...
  mct_event_t* event)
{
  ...
  if (stream_frame_valid) {
    ...
    CPP_ERR("ISP buf drop for id %x frame %d \n",
    ...
    CPP_ERR("ISP buf drop for invalid frame\n");
    return -EINVAL;
  }
      ...
      for(i = 0; i < CPP_MODULE_MAX_STREAMS; i++) {
        if(session_params->stream_params[i] &&
          ...
          cpp_module_update_aec(module,
            session_params->stream_params[i]->identity, &aec_trigger);
          /*OIS Capture */
          session_params->stream_params[i]->hw_params.low_light_capture_enable =
            aec_update->low_light_capture_update_flag;
        }
      }
    }
  ...
  uint32_t identity, int32_t value)
{
  ...
  if (stream_params->stream_type == CAM_STREAM_TYPE_OFFLINE_PROC) {
    CPP_DBG("Update only stream params iden:0x%x\n", identity);
    PTHREAD_MUTEX_LOCK(&(stream_params->mutex));
  ...
  uint32_t identity, cam_sensor_hdr_type_t parm_hdr_mode)
{
  ...
  CPP_LOW("[HDR] iden:0x%x HDR mode:%d\n", identity, parm_hdr_mode);

  if (stream_params->stream_type == CAM_STREAM_TYPE_OFFLINE_PROC) {
    CPP_LOW("Update only stream params iden:0x%x\n", identity);
    PTHREAD_MUTEX_LOCK(&(stream_params->mutex));
    stream_params->hw_params.hdr_mode = parm_hdr_mode;
    CPP_LOW("[HDR] OFFLINE stream type %d, HDR mode %d",
      stream_params->stream_type,
    ...
    for (i = 0; i < CPP_MODULE_MAX_STREAMS; i++) {
      if (session_params->stream_params[i] &&
        ...
        CPP_LOW("[HDR] stream type %d, HDR mode %d",
          session_params->stream_params[i]->stream_type,
  ...
  mct_event_type mct_type, void *payload, uint32_t identity)
{
  ...
  if (!per_frame_params || !payload) {
    CPP_ERR("failed per_frame_params:%p, payload:%p\n",
  ...
  if (MCT_EVENT_MODULE_EVENT == mct_type) {
    ...
    case MCT_EVENT_MODULE_STATS_AEC_UPDATE:
      CPP_LOW("MCT_EVENT_MODULE_STATS_AEC_UPDATE\n");
    ...
    if (parm_data) {
      ...
      CPP_DBG("module event not for CPP\n");
      free(frame_ctrl_data);
    }
  } else if (MCT_EVENT_CONTROL_CMD == mct_type) {
    CPP_DBG("CPP_FRM_CTRL_Q_APPLY enqueue apply_frmid %d"
    ...
    case CAM_INTF_PARM_FPS_RANGE:
      CPP_LOW("CAM_INTF_PARM_FPS_RANGE\n");
    ...
    case CAM_INTF_PARM_ROTATION: {
      ...
      CPP_LOW("CAM_INTF_PARM_ROTATION\n");
    ...
    case CAM_INTF_META_STREAM_ID:
      CPP_DBG("CAM_INTF_META_STREAM_ID\n");
    ...
    case CAM_INTF_PARM_CDS_MODE:
      CPP_LOW("CAM_INTF_PARM_CDS_MODE\n");
      parm_data = calloc(1, sizeof(int32_t));
      if (parm_data) {
        *(int32_t *)parm_data =
    ...
    case CAM_INTF_META_IMG_DYN_FEAT:
      CPP_LOW("CAM_INTF_META_IMG_DYN_FEAT\n");
    ...
    case CAM_INTF_PARM_SENSOR_HDR:
      CPP_LOW("CAM_INTF_PARM_SENSOR_HDR\n");
  ...
  uint32_t cur_frame_id, uint32_t process_type)
{
  ...
  for (i = 0; i < queue_len; i++) {
    ...
    if (frame_ctrl_data->q_entry_type == CPP_FRM_CTRL_Q_APPLY) {
      ...
      CPP_ERR("failed: type:%d rc %d", frame_ctrl_data->u.ctrl_param.type, rc);
      /* Entry is invalid */
      if (MCT_EVENT_MODULE_EVENT == frame_ctrl_data->mct_type)
        free(frame_ctrl_data->u.module_event.module_event_data);
      else
  ...
  boolean *param_processed, uint32_t current_frame_id)
{
  ...
  case CAM_INTF_PARM_BESTSHOT_MODE: {
    int32_t value = *(int32_t*)(parm_data);
    CPP_DBG("CAM_INTF_PARM_BESTSHOT_MODE, value=%d, identity=0x%x",
      value, identity);
  ...
  case CAM_INTF_META_NOISE_REDUCTION_MODE: {
    int32_t value = *(int32_t*)(parm_data);
    CPP_DBG("CAM_INTF_META_NOISE_REDUCTION_MODE, value=%d,"
  ...
  case CAM_INTF_PARM_ROTATION: {
    ...
    CPP_DBG("CAM_INTF_PARM_ROTATION,rotation %d", rotation.rotation);
  ...
  case CAM_INTF_META_STREAM_INFO : {
    ...
    cpp_module_stream_params_t  *stream_params = NULL;
    uint32_t i;
    CPP_DBG("CAM_INTF_META_STREAM_INFO\n");
    /* get parameters based on the event identity */
    rc = cpp_module_get_params_for_identity(ctrl, identity, &session_params,
      &stream_params);
    if(!session_params) {
  ...
  case CAM_INTF_PARM_DIS_ENABLE: {
    ...
    CPP_DBG("CAM_INTF_PARM_DIS_ENABLE, enable=%d, identity=0x%x",
      dis_enable, identity);
  ...
  case CAM_INTF_PARM_SET_PP_COMMAND: {
    ...
    CPP_LOW("Received CAM_INTF_PARM_SET_PP_COMMAND event\n"
  ...
  case CAM_INTF_PARM_CDS_MODE: {
    ...
    if(rc < 0) {
      CPP_ERR("CAM_INTF_PARM_CDS_MODE failed");
      return rc;
    }
    break;
  }
  case CAM_INTF_META_IMG_DYN_FEAT: {
    ...
    if(rc < 0) {
      CPP_ERR("CAM_INTF_META_IMG_DYN_FEAT failed");
      return rc;
    }
    break;
  }
  case CAM_INTF_PARM_SENSOR_HDR: {
    ...
    CPP_LOW("CAM_INTF_PARM_SENSOR_HDR %d", hdr_type);
    rc = cpp_module_set_parm_hdr(ctrl, identity, hdr_type);
    if (rc < 0) {
      CPP_LOW("CAM_INTF_PARM_SENSOR_HDR failed");
      return rc;
    }
    break;
  }
  ...
  mct_event_t *event)
{
  ...
  if (session_params->is_stream_on == FALSE) {
    ...
    if((cur_frame_id <= apply_frame_id) &&
      ...
      CPP_ERR("frame to apply is missed cur %d apply %d\n",
        cur_frame_id, apply_frame_id);
      return -EFAULT;
    }
  }
  ...
  mct_event_t* event, boolean delete_buf)
{
 ...
 /* synchronize with stream on/off/buffer divert decision */
  ...
  if(rc < 0) {
    CPP_ERR("CPP_HW_CMD_BUF_UPDATE failed\n");
    goto CPP_MODULE_BUF_UPDATE_ERROR2;
  }
  rc = cpp_module_send_event_downstream(module,event);
  if(rc < 0) {
    CPP_ERR("send down stream failed\n");
    goto CPP_MODULE_BUF_UPDATE_ERROR2;
  }
  ...
  mct_event_t* event)
{
  ...
  cpp_module_set_clock_freq(ctrl, module, stream_params, 1);
  ...
  mct_event_t* event)
{
  ...
  cpp_module_set_clock_freq(ctrl, module, stream_params, 0);
  ...
  if ((ctrl->tnr_module_func_tbl.unprepare) &&
    (stream_params->hw_params.tnr_mask)) {
      ...
      if (rc < 0) {
        CPP_ERR("TNR unprepare failed %d\n", rc);
      }
  }
  ...
  if (real_time_stream_cnt) {
...
#ifdef ASF_OSD
    {
      ...
      if(parent_module == NULL) {
        CPP_ERR("failed, parent module NULL");
        return -EFAULT;
      }
      ...
      cpp_module_util_convert_asf_region_type(&regions,
        session_params->hw_params.asf_info.region);
      else
      cpp_module_util_convert_asf_region_type(&regions,
        stream_params->hw_params.asf_info.region);
      #endif

      CPP_LOW("sending bus msg, session_id=%d, R1=%d, R2=%d",
        bus_msg.sessionid, regions.region1, regions.region2);

      if (mct_module_post_bus_msg(parent_module, &bus_msg) != TRUE) {
        CPP_ERR("mct_module_post_bus_msg() failed. session_id=%d",
          bus_msg.sessionid);
      }
   }
#endif
  ...
  mct_event_t *event)
{
  ...
  CPP_DBG("process frmid:%d q_idx:%d \n", skip_frame_id, q_idx);
  pthread_mutex_lock(&per_frame_params->frame_ctrl_mutex[q_idx]);
  queue_len = per_frame_params->frame_ctrl_q[q_idx]->length;
  for (i = 0; i < queue_len; i++) {
    queue_compatible = cpp_module_pop_per_frame_entry(ctrl, per_frame_params,
  ...
  uint32_t cur_frame_id, cam_intf_parm_type_t type)
{
  ...
  if (!ctrl || !per_frame_params) {
    CPP_ERR("failed ctrl:%p, per_frame_params:%p\n",
  ...
  case CAM_INTF_PARM_CDS_MODE:
    ...
    break;
  case CAM_INTF_META_IMG_DYN_FEAT:
    CPP_DENOISE_LOW("CAM_INTF_META_IMG_DYN_FEAT\n");
    parm_data =  calloc(1, sizeof(cam_dyn_img_data_t));
    if (parm_data) {
      ((cam_dyn_img_data_t *)parm_data)->dyn_feature_mask |=
  ...
  int32_t value)
{
  ...
  if (!ctrl->cpphw) {
    CPP_ERR("failed cpphw %p", ctrl->cpphw);
    rc = -EFAULT;
    goto end;
  }
  ...
  if (ctrl->cpphw->hwinfo.version != CPP_HW_VERSION_6_0_0 &&
    ...
    CPP_DBG("%s:%d: CPP CDS not supported for version 0x%x",
  ...
  cpp_module_get_params_for_identity(ctrl, identity,
    &session_params, &stream_params);
  if(!session_params || !stream_params) {
    CPP_ERR("failed, iden 0x%x, session_param %p, stream param %p\n",
      identity, session_params, stream_params);
    rc = -EFAULT;
    goto end;
  }
    ...
    for (i = 0; i < CPP_MODULE_MAX_STREAMS; i++) {
      if (session_params->stream_params[i]) {
        ...
        case CAM_STREAM_TYPE_OFFLINE_PROC:
          ...
          break;
        default:
          CPP_DBG("CPP CDS not supported for stream %d",
            session_params->stream_params[i]->stream_type);
          break;
        }
      }
  ...
  uint32_t identity, uint64_t value)
{
  ...
  if (stream_params->stream_type == CAM_STREAM_TYPE_OFFLINE_PROC) {
    ...
    stream_params->hw_params.hyst_pbf_status = ((value & DYN_IMG_PBF_HYS) >>
      DYN_IMG_PBF_HYS_BIT);
    CPP_TNR_HIGH("offline: value %llu, dsdn_status %d, tnr status %d pbf status %d",
    ...
    for (i = 0; i < CPP_MODULE_MAX_STREAMS; i++) {
      if (session_params->stream_params[i]) {
        ...
        case CAM_STREAM_TYPE_OFFLINE_PROC:
          ...
          break;
        default:
          CPP_LOW("HYSTERSIS INFO not supported for stream %d",
            session_params->stream_params[i]->stream_type);
          break;
        }
      }
