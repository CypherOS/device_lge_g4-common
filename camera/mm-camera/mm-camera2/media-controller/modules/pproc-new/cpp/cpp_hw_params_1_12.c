    ...
    struct cpp_frame_info_t* cpp_frame_info, uint32_t* len)
{
  ...
  *len = msg_len;
  ...
  for (i = 0; i < 3; i++) {
    ...
    for (j = 0; j < 4; j++) {
      k = base + 44 + i * 80 + j;
      frame_msg[k] = p_rnr->bilat_info[j].weight1_rnr_disable & 0x1;
    }
  ...
  for (i = 0; i < cpp_frame_info->num_planes; i++) {
    ...
    /* TODO: PP_m_CENTER_PIXEL_COORDINATES */
  ...
  for (j = 0; j < cpp_frame_info->num_planes; j++) {
    ...
    for (k = 0; k < cpp_frame_info->plane_info[j].num_stripes; k++, idx += 63) {
      ...
      frame_msg[idx + 60] =
        (asf_info->cpp_1_12_ext.asf_1_12_rnr_output.rnr_start_offset_v & 0xffff) << 16 |
        (asf_info->cpp_1_12_ext.asf_1_12_rnr_output.rnr_start_offset_h & 0xffff);
