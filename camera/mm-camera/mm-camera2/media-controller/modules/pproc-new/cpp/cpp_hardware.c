  ...
  cpp_hardware_event_data_t *event_data)
{
  ...
  CDBG("%s:%d, identity %x, frame id %d",
    __func__, __LINE__, event_data->identity,
    event_data->frame_id);
  ...
  cpp_hardware_clock_settings_t *clock_settings)
{
  ...
  CPP_INFO("Set clock %ld BW abg %lld BW inst %lld",
...
int32_t cpp_hardware_get_capabilities(cpp_hardware_t *cpphw)
{
  ...
  property_get("persist.camera.llnoise", cpp_prop, "1");
  if (atoi(cpp_prop))
  ...
  cpp_hardware_buff_update_t *buff_update)
{
  ...
  /* Translate to msm stream buff list */
   ...
   if (NULL == cpp_strm_buff_info.buffer_info) {
     CPP_ERR("error allocating buffer info\n");
     return -ENOMEM;
   }
  ...
  cpp_hardware_streamoff_event_t streamoff_data)
{
  ...
  while ((stream_status->pending_buf != 0) ||
    ...
    CPP_DBG("waiting for pending buf, identity=0x%x, pending_buf=%d, "
  ...
  for (i = 0; i < streamoff_data.num_linked_streams; i++) {
    ...
    if (duplicate_stream_status) {
      ...
      while ((duplicate_stream_status->pending_buf != 0) ||
        ...
        CPP_DBG("waiting for pending buf, identity=0x%x,"
  ...
  cpp_hardware_event_data_t *event_data)
{
  ...
  CDBG("%s: %d:frame_id=%d, buf_idx=%d, identity=0x%x",
...
int32_t cpp_hardware_notify_buf_done(cpp_hardware_t *cpphw, uint32_t identity)
{
  ...
  if (stream_status) {
    ...
    if ((stream_status->stream_off_pending == TRUE) &&
      ...
      (stream_status->pending_divert == 0) &&
      (stream_status->pending_partial_frame == 0)) {
      CPP_DBG("info: pp_dbg sending broadcast for pending stream-off");
      pthread_cond_broadcast(&(cpphw->subdev_cond));
    }
  } else {
  ...
  cpp_hardware_params_t *hw_params)
{
  ...
  switch(hw_params->stream_type) {
    case CAM_STREAM_TYPE_PREVIEW:
      ...
      if ((temp != 0) && (cpphw->dump_preview_cnt == 0)) {
        ...
        CPP_DBG("dumping preview %d", cpphw->dump_preview_cnt);
      }
      ...
      case CAM_STREAM_TYPE_VIDEO:
        ...
        if ((temp != 0) && (cpphw->dump_video_cnt == 0)) {
          ...
          CPP_DBG("dumping video %d", cpphw->dump_video_cnt);
        }
    ...
    case CAM_STREAM_TYPE_OFFLINE_PROC:
      ...
      if (temp != 0) {
        ...
        CPP_DBG("dumping snapshot %d", cpphw->dump_snapshot_cnt);
      }
      count = cpphw->dump_snapshot_cnt;
      break;
      default:
        CPP_DBG("dumping not supported for stream type %d", hw_params->stream_type);
        break;
  }
  ...
  if (msm_cpp_frame_info->frame_id < count ) {
    ...
    CPP_DBG("dumping payload for stream type %d frame_id %d",
  ...
  cpp_hardware_cmd_t *cmd)
{
  ...
  CPP_DBG("identity 0x%x buf_divert %d duplicate 0x%x dup_buf_divert %d",
  ...
  if (rc < 0) {
    CPP_DBG("v4l2 ioctl() failed. rc:%d, trans_code:%d, stream_id=%d\n",
    ...
    if (*msm_cpp_frame_info->status == -EAGAIN && hw_params->retry_disable == 0) {
      do {
        ...
        if (*msm_cpp_frame_info->status == -EAGAIN) {
          CPP_DBG("v4l2 ioctl() failed. retry=%d\n", retry);
          retry = retry + 1;
        } else {
          break;
        }
  ...
  CPP_DBG("frame scheduled for processing, frame_id=%d, "
  ...
  struct msm_cpp_frame_info_t *partial_frame)
{
  ...
  CPP_DBG("identity=0x%x", partial_frame->identity);
