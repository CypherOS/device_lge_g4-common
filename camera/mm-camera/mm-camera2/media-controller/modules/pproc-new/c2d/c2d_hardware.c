  ...
  uint32_t identity)
{
  ...
  while (stream_status->pending_buf != 0) {
    C2D_HIGH("waiting for pending buf, identity=0x%x, pending_buf=%d",
      identity, stream_status->pending_buf);
  ...
  C2D_HIGH("pending buffers done, hw streaming off. identity=0x%x\n",
    identity);
