  ...
  mct_event_t* event)
{
  ...
  case MCT_EVENT_MODULE_EVENT: {
    ...
    case MCT_EVENT_MODULE_IFACE_REQUEST_PP_DIVERT: {
      C2D_LOW("MCT_EVENT_MODULE_IFACE_REQUEST_PP_DIVERT: iden=0x%x", identity);
            ...
            if ((stream_info->is_type == IS_TYPE_EIS_2_0  ||
              ...
              if (linked_stream_params) {
                if ((linked_stream_params->hw_params.output_info.width >
                  ...
                  (linked_stream_params->hw_params.output_info.plane_info.mp[0].stride >
                  stream_info->buf_planes.plane_info.mp[0].stride) ||
                  (linked_stream_params->hw_params.output_info.plane_info.mp[0].scanline >
                  ...
                  hw_params->output_info.height =
                    linked_stream_params->hw_params.output_info.height;
                  hw_params->output_info.plane_info =
                    linked_stream_params->hw_params.output_info.plane_info;

                  /* Reuse the native buffer queue and streaming identity */
                  stream_params->streaming_identity =
                    linked_stream_params->streaming_identity;
                } else {
                  ...
                  c2d_module_init_native_buffers(ctrl,
                    &hw_params->output_info.plane_info, identity,
                  ...
                  linked_stream_params->hw_params.output_info.plane_info =
                    hw_params->output_info.plane_info;
                  /* Update streaming identity & attach it to linked stream */
                  stream_params->streaming_identity = identity;
                  linked_stream_params->streaming_identity = identity;
                }
              } else {
