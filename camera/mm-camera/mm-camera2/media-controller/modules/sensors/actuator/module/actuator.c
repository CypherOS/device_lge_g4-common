...
static int32_t af_actuator_move_focus(void *ptr, void *data)
{
 ...
 /*Find the number of bits to be shifted to print the +ve and -ve DAC values*/
  bit_shift = (sizeof(af_actuator_ptr->curr_lens_pos) * 8) -
    af_driver_ptr->actuator_params.data_size;
  SLOW("bit_shift:%d, curr_step_pos:%d, curr_len_pos_dac:%d",bit_shift,
