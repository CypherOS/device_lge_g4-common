  ...
  void __attribute__((unused))*peer_caps, void *info)
{
  ...
  if (bundle_info.session_id != sensor_src_port_cap->session_id) {
    SERR("session id doesn't match: %d : %d",
      bundle_info.session_id,sensor_src_port_cap->session_id);
    return FALSE;
  }
  ...
  if((stream_info->dis_enable) && (stream_info->stream_type == CAM_STREAM_TYPE_VIDEO)) {
    if (bundle_info.s_bundle->max_width <
        (uint32_t)((stream_info->dim.width * 11) / 10))
      bundle_info.s_bundle->max_width =
        (uint32_t)((stream_info->dim.width * 11) / 10);
    if (bundle_info.s_bundle->max_height <
        (uint32_t)((stream_info->dim.height * 11) / 10))
  ...
  uint32_t identity)
{
  ...
  if (bundle_info.s_bundle->init_config_done == 0) {
    memset(&ts, 0, sizeof(ts));
    clock_gettime(CLOCK_REALTIME, &ts);
    ts.tv_nsec  += INIT_CONFIG_TIMEOUT;
    pthread_cond_timedwait(&bundle_info.s_bundle->cond,
      &bundle_info.s_bundle->mutex, &ts);
  }
  ...
  mct_port_t *port, mct_event_t *event, sensor_bundle_info_t *bundle_info)
{
  ...
  case MCT_EVENTS_MODULE_PDAF_ISP_INFO:{
    ...
    s_pdaf.pdaf_gain_ptr = bundle_info->s_bundle->formatted_data->pdaf_ptr;
