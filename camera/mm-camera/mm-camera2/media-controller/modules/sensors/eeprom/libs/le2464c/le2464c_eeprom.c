...
void le2464c_format_afdata(sensor_eeprom_data_t *e_ctrl) {
  ...
  ALOGE("OTP:AF raw infi 0x%X, macro 0x%X",af_infi,af_macro);
  ALOGE("OTP:near margin %.2f, FAR_MARGIN %.2f",NEAR_MARGIN,FAR_MARGIN);
  ALOGE("OTP: VCM TOP: raw:0x%X, 12bit:%d, BOTTOM: raw:0x%X, 12bit:%d",
    af_top, af_top>>4, af_bottom, af_bottom>>4);

  ALOGE("OTP:AF Macro DAC 12bit= %d", (int16_t)e_ctrl->eeprom_data.afc.macro_dac);
  ALOGE("OTP:AF Infinity DAC 12bit= %d", (int16_t)e_ctrl->eeprom_data.afc.infinity_dac);
  ALOGE("OTP:AF offset: 0x%x, bias: 0x%x, size: 0x%x, top: 0x%x, bottom: 0x%x",
