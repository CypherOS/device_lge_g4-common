  ...
  sensor_submodule_event_type_t event, void *data)
{
  ...
  case LED_FLASH_GET_CURRENT: {
    ...
    rc = flash_get_current(awb_update, dual_led_setting);
    goto flash_process_Exit;
  }
  case LED_FLASH_INIT:
  ...
  case LED_FLASH_GET_RER_CHROMATIX: {
    ...
    rc = flash_rer_get_chromatix(rer, rer_chromatix);
    goto flash_process_Exit;
  }
  ...
  case LED_FLASH_SET_TORCH: {
    ...
    /* Use default settings for torch mode*/
...
int32_t flash_get_current(
  awb_update_t *awb_update,
  awb_dual_led_settings_t *dual_led_setting)
{

  RETURN_ERR_ON_NULL(awb_update, SENSOR_FAILURE);
  RETURN_ERR_ON_NULL(dual_led_setting, SENSOR_FAILURE);
  ...
  */
  if (dual_led_setting->led1_low_setting == 0 &&
    dual_led_setting->led2_low_setting == 0) {
    dual_led_setting->led1_low_setting = -1;
    dual_led_setting->led2_low_setting = -1;
  }

  if (dual_led_setting->led1_high_setting == 0 &&
    dual_led_setting->led2_high_setting == 0) {
    dual_led_setting->led1_high_setting = -1;
    dual_led_setting->led2_high_setting = -1;
  }

  return SENSOR_SUCCESS;
}

/** flash_rer_get_chromatix:
...
int32_t flash_rer_get_chromatix(
  rer_cfg_t *rer,
  ...
  module_sensor_params_t *led_module_params)
{
  ...
  if (flash_enable == 1) {
    ...
    struct timeval ts_start = {0}, ts_on = {0}, ts_off = {0}, ts_diff = {0};
    boolean  cnt = FALSE;
