...
static int32_t sensor_get_cur_fps(void *sctrl, void *fps_data)
{
  ...
  if (ctrl->s_data->current_linecount > frame_length_lines)
    cur_fps = (cur_fps * (float)frame_length_lines) /
...
static int32_t sensor_set_frame_duration(void *sctrl, void *data)
{
  ...
  if ((cur_res >= 0) && (cur_res < out_info_array->size)) {
    new_frame_length_lines =
      (uint16_t)(((float)out_info_array->out_info[cur_res].vt_pixel_clk /
    ...
    ctrl->s_data->current_fps_div =
      (float)new_frame_length_lines / ctrl->s_data->cur_frame_length_lines;
...
static int32_t sensor_set_aec_update_for_dual(void *sctrl, void *data)
{
  ...
  linecount = (1.0 * exp_time * line_tune *
...
static int32_t sensor_get_cur_chromatix_name(void *sctrl, void *data)
{
  ...
  for (i = 0; i < SENSOR_CHROMATIX_MAX; i++)
    if (name[i]) {
      SHIGH("res_idx = %d chromatix_lib_name[%d] = %s",cur_res, i, name[i]);
    } else {
      SHIGH("res_idx = %d chromatix_lib_name[%d] = %s",cur_res, i, "NULL");
    }
  ...
  if (lib->sensor_lib_ptr->sensor_output.output_format == SENSOR_BAYER) {
    ...
    for (i = 0; i < size; i++) {
      min = (uint64_t)((((float)out_info[i].line_length_pclk *
        NANO_SEC_PER_SEC) / (float)out_info[i].vt_pixel_clk) + 0.5);
      min_round_down = (uint64_t)(((float)out_info[i].line_length_pclk *
        NANO_SEC_PER_SEC) / (float)out_info[i].vt_pixel_clk);
...
static int32_t sensor_get_resolution_info(void *sctrl, void *data)
{
  ...
  sensor_out_info->sensor_hdr_enable = ctrl->s_data->hdr_mode;
       ...
       case CSI_DPCM8: {
           ...
           switch (pix_fmt_fourcc) {
            ...
            case MSM_V4L2_PIX_FMT_SRGGB14:
              ...
              break;
            case V4L2_PIX_FMT_NV12:
              sensor_out_info->fmt = CAM_FORMAT_YUV_420_NV12;
              break;
            case V4L2_PIX_FMT_NV16:
              sensor_out_info->fmt = CAM_FORMAT_YUV_422_NV16;
              break;
            case V4L2_PIX_FMT_NV61:
              sensor_out_info->fmt = CAM_FORMAT_YUV_422_NV61;
              break;
            case V4L2_MBUS_FMT_YUYV8_2X8:
              sensor_out_info->fmt =
                CAM_FORMAT_YUV_RAW_8BIT_YUYV;
...
static int32_t sensor_pdaf_cal_defocus(void *sctrl, void *data)
{
  ...
  if(sensor_custom_API->sensor_custom_calc_defocus) {
    ...
    if(lib->sensor_custom_API.pdlib_get_defocus)
      ...
      get_defocus = &pdaf_get_defocus_with_pd_data;
