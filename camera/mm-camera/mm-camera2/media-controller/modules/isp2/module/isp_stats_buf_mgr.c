  ...
  iface_resource_request_t *resource_request)
{
  ...
  if (ret == TRUE) {
    ...
    for (i = 0; i < resource_request->num_stats_stream; i++) {
      ...
      stats_type = isp_stats_info->stats_type;
      ...
      if (buf_mgr->stats_buf_info[stats_type]) {
        ISP_ERR("error: stats_type %d already initialized", stats_type);
        break;
      }
      buf_mgr->stats_buf_info[stats_type] =
      ...
      if (!buf_mgr->stats_buf_info[stats_type]) {
        ...
        stats_buf_info = buf_mgr->stats_buf_info[stats_type];
        for (j = 0; j < MAX_STATS_BUFFERS; j++) {
          stats_buf_info[j].addr =
            malloc(s_parsed_stats_size[stats_type]);
          if (!stats_buf_info[j].addr) {
            ISP_ERR("failed: no memory");
            ret = FALSE;
            break;
          }
          stats_buf_info[j].buf_size = s_parsed_stats_size[stats_type];
