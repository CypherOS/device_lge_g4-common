...
boolean isp_pipeline47_parser_fill_stats_info(uint32_t session_id,
  iface_resource_request_t *iface_resource_request, uint32_t stats_mask,
  boolean fast_aec_mode, uint32_t num_isp)
{
  isp_stats_info_t *stats_info = NULL;

  if (!iface_resource_request) {
    ISP_ERR("failed: iface_resource_request %p", iface_resource_request);
    return FALSE;
  }
  ...
  isp_parser_session_params_t *parser_session_params)
{
  ...
  for (j = 0; j < v_rgns_total; j++) {
    for (i = h_rgns_start; i <= h_rgns_end; i++) {
      rs_stats->row_sum[i][j] = ((rs_stats->row_sum[i][j] >> shiftBits) +
