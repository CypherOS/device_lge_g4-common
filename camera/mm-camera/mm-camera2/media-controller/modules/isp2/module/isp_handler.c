  ...
  isp_session_param_t *session_param)
{
  ...
  mct_stats_info.stats_mask = session_param->parser_params.stats_mask;
  ...
  mct_port_t *port, mct_event_t *event)
{
  ...
  if (session_param->reg_update_info.reg_update_state ==
    ...
    ISP_ERR(" Warning! Invalid reg_update state %d",
      session_param->reg_update_info.reg_update_state);
  }
  ...
  mct_port_t *port, mct_event_t *event)
{
  ...
  for (hw_id = 0; hw_id < ISP_HW_MAX; hw_id++) {
    if (isp_cfg_request->isp_id_mask & (1 << hw_id) &&
      hw_id == session_param->offline_hw_id[0]) {
      ISP_ERR("fetch_Dbg fetch config VFE%d START/REPROCESS/STOP %d (0/1/2)",
          hw_id, isp_cfg_request->fetch_cfg_cmd);
      if (isp_cfg_request->fetch_cfg_cmd == ISP_FETCH_PIPELINE_START) {
        ...
        ret = isp_handler_create_internal_link_for_stream(module,
            session_param, event->identity, hw_id);
        if (ret == FALSE) {
        ...
        isp_resource_destroy_internal_link_hw_id(port,
           (isp_port_data_t *)MCT_OBJECT_PRIVATE(port), event->identity, hw_id);
        session_param->hw_update_params.hw_update_list_params[hw_id].fd = 0;
      } else {
    ...
    if (fetch_engine_cfg_request->isp_id_mask & (1 << hw_id)  &&
      ...
      if (fetch_eng_cfg_data.is_hw_update_list_overwrite == TRUE) {
        ...
        ret = isp_hw_update_util_enqueue(hw_update_params,
          fetch_eng_cfg_data.offline_hw_update_list, hw_id);
        if (ret == FALSE) {
          ISP_ERR("failed: failed overwrite offline hw list hw_id %d", hw_id);
        }
  ...
  mct_port_t *port, mct_event_t *event)
{
  ...
  isp_t               *isp = NULL;
  isp_session_param_t *session_param = NULL;
  sensor_out_info_t   *sensor_output = NULL;
  ...
  session_param->sensor_fmt = sensor_output->fmt;
    ...
    cam_dimension_t     *dim_update_info)
{
  ...
  dim_update_info->width =
      ...
      (uint16_t)(dim_update_info->width * DIS_MARGIN_10));
  dim_update_info->height =
      CEILING32((dim_update_info->height) +
      (uint16_t)(dim_update_info->height * DIS_MARGIN_10));
...
void isp_handler_destroy_stream_param(mct_port_t *port, unsigned int identity)
{
  ...
  if (!l_stream_params) {
    ...
    /* Reset stats mask */
    session_param->parser_params.stats_mask = 0;
