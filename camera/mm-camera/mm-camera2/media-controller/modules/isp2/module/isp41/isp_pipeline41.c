...
static boolean isp_pipeline41_udpate_module_cfg(int32_t fd,
...
static isp_pipeline_stats_func_table_t func_table = {
  ...
  .pipeline_udpate_module_cfg = isp_pipeline41_udpate_module_cfg,
  ...
  uint32_t *register_mask, uint32_t *register_val)
{
  ...
  uint32_t                     register_bit[ISP_PIPELINE41_MOD_EN_MAX] = {~0};
...
/** isp_pipeline41_udpate_module_cfg:
 *
 *  @fd: file descriptor
...
isp_pipeline_t *isp_pipeline41_open(uint32_t isp_version, uint32_t isp_id)
{
  ...
  if (isp_version == ISP_MSM8952) {
    ...
    curr_pipeline->max_nominal_pix_clk = ISP_PIX41_CLK_USER_MAX;
