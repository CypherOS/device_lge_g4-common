...
static boolean isp_pipeline40_udpate_module_cfg(int32_t fd,
...
static isp_pipeline_stats_func_table_t func_table = {
  ...
  .pipeline_udpate_module_cfg = isp_pipeline40_udpate_module_cfg,
  ...
  uint32_t *register_mask, uint32_t *register_val)
{
  ...
  uint32_t                     register_bit[ISP_PIPELINE40_MOD_EN_MAX] = {~0};
...
/** isp_pipeline40_udpate_module_cfg:
 *
 *  @fd: file descriptor
  ...
  uint8_t *curr_submod_enable, isp_module_enable_info_t *enable_bit_info)
{
  ...
  if ((fd <= 0) || !enable_bit_info) {
    ISP_ERR("failed: fd %d enable_bit_info %p", fd, enable_bit_info);
    return FALSE;
  }
...
isp_pipeline_t *isp_pipeline40_open(uint32_t isp_version, uint32_t isp_id)
{
  ...
  if (isp_version >= ISP_MSM8226_V1) {
    ...
    curr_pipeline->max_nominal_pix_clk = ISP_PIX40_CLK_USER_MAX;
