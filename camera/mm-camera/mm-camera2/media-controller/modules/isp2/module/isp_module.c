  ...
  unsigned int sessionid)
{
  ...
  if (!isp->isp_resource.isp_resource_info[ISP_HW_0].isp_pipeline) {
    ISP_ERR("failed: isp_pipeline %p",
      isp->isp_resource.isp_resource_info[ISP_HW_0].isp_pipeline);
    return FALSE;
  }
  ...
  if (isp->isp_resource.isp_resource_info
    [ISP_HW_0].isp_pipeline->is_camif_crop_supported) {
  ...
  isp_cap->max_viewfinder_size.width = isp->isp_resource.isp_resource_info
    [ISP_HW_0].isp_pipeline->hw_stream_info[ISP_HW_STREAM_VIEWFINDER].max_width;
  isp_cap->max_viewfinder_size.height = isp->isp_resource.isp_resource_info
    [ISP_HW_0].isp_pipeline->hw_stream_info[ISP_HW_STREAM_VIEWFINDER].max_height;;
    ...
    for (i = 0; i < ISP_MAX_SESSION; i++) {
      ...
      if ((isp_resource->session_info[i].is_valid == TRUE) &&
        ...
        if (isp_resource->isp_resource_info[ISP_HW_0].isp_pipeline) {
          ...
          max_turbo_clk = ISP_PIX_CLK_600MHZ;
        }

        /* If this is pip case, reserve single vfe */
        if (dual_vfe_forced ||
          (isp_resource->session_info[i].max_op_pixel_clk > max_turbo_clk)) {
          /* If max op pixel clk for this session >= turbo, reserve 2 vfe */
  ...
  for (hw_id = 0; hw_id < ISP_HW_MAX; hw_id++) {
    ...
    if (hw_update_params->metadata_dump_enable) {
      hw_update_list_params->meta_reg_dump = (uint32_t*)malloc(
         (isp->isp_resource.isp_resource_info[hw_id].
    ...
    session_param->hw_id[hw_id] = ISP_HW_MAX;
