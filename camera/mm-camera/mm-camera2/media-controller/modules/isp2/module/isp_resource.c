  ...
  isp_hw_id_t hw_id, int32_t fd, isp_module_enable_info_t *enable_bit_info)
{
  ...
  if (isp_pipeline->func_table &&
    isp_pipeline->func_table->pipeline_udpate_module_cfg) {
    ret = isp_pipeline->func_table->pipeline_udpate_module_cfg(fd,
      isp_pipeline->submod_enable, enable_bit_info);
    if (ret == FALSE) {
      ISP_ERR("failed: pipeline_udpate_module_cfg");
      goto ERROR;
    }
  } else {
    ISP_ERR("func_table %p or pipeline_udpate_module_cfg NULL",
  ...
  if (dual_vfe_enabled == 1 && !session_param->stream_port_map.raw_stream_exists) {
    ...
    for (i = 0; i < isp_resource->num_isp; i++) {
      ...
      if ((res_alloc->state == ISP_RESOURCE_FREE) &&
          ...
          ((res_info->isp_pipeline->max_svs_pix_clk >= pix_clk)||
          ((res_info->isp_pipeline->max_svs_pix_clk < (pix_clk / 2)) &&
...
static boolean isp_resource_get_stats_type_mask(mct_module_t *module,
  isp_t *isp, isp_session_param_t *session_param, uint32_t *stats_type_mask,
  boolean is_offline)
{
  boolean                       ret = TRUE;
  uint32_t                      i = 0;
  ...
  if (!module || !isp || !session_param || !stats_type_mask) {
    ISP_ERR("failed: %p %p %p %p", module, isp, session_param, stats_type_mask);
    return FALSE;
  }
  ...
  *stats_type_mask = stats_type.enable_stats_mask;
  if (is_offline) {
    ISP_ERR("off_dbg stats mask %x", *stats_type_mask);
  }
   ...
   */
  if (!is_offline) {
    *stats_type_mask |= (1 << MSM_ISP_STATS_BHIST);
    ISP_ERR("enable_stats_mask %x", stats_type.enable_stats_mask);
  }
  return TRUE;
}
  ...
  mct_port_t *port, mct_event_t *event)
{
  ...
  ret = isp_resource_get_stats_type_mask(module, isp, session_param,
    &session_param->parser_params.stats_mask, TRUE);
  if (ret == FALSE) {
  ...
  for (num_isp = 0; num_isp < session_param->offline_num_isp; num_isp++) {
    ...
    if (func_table->pipeline_fill_stats_info) {
      ret = func_table->pipeline_fill_stats_info(session_param->session_id,
        resource_request,
        session_param->parser_params.stats_mask,
  ...
  mct_port_t *port, mct_event_t *event)
{
  ...
  /* Get stats mask */
  ret = isp_resource_get_stats_type_mask(module, isp, session_param,
    &session_param->parser_params.stats_mask, FALSE);
  if (ret == FALSE) {
    ISP_ERR("failed: isp_util_get_stats_type_mask");
  }
  ...
  uint32_t isp_id)
{
  ...
  isp_resource_info->isp_pipeline->max_svs_pix_clk = clk_rate.svs_rate;
