  ...
  isp_hw_update_params_t *hw_update_params)
{
  ...
  ret = isp_hw_update_util_free_all(hw_update_params);
  if (ret == FALSE) {
    ISP_ERR("failed: isp_hw_update_list_free_all");
  }
...
void isp_hw_update_thread_join(isp_session_param_t *session_param)
{
  ...
  ret = isp_hw_update_thread_free_hw_udpate_list(hw_update_params);
  if (ret == FALSE) {
    ISP_ERR("failed: isp_hw_update_free_hw_udpate_list");
  }
