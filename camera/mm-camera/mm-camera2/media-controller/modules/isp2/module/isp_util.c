  ...
  isp_hw_id_t hw_id, uint32_t identity)
{
  ...
  stats_update.awb_update = fetch_eng_cfg_data->awb_update;
  ...
  stats_update.aec_update = fetch_eng_cfg_data->aec_update;
  ...
  cam_stream_type_t    stream_type __unused)
{
  ...
  for (hw_id = 0; hw_id < ISP_HW_MAX; hw_id++) {
    ...
    isp_resource_update_module_cfg(isp_resource, hw_id,
    session_param->hw_update_params.hw_update_list_params[hw_id].fd,
    &(enable_bit_info.cfg_info));
  }
   ...
   void *user_data)
{
  ...
  for (hw_id = 0; hw_id < session_param->num_isp; hw_id++) {
    ret = isp_util_forward_event_to_internal_pipeline(session_param, event,
      session_param->hw_id[hw_id]);
    if (ret == FALSE) {
      ISP_ERR("failed: ret %d", ret);
    }
  }
   ...
   for (i = 0; i < session_param->num_isp; i++) {
     for (j = 0; j < ISP_METADUMP_MAX_NUM; j++) {
       if (hw_update_params->hw_update_list_params[session_param->hw_id[i]].
           applied_meta_dump_parms.meta_entry[j].isp_meta_dump == NULL ||
         ...
         memset(hw_update_params->hw_update_list_params
           [session_param->hw_id[i]].applied_meta_dump_parms.meta_entry,
  ...
  isp_session_param_t *session_param)
{
  ...
  for (i = 0; i < session_param->num_isp; i++) {
    ...
    for (j = 0; j < ISP_METADUMP_MAX_NUM; j++) {
      ...
      if (dmi_info->hw_dmi_tbl == NULL || dmi_info->read_length == 0) {
        continue;
      } else {
  ...
  isp_session_param_t *session_param)
{
  ...
  applied_meta_dump_parms->meta_entry[ISP_META_REGISTER_DUMP].dump_type
    = ISP_META_REGISTER_DUMP;
  applied_meta_dump_parms->meta_entry[ISP_META_REGISTER_DUMP].len
    = num_reg * sizeof(uint32_t);
  applied_meta_dump_parms->meta_entry[ISP_META_REGISTER_DUMP].start_addr
    = 0;

  if (session_param->max_reporting_delay == 0) {
    ret = isp_util_read_reg_dump(hw_update_params->
      hw_update_list_params[session_param->hw_id[0]].fd,
      applied_meta_dump_parms->meta_entry[ISP_META_REGISTER_DUMP].isp_meta_dump,
      VFE_READ, num_reg * sizeof(uint32_t), 0);
  } else if (session_param->max_reporting_delay == 1) {
    memcpy(applied_meta_dump_parms->meta_entry[ISP_META_REGISTER_DUMP].isp_meta_dump,
      hw_update_params->hw_update_list_params[session_param->hw_id[0]].meta_reg_dump,
      num_reg * sizeof(uint32_t));
    ret = isp_util_read_reg_dump(hw_update_params->
      hw_update_list_params[session_param->hw_id[0]].fd,
      hw_update_params->hw_update_list_params[session_param->hw_id[0]].meta_reg_dump,
...
boolean isp_util_trigger_internal_hw_update_event(
 ...
 isp_hw_id_t *hw_ids, uint32_t num_isp)
{
  boolean                      ret = TRUE;
  mct_event_t                  event;
  isp_hw_id_t                  hw_id = 0;
...
enum msm_vfe_axi_stream_src isp_util_get_axi_src_type(cam_format_t fmt,
  uint8_t stream_id)
{
  ISP_HIGH("<camif_raw> fmt %d stream id %d", fmt, stream_id);
  switch (fmt) {
  ...
  mct_bus_msg_isp_sof_t *sof_event, isp_saved_stats_params_t *stats_params)
{
  ...
  for (hw_id = 0; hw_id < ISP_HW_MAX; hw_id++) {
    if (session_param->hw_id[hw_id] != ISP_HW_MAX) {
      break;
    }
  }
  if (hw_id == ISP_HW_MAX) {
  ...
  used_stats_params = &stats_params[session_param->hw_id[hw_id]];
  ...
  isp_session_param_t *session_param, cam_stream_size_info_t *streams_desc)
{
  ...
  if (streams_desc->num_streams) {
    if (session_param->num_isp) {
      ISP_ERR("failed: ISP resource is already allocated");
      return FALSE;
    }
    ...
    if (session_param->l_stream_params &&
      ...
      ISP_ERR("failed: Cannot free ISP resource. %d streams are unlinked.",
        ...
        if (session_param->l_stream_params->next_num == 1) {
          ...
          if ((ret == FALSE) || !stream_param) {
            ISP_ERR("failed: 1 pending unlink and its not offline %d %p", ret,
              stream_param);
            return FALSE;
          } else {
