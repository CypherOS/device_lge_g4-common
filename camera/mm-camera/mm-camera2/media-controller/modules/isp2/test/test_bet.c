...
chromatix_VFE_common_type chromatix_common_data;
#if defined(CHROMATIX_VERSION) && (CHROMATIX_VERSION >= 0x306)
chromatix_3a_parms_type chromatix_3a_data;
#endif
  ...
  mct_port_t *sink_port)
{
  ...
  sensor_out_info->fmt = CAM_FORMAT_BAYER_MIPI_RAW_10BPP_BGGR;
  ...
  vfe_test_vector_t *vfe_test_vector)
{
  ...
  aec->aec_update.exp_ratio      = vfe_test_vector->exp_ratio;
  aec->aec_update.exp_time_ratio = vfe_test_vector->exp_time_ratio;
  ...
  testcase_report_t* testcase_report)
{
  ...
  memcpy(&chromatix_data, chromatix_ptr, sizeof(chromatix_data));
  memcpy(&chromatix_common_data, chromatix_common_ptr, sizeof(chromatix_common_data));

  module_chromatix.chromatixPtr = &chromatix_data;
  module_chromatix.chromatixComPtr = &chromatix_common_data;
  CDBG_ERROR("bet_dbg init");
  memset(&vfe_test_vector, 0, sizeof(vfe_test_vector_t));
  vfe_test_vector.chromatix_data = &chromatix_data;
  ...
  memcpy(&chromatix_3a_data, chromatix_3a_ptr, sizeof(chromatix_3a_parms_type));
  module_chromatix.chromatix3APtr = &chromatix_3a_data;
#endif
  ...
  CDBG_ERROR("bet_dbg Raw stats event frame id = %d", raw_stats.frame_id);
  event.identity = identity;
  event.direction = MCT_EVENT_DOWNSTREAM;
  event.type = MCT_EVENT_MODULE_EVENT;
