...
#define MODULE_LENS_CGC_OVERRIDE 0x2C
#define MODULE_STATS_CGC_OVERRIDE 0x30
#define MODULE_COLOR_CGC_OVERRIDE 0x34
#define MODULE_ZOOM_CGC_OVERRIDE 0x38

typedef struct {
  char vfe_module_name[ISP_MOD_MAX_NUM][100];
  uint32_t show_mismatch[ISP_MOD_MAX_NUM];
  uint32_t show_match[ISP_MOD_MAX_NUM];
  int (*vfe_tv_validate_func[ISP_MOD_MAX_NUM]) (void* test_input, void* test_output);
} isp_bet_test_list_t;
...
#define VALIDATE_TST_VEC(ip, op, range, str)({\
  if (abs(ip - op) > range) {\
      ...
      if (show_match) \
        CDBG_TV("\tBET: %s: MATCH!, %s, diff: %d, ip: %08x, op : %08x\n",\
        __func__, str, abs(ip - op), ip, op);\
  }\
})
...
#define VALIDATE_TST_VEC2(ip, op, range, str1, str2)({\
    if (abs(ip - op) > range) {\
      ...
      if (show_match) \
        CDBG_TV("\tBET: %s %s: MATCH!, %s, diff: %d, ip: %08x, op : %08x\n",\
        __func__, str1, str2, abs(ip - op), ip, op);\
    }\
  })
...
#define VALIDATE_TST_LUT(ip, op, range, str, index)({\
    if ((ip - op) > range) {\
      ...
      if (show_match) \
        CDBG_TV("\tBET: %s: Match, %s, Index :%d, diff: %d, ip: %08x, op : %08x\n",\
         __func__, str, index, (ip - op), ip, op);\
    }\
})

#define VALIDATE_TST_LUT64(ip, op, range, str, index)({\
    if ((ip - op) > range) {\
      has_mismatch = 1; \
      if (show_mismatch) \
        CDBG_TV("\tBET: %s: Mismatch, %s, Index :%d, diff: %llu, ip: %08x, op : %08x\n",\
         __func__, str, index, ip - op, (uint32_t)ip, (uint32_t)op);\
    } else{ \
      if (show_match) \
        CDBG_TV("\tBET: %s: Match, %s, Index :%d, diff: %llu, ip: %08x, op : %08x\n",\
         __func__, str, index, ip - op, (uint32_t)ip, (uint32_t)op);\
    }\
})
