...
#include "../module/isp47/isp_pipeline_reg.h"
...
#include "../hw/gic/gic46/gic46.h"
#include "gic_reg.h"
#include "../hw/abf/abf47/abf47.h"
...
extern int vfe_bls_tv_validate(void *input, void *output);
...
static isp_bet_test_list_t bet_test_list =  {
  ...
  .vfe_tv_validate_func = {
   ...
   .show_mismatch = {
     ...
     [ISP_MOD_SNR]                 = 0,
     [ISP_MOD_BLS]                 = 0,
     [ISP_MOD_COLOR_XFORM_VIEWFINDER] = 1,
     [ISP_MOD_LTM]                 = 0,
   },
  .show_match = {
     [ISP_MOD_GAMMA]               = 0,
  ...
  const char* attribute, const char* value)
{
  ...
  char input_last_entry[] = "bihist_gr_bin";
  ...
  if (!strcmp(AWB_R_GAIN, attribute)) {
    ...
    int data = atoi(value);
    CDBG("%s: ADRC_ENABLE %d", __func__, data);
    ...
    float data = atof(value);
    CDBG("%s: ADRC_TOTAL_GAIN %f", __func__, data);
    ...
    mod->hdr_mode = strcmp(NON_HDR_MODE, value) ? 1 : 0;
    CDBG("bet_dbg HDR_OR NON_HDR_MODE\n");
  ...
  switch(module_type) {
    ...
    case ISP_MOD_ABF: {
      if (!test_vector_strcmp("table", test_vector_str_to_lower(p_val[0]))) {
        ...
        } else if (!test_vector_strcmp("Mesh 2D", p_val[1])){
          status = test_vector_extract_table_int(
            mod, p_val, count, &mod->mod_input.abf_mesh_2D, "ABF Mesh 2D");
        }
      } else {
    ...
    case ISP_MOD_GIC: {
      if (!test_vector_strcmp("table", test_vector_str_to_lower(p_val[0]))) {
        if (!test_vector_strcmp("noise_std2_lut_level0", p_val[1])) {
          ...
          status = test_vector_extract_table_int(
            mod, p_val, count, &mod->mod_input.gic_sig2_l0, "GIC sig2 L0");
        }
      } else {
    ...
    case ISP_MOD_PEDESTAL: {
      if (!test_vector_strcmp("table", test_vector_str_to_lower(p_val[0]))) {
        ...
        } else if (!test_vector_strcmp("DMIRAM_PEDESTAL_T2_L_GR_R_LUT", p_val[1])) {
          status = test_vector_extract_table_int(
            mod, p_val, count, &mod->mod_input.pedestal_t2_gr_r, "pedestal mesh table T2 GR_R");
        } else if (!test_vector_strcmp("DMIRAM_PEDESTAL_T2_L_GB_B_LUT", p_val[1])) {
          status = test_vector_extract_table_int(
            mod, p_val, count, &mod->mod_input.pedestal_t2_gb_b, "pedestal mesh table T2 GB_B");
        }
      } else {
...
vfe_status_t vfe_test_module_data_init(vfe_test_module_data_t *mod_data)
{
  ...
  mod_data->reg_size = ISP47_NUM_REG_DUMP * sizeof(uint32_t);
  ...
  test_vector_malloc_table(
     &mod_data->abf_mesh_2D, sizeof(abf_mesh_lut_t)/ sizeof(uint32_t));
  test_vector_malloc_table(
     &mod_data->gic_sig2_l0, GIC_SIGNAL_SQR_LUT_SIZE);
  test_vector_malloc_table(
  ...
  test_vector_malloc_table(
     &mod_data->pedestal_t2_gr_r, PEDESTAL_LUT_SIZE);
  test_vector_malloc_table(
     &mod_data->pedestal_t2_gb_b, PEDESTAL_LUT_SIZE);
  test_vector_malloc_table_64(
...
void vfe_test_module_data_deinit(vfe_test_module_data_t *mod_data)
{
  ...
  _FREE(mod_data->abf_mesh_2D.table);
  ...
  _FREE(mod_data->pedestal_t2_gr_r.table);
  _FREE(mod_data->pedestal_t2_gb_b.table);
...
#define FETCH_TABLE_DIRECT(tbl, bank, type, sz) \
  ...
  FETCH_TABLE(tv_params->abf_mesh_2D,
    ABF_MESH_LUT_BANK0, ISP_ABF3_OFF, 2, VFE_READ_DMI_32BIT, sizeof(uint32_t));

  /*GIC*/
  FETCH_TABLE(tv_params->gic_std2_l0,
    GIC_NOISE_STD2_LUT_BANK0, ISP_GIC_OFF, 0, VFE_READ_DMI_32BIT, sizeof(uint32_t));
  FETCH_TABLE(tv_params->gic_sig2_l0,
    GIC_SIGNAL_SQR_LUT_BANK0, ISP_GIC_OFF, 0, VFE_READ_DMI_32BIT, sizeof(uint32_t));
  ...
  dmi_bank_sel = PEDESTAL_F0_RAM_L_GR_R_BANK0 +
    8 * ((tv_params->reg_dump[ISP_PEDESTAL_OFF/4] >> 8) & 1);
  ...
  /*Tbl1 GR_R*/
  dmi_bank_sel = PEDESTAL_F1_RAM_L_GR_R_BANK0 +
    8 * ((tv_params->reg_dump[ISP_PEDESTAL_OFF/4] >> 8) & 1);
  FETCH_TABLE_DIRECT(tv_params->pedestal_t2_gr_r, dmi_bank_sel,
    VFE_READ_DMI_32BIT, sizeof(uint32_t));

  /*Tbl1 GB_B*/
  dmi_bank_sel = PEDESTAL_F1_RAM_L_GB_B_BANK0 +
    8 * ((tv_params->reg_dump[ISP_PEDESTAL_OFF/4] >> 8) & 1);
  FETCH_TABLE_DIRECT(tv_params->pedestal_t2_gb_b,
    dmi_bank_sel, VFE_READ_DMI_32BIT, sizeof(uint32_t));
