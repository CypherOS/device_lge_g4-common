...
#include "../module/isp41/isp_pipeline_reg.h"
#include "gamma40.h" // for ISP_GAMMA_NUM_ENTRIES
#include "gamma40_reg.h" // for ISP_GAMMA_NUM_ENTRIES
...
#define VFE_TEST_VEC_LTM                   "Local Tone Map"
...
vfe_status_t vfe_test_module_data_init(vfe_test_module_data_t *mod_data)
{
  ...
  mod_data->reg_size = ISP40_NUM_REG_DUMP * sizeof(uint32_t);
  ...
  test_vector_malloc_table(
     &mod_data->mesh_rolloff.gr_r, ISP_MESH_ROLLOFF40_TABLE_SIZE); //130
  test_vector_malloc_table(
    &mod_data->mesh_rolloff.gb_b, ISP_MESH_ROLLOFF40_TABLE_SIZE);
  test_vector_malloc_table(
...
#define FETCH_TABLE_DIRECT(tbl, bank, type, sz) \
  ...
  FETCH_TABLE_ROLLOFF(tv_params->mesh_rolloff.gr_r,
    LENS_ROLLOFF_RAM_BANK0, ISP_MESH_ROLLOFF_CFG_OFF, 16, VFE_READ_DMI_32BIT, sizeof(uint32_t));
  FETCH_TABLE_ROLLOFF(tv_params->mesh_rolloff.gb_b,
    LENS_ROLLOFF_RAM_BANK1, ISP_MESH_ROLLOFF_CFG_OFF, 16, VFE_READ_DMI_32BIT, sizeof(uint32_t));
