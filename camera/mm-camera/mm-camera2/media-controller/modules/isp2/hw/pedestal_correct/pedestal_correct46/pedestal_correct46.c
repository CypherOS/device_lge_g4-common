  ...
  pedestal46_mod_t *pedestal_mod, isp_sub_module_output_t *output)
{
  ...
  *pedestal_cfg = pedestal_mod->pedestal_cfg;
  ...
  ret = isp_sub_module_util_write_dmi(
    ...
    PEDESTAL_F0_RAM_L_GR_R_BANK0 + pedestal_cfg->lutBankSel * 8,
  ...
  ret = isp_sub_module_util_write_dmi(
    ...
    PEDESTAL_F0_RAM_L_GB_B_BANK0 + pedestal_cfg->lutBankSel * 8,
    VFE_WRITE_DMI_32BIT, ISP_DMI_CFG_OFF, ISP_DMI_ADDR, isp_sub_module);
  if (ret == FALSE) {
  ...
  ret = isp_sub_module_util_write_dmi(
    ...
    PEDESTAL_F1_RAM_L_GR_R_BANK0 + pedestal_cfg->lutBankSel * 8,
    VFE_WRITE_DMI_32BIT, ISP_DMI_CFG_OFF, ISP_DMI_ADDR, isp_sub_module);
  if (ret == FALSE) {
  ...
  ret = isp_sub_module_util_write_dmi(
    ...
    PEDESTAL_F1_RAM_L_GB_B_BANK0 + pedestal_cfg->lutBankSel * 8,
    VFE_WRITE_DMI_32BIT, ISP_DMI_CFG_OFF, ISP_DMI_ADDR, isp_sub_module);
  if (ret == FALSE) {
 ...
 *  Return void
 **/
static void pedestal46_mesh_sensor_calc_table_debug(float *meshtbl)
{
  int  i, j;
  ...
  for (i = 0; i < HW_PED_NUM_COL; i++) {
    for(j = 0; j < HW_PED_NUM_ROW; j++) {
      snprintf(p_str, 8, "%1.2f ", meshtbl[(i*HW_PED_NUM_ROW)+j]);
      p_str += 5;
    }
    ISP_DBG("");
    line_str[0] = '\0';
  ...
  int Ny)
{
  ...
  for (i=1; i<Ny+1; i++)
    for (j=1; j<Nx+1; j++)
      MeshOut[i*(Nx+2)+j] = (float)MeshIn[(i-1)*Nx+j-1];
 ...
 *  Return void
 **/
static void pedestal46_scale_table(
  ...
  chromatix_pedestalcorrection_type *outTbl, sensor_rolloff_config_t* sensor_info)
{
  ...
  pedestal46_mesh_sensor_calc_table_debug(&inTbl->pctable[i].channel_black_level_r);
#endif
  ...
  pedestal46_mesh_sensor_calc_table_debug(&outTbl->pctable[i].channel_black_level_r);

  ISP_DBG("===== input B table======");
  pedestal46_mesh_sensor_calc_table_debug(&inTbl->pctable[i].channel_black_level_b);
#endif
  ...
  pedestal46_mesh_sensor_calc_table_debug(&outTbl->pctable[i].channel_black_level_b);

  ISP_DBG("===== input GB table======");
  pedestal46_mesh_sensor_calc_table_debug(&inTbl->pctable[i].channel_black_level_gb);
#endif
  ...
  pedestal46_mesh_sensor_calc_table_debug(&outTbl->pctable[i].channel_black_level_gb);

  ISP_DBG("===== input GR table======");
  pedestal46_mesh_sensor_calc_table_debug(&inTbl->pctable[i].channel_black_level_gr);
#endif
  ...
  pedestal46_mesh_sensor_calc_table_debug(&outTbl->pctable[i].channel_black_level_gr);
#endif
  }
}
...
static void pedestal46_prepare_hw_tables(pedestal46_mod_t *mod)
{
  uint32_t i = 0;
  uint32_t k = 0;
  uint16_t blk_increase = (uint16_t)(mod->aec_ratio *
    mod->max_blk_increase);

  for (i = 0; i < 2; i++) {
    for (k = 0; k < PEDESTAL_LUT_SIZE; k++) {
      uint32_t gr = mod->chromatix_tbls_output.pctable[i].channel_black_level_gr[k] - blk_increase;
      uint32_t r  = mod->chromatix_tbls_output.pctable[i].channel_black_level_r [k] - blk_increase;
      uint32_t gb = mod->chromatix_tbls_output.pctable[i].channel_black_level_gb[k] - blk_increase;
      uint32_t b  = mod->chromatix_tbls_output.pctable[i].channel_black_level_b [k] - blk_increase;
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  mod->chromatix_tbls.pctable[0] =
    chromatix_common->chromatix_pedestal_correction.pctable[0];
  mod->chromatix_tbls.pctable[1] =
    chromatix_common->chromatix_pedestal_correction.pctable[1];

#if defined(CHROMATIX_VERSION) && (CHROMATIX_VERSION == 0x304)
  /* Note: chromatix_black_level is removed for 0305 */
  mod->chromatix_black_lvl =
   &chromatix_parms->chromatix_VFE.chromatix_black_level;
  mod->max_blk_increase = chromatix_parms->chromatix_VFE.chromatix_black_level.max_blk_increase;
#else
  mod->max_blk_increase = 0;
#endif
...
FILL_METADATA:
  if (output->metadata_dump_enable == 1) {
    ...
    ISP_DBG("<new_dmi_debug> pedestal read length %d", pedestal_dmi_info->len);
    /*dmi type*/
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  if (!mod->chromatix_black_lvl) {
    ISP_DBG("%s failed: chomratix_ptr %p, 306 header no blck lv data",
      __func__, mod->chromatix_black_lvl);
    isp_sub_module->trigger_update_pending = TRUE;
    return TRUE;
  }
  ...
  if (mod->chromatix_black_lvl->control_blk) { // gain controlled
    float gain       = stats_update->aec_update.real_gain;
    float gain_start = mod->chromatix_black_lvl->blk_lowlight_trigger.gain_start;
    float gain_end   = mod->chromatix_black_lvl->blk_lowlight_trigger.gain_end;

    if (gain >= gain_start)
      ratio = (gain - gain_start) / (gain_end - gain_start);
  } else { // lux index controlled
    float lux_index = stats_update->aec_update.lux_idx;
    float lux_start = mod->chromatix_black_lvl->blk_lowlight_trigger.lux_index_start;
    float lux_end   = mod->chromatix_black_lvl->blk_lowlight_trigger.lux_index_end;

    if (lux_index >= lux_start)
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  if (isp_sub_module->is_zzhdr_mode){
    ISP_DBG("disabling pedestal for zzhdr");
