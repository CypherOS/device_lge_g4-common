  ...
  fovcrop40_t *fovcrop, mct_event_t *event)
{
  ...
  if (fovcrop->sensor_out_info.fmt >= CAM_FORMAT_BAYER_QCOM_RAW_8BPP_GBRG &&
      fovcrop->sensor_out_info.fmt <=
      CAM_FORMAT_BAYER_IDEAL_RAW_PLAIN16_12BPP_BGGR) {
    fovcrop->is_bayer_sensor = TRUE;
  } else
    fovcrop->is_bayer_sensor = FALSE;
 ...
 *  Return TRUE
 **/
static boolean port_fovcrop40_handle_cds_request(mct_module_t *module,
  fovcrop40_t *fovcrop, mct_event_t *event)
{
  boolean             ret = TRUE;
 ...
 *  Return TRUE
 **/
static boolean port_fovcrop40_handle_hal_set_parm(mct_module_t *module,
  ...
  mct_event_t *event)
{
  ...
  case MCT_EVENT_MODULE_ISP_PRIVATE_EVENT: {
    ...
    case ISP_PRIVATE_REQUEST_STREAM_SPLIT_INFO: {
      ...
      /*get latest hw stream and config*/
      ret = port_fovcrop40_config(module, fovcrop, event->identity);
      if (ret == FALSE) {
        ISP_ERR("failed: port_fovcrop40_get_fovcrop_info");
      }
    ...
    default:
      ret = fovcrop40_handle_isp_private_event(fovcrop,
