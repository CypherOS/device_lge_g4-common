  ...
  cam_format_t fmt)
{
  ...
  case CAM_FORMAT_BAYER_MIPI_RAW_14BPP_BGGR:
    return ISP_BAYER_BGBGBG;
  default:
   ISP_ERR("%s: invalid fmt=%d", __func__, fmt);
 ...
 *  Returns pix pattern associated with input bayer packing
 **/
boolean isp_sub_module_isp_bayer_fmt(cam_format_t fmt)
{
  ISP_DBG("%s: format %d", __func__, fmt);
  switch (fmt) {
  case CAM_FORMAT_BAYER_MIPI_RAW_10BPP_BGGR:
  case CAM_FORMAT_BAYER_MIPI_RAW_10BPP_GBRG:
  case CAM_FORMAT_BAYER_MIPI_RAW_10BPP_GRBG:
 ...
 ISP_DBG("%s: non bayer fmt=%d", __func__, fmt);
  return FALSE;
}
  ...
  void *trigger_ptr, aec_update_t* aec_out)
{
  ...
  case 1: {
    ...
    ratio_to_low_start = isp_sub_module_util_calc_interpolation_weight(
      real_gain, trigger->gain_start, trigger->gain_end);
  }
    break;
  ...
  trigger_ratio_t *rt)
{
  ...
  ISP_DBG("lux_idx %f, current_real_gain %f",
  ...
  case 1: {
    ...
    if (real_gain < outdoor_trigger->gain_start) {
      rt->ratio = isp_sub_module_util_calc_interpolation_weight(real_gain,
        outdoor_trigger->gain_end, outdoor_trigger->gain_start);
      rt->lighting = TRIGGER_OUTDOOR;
    } else if (real_gain > lowlight_trigger->gain_start) {
      rt->ratio = 1.0 - isp_sub_module_util_calc_interpolation_weight(real_gain,
 ...
 *
 **/
boolean isp_sub_module_util_is_adrc_gain_set(float drc_gain)
{
  return drc_gain >= 1.0 ? TRUE : FALSE;
}
