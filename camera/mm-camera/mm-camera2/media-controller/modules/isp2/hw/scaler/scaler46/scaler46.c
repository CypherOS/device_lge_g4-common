  ...
  isp_stripe_request_t *stripe_request_info)
{
  ...
  left_stripe = split_point + limits->min_overlap_half;
  right_stripe = sensor_output_width - limits->smallest_split_point +
    limits->min_overlap_half;
  ...
  if (left_stripe > limits->max_left_split) {
    ...
    split_point += left_stripe - limits->max_right_split;
    split_point += rolloff_min_dist_from_grid;
  }
  ...
  stripe_offset = limits->smallest_split_point - limits->min_overlap_half;
  ...
  ISP_DBG("<stripe_dbg> sensor_output_width %d, mid_point %d, split_point %d,"\
    "stripe_offset %d", sensor_output_width, mid_point,split_point,
    stripe_offset);
...
void scaler46_calculate_dis(scaler46_t *scaler)
{
  ...
  if (scaler->dis_enable) {
    scaler->hw_stream_info.width =
      ...
      (uint16_t)(scaler->hw_stream_info.width_before_dis * DIS_MARGIN_10));
    scaler->hw_stream_info.height =
      CEILING32((scaler->hw_stream_info.height_before_dis) +
      (uint16_t)(scaler->hw_stream_info.height_before_dis * DIS_MARGIN_10));
...
boolean scaler46_config(scaler46_t *scaler)
{
  ...
  scaler46_calculate_zoom_scaling(scaler, y_cfg, scaler->crop_factor);
...
boolean scaler46_parm_zoom(mct_module_t *module,
  scaler46_t *scaler, int32_t *zoom_val, uint32_t identity)
{
  boolean ret = TRUE;
  boolean is_stream_on = FALSE;
 ...
 * Return: 0 always
 **/
boolean scaler46_init(void *mod_ctrl)
{
  scaler46_t *scaler = mod_ctrl;
 ...
 *  Return TRUE on success and FALSE on failure
 **/
boolean scaler46_handle_isp_private_event(scaler46_t *scaler,
  void *data, uint32_t identity)
{
  ...
  case ISP_PRIVATE_FETCH_CROP_FACTOR:
    ret = scaler46_get_crop_factor(scaler, (uint32_t *)private_event->data);
    if (ret == FALSE) {
...
{
  scaler46_t *scaler = malloc(sizeof(scaler46_t));
