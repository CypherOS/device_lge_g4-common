  ...
  mct_event_t *event)
{
  ...
  if ((scaler->sensor_out_info.fmt >= CAM_FORMAT_BAYER_QCOM_RAW_8BPP_GBRG &&
      scaler->sensor_out_info.fmt <=
      CAM_FORMAT_BAYER_IDEAL_RAW_PLAIN16_12BPP_BGGR) ||
      (scaler->sensor_out_info.fmt >= CAM_FORMAT_BAYER_QCOM_RAW_14BPP_GBRG &&
       scaler->sensor_out_info.fmt <= CAM_FORMAT_BAYER_IDEAL_RAW_PLAIN16_14BPP_BGGR)) {
    scaler->is_bayer_sensor = TRUE;
    scaler->camif_dim.width = sensor_out_info->request_crop.last_pixel -
      sensor_out_info->request_crop.first_pixel + 1;
  } else {
    scaler->is_bayer_sensor = FALSE;
    scaler->camif_dim.width = (sensor_out_info->request_crop.last_pixel -
 ...
 *  Return TRUE
 **/
static boolean port_scaler46_handle_cds_request(scaler46_t *scaler,
  mct_event_t *event)
{
  boolean ret = TRUE;
 ...
 *  Return TRUE
 **/
static boolean port_scaler46_handle_hal_set_parm(mct_module_t* module,
  ...
  mct_event_t *event)
{
  ...
  case MCT_EVENT_MODULE_ISP_PRIVATE_EVENT:
    ret = scaler46_handle_isp_private_event(scaler,
