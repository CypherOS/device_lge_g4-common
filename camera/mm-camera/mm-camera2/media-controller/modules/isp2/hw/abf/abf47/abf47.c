...
#define FILL_LUT_INTERP(dst, src1, src2, len, ratio, adj) \
  { \
    ...
    int32_t delta; \
    for (i = 0; i < len - 1; i++) { \
      base = roundf(((src1)[i] * adj) * (ratio) + ((src2)[i] * adj) * (1 - ratio)); \
      next = roundf(((src1)[i+1] * adj) * (ratio) + ((src2)[i+1] * adj) * (1 - ratio)); \
      base = CLAMP_ABF(base,65535);\
  ...
  abf_adjustment_t          abf_adj)
{
  ...
  /*curve offset*/
  tmp2 = (unsigned short)(LINEAR_INTERPOLATION(
    abf3_data_start->curve_offset[0][idx],
    abf3_data_end->curve_offset[0][idx], ratio));
  chCfg->curvOffLv0 = tmp2;

  tmp2 = (unsigned short)(LINEAR_INTERPOLATION(
    abf3_data_start->curve_offset[1][idx],
    abf3_data_end->curve_offset[1][idx], ratio));
  ...
  abf47_t *abf)
{
  ...
  float   ratio     = abf->aec_ratio.ratio;
  float   ratio_adj = abf->aec_ratio.ratio_adj;
  ...
  if (abf->trigger_index + 1 >= MAX_LIGHT_TYPES_FOR_SPATIAL){
    ...
    FILL_LUT(abf->abf3_parms.noise_std2_lut_lv0,
      abf3_data_start->noise_std2_lut_level0, 128);
    FILL_LUT(abf->abf3_parms.noise_std2_lut_lv1,
      abf3_data_start->noise_std2_lut_level1, 128);
  } else {
  ...
  abf47_t *abf)
{
  ...
  if (abf->trigger_index + 1 > MAX_LIGHT_TYPES_FOR_SPATIAL) {
    ISP_ERR("trigger index = %u + 1 >= MAX, no interpolation! ratio %f",
      abf->trigger_index, abf->aec_ratio.ratio);
  } else {
      abf3_data_end   = &(chromatix_ABF3->abf3_config[abf->trigger_index + 1]);
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  if (isp_sub_module->submod_enable == FALSE) {
    /* do not config HW, but report meta if any manula control mode*/
    goto FILL_METADATA;
  }
  ...
  for (i = 0; i < MAX_SETS_FOR_TONE_NOISE_ADJ; i++) {
    ...
    /* index is within interpolation range, find ratio */
     if (aec_reference_adj >= start && aec_reference_adj < end) {
       ratio_adj         = (end - aec_reference_adj)/(end - start);
