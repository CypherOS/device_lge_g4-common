  ...
  bf_scale_stats46_t *bf_scale_stats)
{
  ...
  copy_cmd = (ISP_StatsBf_Scale_CfgCmdType *)malloc(sizeof(*copy_cmd));
  if (!copy_cmd) {
  ...
  isp_sub_module_t *isp_sub_module, void *data)
{
  ...
  if ((stripe_limit->min_overlap_half * 2) < bf_grid_width) {
    stripe_limit->min_overlap_half = (bf_grid_width + 1) / 2;
  }
...
boolean bf_scale_stats46_init(isp_sub_module_t *isp_sub_module)
{
  ...
  bf_scale_stats = (bf_scale_stats46_t *)malloc(sizeof(bf_scale_stats46_t));
  if (!bf_scale_stats) {
    ISP_ERR("failed: bf_scale_stats %p", bf_scale_stats);
    PTHREAD_MUTEX_UNLOCK(&isp_sub_module->mutex);
    return FALSE;
