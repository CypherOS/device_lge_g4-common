  ...
  void *data)
{
  ...
  if (isp_sub_module->trigger_update_pending == TRUE) {
    ...
    ret = isp_sub_module_util_reset_hist_dmi(isp_sub_module, STATS_BHIST_RAM0,
    ...
    ret = isp_sub_module_util_reset_hist_dmi(isp_sub_module, STATS_BHIST_RAM1,
      ISP_DMI_CFG_DEFAULT, ISP_DMI_CFG_OFF, ISP_DMI_ADDR);
    if (ret == FALSE) {
      ISP_ERR("failed: isp_sub_module_util_reset_hist_dmi RAM1");
      goto ERROR;
    }
