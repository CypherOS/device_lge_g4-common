  ...
  aec_bg_stats47_t *aec_bg_stats)
{
  ...
  *copy_cmd = *pcmd;
  ...
  reg_cfg_cmd[0].u.mask_info.reg_offset = STATS_CFG_OFF_AEC_BG;
  reg_cfg_cmd[0].u.mask_info.mask       = STATS_CFG_MASK_AEC_BG;
  reg_cfg_cmd[0].u.mask_info.val        = STATS_CFG_VAL_AEC_BG;
  reg_cfg_cmd[0].cmd_type               = VFE_CFG_MASK;

  reg_cfg_cmd[1].u.rw_info.cmd_data_offset = 0;
  reg_cfg_cmd[1].cmd_type = VFE_WRITE;
  reg_cfg_cmd[1].u.rw_info.reg_offset = AEC_BG_STATS_OFF;
  reg_cfg_cmd[1].u.rw_info.len = AEC_BG_STATS_LEN * sizeof(uint32_t);
 ...
 *  Return TRUE on success and FALSE on failure
 **/
static boolean aec_bg_stats47_fill_stats_parser_params(aec_bg_stats47_t *aec_bg_stats,
  isp_saved_stats_params_t *stats_params)
{
  ...
  if (!aec_bg_stats || !stats_params) {
    ISP_ERR("failed: %p %p", aec_bg_stats, stats_params);
    return FALSE;
  }
  ...
  void *data)
{
  ...
  if ((isp_sub_module->submod_enable == FALSE) ||
    ...
    if (sub_module_output->stats_params) {
      ret = aec_bg_stats47_fill_stats_parser_params(aec_bg_stats,
  ...
  if ((aec_bg_config->grid_info.h_num == 0) || (aec_bg_config->grid_info.v_num == 0)) {
    ISP_DBG("Invalid BG h_num & v_num from 3A, h_num = %d, v_num = %d\n",
      aec_bg_config->grid_info.h_num, aec_bg_config->grid_info.v_num);
    goto ERROR;
  }
  ...
  if ( aec_bg_config->roi.width / aec_bg_config->grid_info.h_num <
    ...
    aec_bg_config->grid_info.h_num = aec_bg_config->roi.width /bg_rgn_width;
  } else {
     bg_rgn_width =
  ...
  if ( aec_bg_config->roi.height / aec_bg_config->grid_info.v_num <
    ...
    aec_bg_config->grid_info.v_num = aec_bg_config->roi.height /bg_rgn_height;
  } else {
     bg_rgn_height =
  ...
  void *data)
{
  ...
  aec_bg_config = &aec_bg_stats->aec_bg_config;
  aec_bg_config->grid_info.h_num = 64;
  aec_bg_config->grid_info.v_num = 48;
  ...
  aec_bg_config->r_Max = (255 - 16) << ISP_STATS_BITS_DIFF;
  aec_bg_config->gr_Max = (255 - 16) << ISP_STATS_BITS_DIFF;
  aec_bg_config->b_Max = (255 - 16) << ISP_STATS_BITS_DIFF;
  aec_bg_config->gb_Max = (255 - 16) << ISP_STATS_BITS_DIFF;
  ...
  pcmd->rgnHNum    = 63;
  pcmd->rgnVNum    = 47;
  pcmd->rMax       = (255 - 16) << ISP_STATS_BITS_DIFF;
  pcmd->grMax      = (255 - 16) << ISP_STATS_BITS_DIFF;
  pcmd->bMax       = (255 - 16) << ISP_STATS_BITS_DIFF;
  pcmd->gbMax      = (255 - 16) << ISP_STATS_BITS_DIFF;
  pcmd->rMin       = 0;
  pcmd->grMin      = 0;
  pcmd->bMin       = 0;
  pcmd->gbMin      = 0;
