  ...
  void *data)
{
  ...
  if (stripe_limit->min_overlap_half * 2 < bf_stats_min_overlap)
    stripe_limit->min_overlap_half = (MIN_X_OFFSET + H_FIR_MARGIN) / 2;
 ...
 *  Adjust sign bit
 **/
static int32_t bf_stats47_adjust_signbit(int32_t val, uint8_t num_bits)
{
  if (val < 0) {
    val = -(val);
    val &= ((1 << num_bits) - 1);
  ...
  bf_stats47_t *bf_stats)
{
  ...
  for (i = 0; i < bf_fw_roi_cfg->num_bf_fw_roi_dim; i++) {
    ...
    packed_roi_dmi_cfg[i] =
      (((uint64_t)bf_fw_roi_cfg->bf_fw_roi_dim[i].region) <<
      DMI_I_SHIFT) |
      ((((uint64_t)bf_fw_roi_cfg->bf_fw_roi_dim[i].x) & DMI_X_BITS) <<
      DMI_X_SHIFT) |
      ((((uint64_t)bf_fw_roi_cfg->bf_fw_roi_dim[i].y) & DMI_Y_BITS) <<
      DMI_Y_SHIFT) |
      ((((uint64_t)bf_fw_roi_cfg->bf_fw_roi_dim[i].w) & DMI_W_BITS) <<
      DMI_W_SHIFT) |
      ((((uint64_t)bf_fw_roi_cfg->bf_fw_roi_dim[i].h) & DMI_H_BITS)) <<
      DMI_H_SHIFT;
  }
   ...
   */
  packed_roi_dmi_cfg[i] =
    (((uint64_t)bf_fw_roi_cfg->bf_fw_roi_dim[i-1].region) <<
    DMI_I_SHIFT) |
    ((((uint64_t)bf_fw_roi_cfg->bf_fw_roi_dim[i-1].x) & DMI_X_BITS) <<
    DMI_X_SHIFT) |
    ((((uint64_t)bf_fw_roi_cfg->bf_fw_roi_dim[i-1].y) & DMI_Y_BITS) <<
    DMI_Y_SHIFT) |
    ((((uint64_t)bf_fw_roi_cfg->bf_fw_roi_dim[i-1].w) & DMI_W_BITS) <<
    DMI_W_SHIFT) |
    ((((uint64_t)bf_fw_roi_cfg->bf_fw_roi_dim[i-1].h) & DMI_H_BITS)) <<
    DMI_H_SHIFT;
