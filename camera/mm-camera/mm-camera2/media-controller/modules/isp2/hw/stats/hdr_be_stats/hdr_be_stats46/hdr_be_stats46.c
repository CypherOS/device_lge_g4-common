...
#define HI_THRESHOLD_NUM_BITS_IN_CFG 8
#define HI_THRESHOLD_NUM_BITS_IN_HW  14
#define THRESH_BITS_DIFF   (HI_THRESHOLD_NUM_BITS_IN_HW - \
                            HI_THRESHOLD_NUM_BITS_IN_CFG)
  ...
  void *data)
{
  ...
  if ((be_config->grid_info.h_num != 0) && (be_config->grid_info.v_num != 0)) {
    ...
    pcmd->rMax       = be_config->r_Max << THRESH_BITS_DIFF;
    pcmd->grMax      = be_config->gr_Max << THRESH_BITS_DIFF;
    pcmd->bMax       = be_config->b_Max << THRESH_BITS_DIFF;
    pcmd->gbMax      = be_config->gb_Max << THRESH_BITS_DIFF;
  ...
  /* Default configuration */
    ...
    pcmd->rMax       = ((1 << ISP_PIPELINE_WIDTH) - 1) - PIX_VALUE_OFFSET;
    pcmd->grMax      = ((1 << ISP_PIPELINE_WIDTH) - 1) - PIX_VALUE_OFFSET;
    pcmd->bMax       = ((1 << ISP_PIPELINE_WIDTH) - 1) - PIX_VALUE_OFFSET;
    pcmd->gbMax      = ((1 << ISP_PIPELINE_WIDTH) - 1) - PIX_VALUE_OFFSET;
    ...
    be_config->r_Max = 255;
    be_config->gr_Max = 255;
    be_config->b_Max = 255;
    be_config->gb_Max = 255;
  }
  pcmd->rMin       = 0;
  pcmd->grMin      = 0;
  pcmd->bMin       = 0;
  ...
  void *data)
{
  ...
  pcmd->rgnHNum    = ISP_STATS_HDR_BE_GRID_H - 1;
  pcmd->rgnVNum    = ISP_STATS_HDR_BE_GRID_V - 1;
  pcmd->rMax       = ((1<<ISP_PIPELINE_WIDTH)-1) - 1024;
  pcmd->grMax      = ((1<<ISP_PIPELINE_WIDTH)-1) - 1024;
  pcmd->bMax       = ((1<<ISP_PIPELINE_WIDTH)-1) - 1024;
  pcmd->gbMax      = ((1<<ISP_PIPELINE_WIDTH)-1) - 1024;
  pcmd->rMin       = 0;
  pcmd->grMin      = 0;
  pcmd->bMin       = 0;
  pcmd->gbMin      = 0;
