 ...
 *  Return none
 **/
static void sce_copy_triangle_float(sce_cr_cb_triangle_set *int_triangle,
  ...
  sce_cr_cb_triangle_set_float* ip2, sce_cr_cb_triangle_set_float* op, float ratio)
{
  ...
  int i, size = sizeof(sce_cr_cb_triangle_set)/ sizeof(float);
  ...
  sce_cr_cb_triangle_set* ip2, sce_cr_cb_triangle_set* op, float ratio)
{
  ...
  for (i=0; i<size; i++) {
     ptrOp[i] = Round(
        LINEAR_INTERPOLATION((float)ptrIp1[i], (float)ptrIp2[i], ratio));
  }
} /* trigger_interpolate_sce_triangles */
  ...
  chromatix_SCE_type *p_sce, awb_cct_type cct_type)
{
  ...
  case AWB_CCT_TYPE_D65_TL84: {
    ...
    trigger_interpolate_sce_vectors(&p_sce->shift_vector_D65,
      &p_sce->shift_vector_TL84, &sce->interp_vector, awb_ratio);
  }
    break;
 ...
 *  Return TRUE on success and FALSE on failure
 **/
static boolean sce40_select_table( isp_sub_module_t *isp_sub_module,
  sce40_t *sce)
{
  ...
  if (!sce || !isp_sub_module) {
    ISP_ERR("failed: %p %p", sce, isp_sub_module);
    return FALSE;
  }
  ...
  if (trigger_info->mired_color_temp == 0) {
    ISP_ERR("%s: SCE zero color temperature", __func__);
    return FALSE;
  }

 aec_ratio = isp_sub_module_util_get_aec_ratio_lowlight(
    chromatix_SCE->control_SCE, &chromatix_SCE->SCE_trigger, &sce->aec_update);

  color_drc_gain = sce->aec_update.color_drc_gain;
  sce->enable_adrc = isp_sub_module_util_is_adrc_gain_set(color_drc_gain);
  ...
  if (chromatix_SCE->sce_enable &&
    ...
    ret = sce40_adjust_dest_triangle_by_aec(sce, aec_ratio);
    if (ret == FALSE) {
      ISP_ERR("failed, trigger_sce_get_triangles");
      return FALSE;
    }
  ...
  sce->prev_color_drc_gain = color_drc_gain;
  ...
  void *data)
{
  ...
  if (!isp_sub_module->chromatix_module_enable) {
     if( (sce->sce_adjust_factor <= 1.0) && (sce->sce_adjust_factor >= -1.0) ) {
        isp_sub_module->submod_enable = TRUE;
        isp_sub_module->update_module_bit= TRUE;
    }
  }
