...
typedef struct {
  ...
  double                    adrc_hw_master_curve[CURVE_ENTRY_NUM + 1];
  double                    adrc_hw_master_scale[CURVE_ENTRY_NUM + 1];
  double                    adrc_hw_shift_scale[CURVE_ENTRY_NUM + 1];
  double                    adrc_hw_shift_curve[CURVE_ENTRY_NUM + 1];
  double                    adrc_hw_mask_curve[CURVE_ENTRY_NUM + 1];
  int                       adrc_unpacked_master_scale[LTM_HW_LUT_SIZE + 1];
  int                       adrc_unpacked_master_curve[LTM_HW_LUT_SIZE + 1];
  int                       adrc_unpacked_hw_shift_scale[LTM_HW_LUT_SIZE + 1];
  int                       adrc_unpacked_hw_shift_curve[LTM_HW_LUT_SIZE + 1];
  int                       adrc_unpacked_hw_mask_curve[LTM_HW_LUT_SIZE + 1];
}ltm_algo_t;
