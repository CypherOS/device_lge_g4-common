  ...
  isp_sub_module_t *isp_sub_module, void *data)
{
  ...
  ISP_ERR("min_overlap_half before %d", stripe_limit->min_overlap_half);
  ...
  ISP_ERR("dual_dbg min_overlap_half after %d overlap_factor %f",
  ...
  void *data)
{
  ...
  ltm44_check_enable(isp_sub_module, mod);

  ISP_DBG("trigger index (start)  %u, ratio %f", mod->normal_idx.tr_index,
...
{
  int        i = 0;
  float      dark_str, highlight_str, new_final;
  int32_t    master_scale_val;
  ...
  if (mod->enable_adrc) {
    ...
    new_final = ((float)mod->nGlobalToneStrength/256.0f);
  }
  new_final *= mod->ratio ;

  ISP_DBG ("dark_str %f highlight_str %f", dark_str, highlight_str);
  ISP_DBG ("Global tone strength = %d",mod->nGlobalToneStrength);
  ISP_DBG ("final strength = %f",new_final);
  for (i = 0; i < LTM_HW_LUT_SIZE ; i++) {
   /* weaken master scale */
    if(i < mod->dark_boost_thr_index) {
      master_scale_val = roundf(ms_cur[i] *
        dark_str * new_final);

    }
    else if(i >= mod->highlight_sup_thr_index) {
      master_scale_val = roundf(ms_cur[i] *
       highlight_str * new_final);

    }
    else {
      master_scale_val = roundf(ms_cur[i] * new_final);
    }
    mod->unpacked_master_scale[i] = CLAMP_LTM(master_scale_val,
     -(1 << (LUT_PIX_BITS - 1)), (1 << (LUT_PIX_BITS - 1)) - 1);
  }
  return TRUE;
}

static boolean ltm44_attenuate_shift_scale(int *ss_cur, ltm44_t* mod)
{
  int        i = 0;
  float      new_final;
  int32_t    shift_scale_val;

  RETURN_IF_NULL(mod);
  RETURN_IF_NULL(ss_cur);
  ...
  for (i = 0; i < LTM_HW_LUT_SIZE ; i++) {
   /* weaken shift scale */
    shift_scale_val = roundf(ss_cur[i] * new_final);
    mod->unpacked_shift_scale[i] = CLAMP_LTM(shift_scale_val,
     -(1 << (LUT_PIX_BITS - 1)), (1 << (LUT_PIX_BITS - 1)) - 1);
  }
  return TRUE;
}
 ...
 float exp_based_ratio)
{
  ...
  ISP_DBG("exp_based_ratio %f mod->ratio %f", exp_based_ratio, mod->ratio);
  if (!F_EQUAL(exp_based_ratio, mod->ratio)) {
    ISP_DBG("change in exp_based_ratio %f ", exp_based_ratio);
  }

  mod->ratio = exp_based_ratio;

  ltm44_attenuate_master_scale(mod->unpacked_master_scale_attn, mod);
  ltm44_attenuate_shift_scale (mod->unpacked_shift_scale_attn, mod);
 ...
 *
 *    If adrc is enabled, update the master scale and curve
 *    based on adrc algo
 *  Return:   0 - Success
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  /* Round off master scale and master curve and convert into 64 entries */
  for (i = 0, j = 0; i < CHROMATIX_LUT_ENTRY; i += LUT_SUB_SAMPLE, j++) {
    mod->unpacked_master_scale[j] = Round(
        (float)mod->ltm_adrc_algo.adrc_hw_master_scale[i] / 2.0f);
    mod->ltm_adrc_algo.adrc_unpacked_master_curve[j] =
        Round((float)mod->ltm_adrc_algo.adrc_hw_master_curve[i] / 4.0f);
  }
