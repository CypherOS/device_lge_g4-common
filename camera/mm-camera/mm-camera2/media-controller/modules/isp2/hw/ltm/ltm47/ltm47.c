...
static boolean ltm47_check_enable(isp_sub_module_t *isp_sub_module, ltm47_t *mod)
{
  ...
  if ((mod->normal_idx.ltm_region_enable ||
    ...
    return TRUE;
  } else if ((!mod->normal_idx.ltm_region_enable &&
       !mod->backlit_idx.ltm_region_enable)) {
  ...
  return isp_sub_module->submod_enable;
}
  ...
  isp_sub_module_t *isp_sub_module, void *data)
{
  ...
  ISP_DBG("min_overlap_half before %d", stripe_limit->min_overlap_half);
  /* Using maximum 12x9 grids of each vfe in dual vfe LTM config */
  ltm_overlap = ((sensor_output_width + 18)/19);
  if (ltm_overlap != 0 && stripe_limit->min_overlap_half < ltm_overlap)
    stripe_limit->min_overlap_half = ltm_overlap;
  ISP_DBG("min_overlap_half after %d overlap_factor %f",
    stripe_limit->min_overlap_half, ltm->cell_overlap_factor);
...
static void pack_w_tbl_to_lut(uint16_t *src, uint32_t *dest){
 ...
 for (i=0; i<6; i++) {
    ...
    for( ; j >= 0; j--) {
      int32_t packed = 0;
      packed = ((uint32_t)src[block_start + j + 6] << 16)|
               ((uint32_t)src[block_start + j + 3] << 8) |
               ((uint32_t)src[block_start + j]);
      dest[i*3+(2-j)] = packed;
    }
  }
}
 ...
 *
 *  @chromatix_ltm: Ltm structure from chromatix
 *  @mod: ltm module data
 *  @trigger_idx: Normal, Bright, Low light
 *
 *  Interpolate saturation region over tr_index and tr_index +1
 *
 * Return TRUE on success and FALSE on failure
 **/
static boolean ltm47_interpolate_curve(chromatix_LTM_type* chromatix_ltm,
  int *dest, LTM_light_type trigger_idx, float ratio)
{
  int i = 0;
  RETURN_IF_NULL(chromatix_ltm);
  RETURN_IF_NULL(dest);

  if (trigger_idx + 1 >= LTM_MAX_LIGHT) {
    ISP_DBG("No interpolation, trigger index = %d", trigger_idx);
    return FALSE;
  }

  for (i = 0; i < LTM_HW_LUT_SIZE + 1; i++) {
    dest[i] = (int)Round((float)LINEAR_INTERPOLATION_LTM(
      (float)LTM_CORE_TYPE(chromatix_ltm, sat_curve[i], trigger_idx),
      (float)LTM_CORE_TYPE(chromatix_ltm, sat_curve[i], trigger_idx + 1),
      ratio));
  }
  return TRUE;
}
  ...
  void *data)
{
  ...
  ltm47_check_enable(isp_sub_module, mod);
  ...
  if (ASD_algo_data->backlit_scene_detect.backlit_LTM.enable) {
    ...
    ltm47_interpolate_curve(chromatix_ltm_backlit, mod->unpacked_sat_curve,
      mod->backlit_idx.tr_index, mod->backlit_idx.ratio);
    ...
    ltm47_interpolate_curve(chromatix_ltm_normal, mod->unpacked_sat_curve,
      mod->normal_idx.tr_index, mod->normal_idx.ratio);
    y_ratio_max = (float)LINEAR_INTERPOLATION_LTM(
      (float)LTM_CORE_TYPE(chromatix_ltm_normal,
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  if (mod->enable_adrc == TRUE) {
    ...
    if (!new_aec_update->sof_id ||
      cur_aec_update->sof_id != new_aec_update->sof_id) {
      *cur_aec_update = *new_aec_update;
      isp_sub_module->trigger_update_pending = TRUE;
  ...
  float exp_based_ratio)
{
  ...
  ISP_DBG("exp_based_ratio %f", exp_based_ratio);
  mod->ratio = exp_based_ratio;

  ltm47_attenuate_master_scale(master_scale, mod, exp_based_ratio);

  pack_tbl_to_lut_i32(master_scale, mod->hw_master_scale,
    1.0f);
  ...
  isp_sub_module_t *isp_sub_module)
{
  ...
  exp_based_ratio *= (float)mod->nGlobalToneStrength / 256.0f;
  ...
  if (!filter_off) {
    if (!mod->init_aec_settled) {
      exp_based_ratio =(float)(TEMPORAL_WEIGHT * exp_based_ratio +
  ...
  isp_sub_module_t *isp_sub_module, boolean disable)
{
  ...
  PTHREAD_MUTEX_LOCK(&isp_sub_module->mutex);
  if (chromatix_LTM->enable) {
    if ((disable) && (isp_sub_module->submod_enable)) {
 ...
 * Return TRUE on success and FALSE on failure
 **/
static boolean ltm47_setup_dark_bright_region_index(chromatix_LTM_type *ltm,
 ...
 */
static boolean load_default_tables(isp_sub_module_t *isp_sub_module,
  void *data)
{
  ASD_VFE_struct_type     *ASD_algo_data = NULL;
  ltm47_t                 *mod = NULL;
  ...
  if (chromatix_LTM_backlit) {
    if (chromatix_LTM_normal) {
      ...
      for (i = 0; i < LTM_HW_LUT_SIZE + 1; i++) {
        mod->unpacked_master_scale[i] = (int)Round((float)LINEAR_INTERPOLATION_LTM(
          (float)chromatix_LTM_normal->reservedData.master_scale[i],
          (float)chromatix_LTM_backlit->reservedData.master_scale[i],
          mod->bsd_ratio));
        mod->unpacked_shift_scale[i] = (int)Round((float)LINEAR_INTERPOLATION_LTM(
  ...
  memcpy(mod->unpacked_sat_curve, ltm->chromatix_ltm_core_data[idx].sat_curve,
        ...
        //pack_tbl_to_lut_i32(mod->adrc_unpacked_hw_shift_curve, mod->hw_shift_curve, 1.0f);
      //pack_tbl_to_lut_i32(mod->adrc_unpacked_hw_shift_scale, mod->hw_shift_scale, 1.0f);
  ...
  for (i = 0; i < LTM_HW_W_LUT_SIZE; i++) {
    if (mod->isp_out_info.is_split) {
      mod->hw_weight[i] = weight_default_1x1kernel[i];
    } else {
      mod->hw_weight[i] = weight_default_3x3kernel[i];
    }
  }
  ltm47_setup_dark_bright_region_index(ltm, mod, ltm->reservedData.master_scale);
  return TRUE;
}
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  default: {
    if (!isp_sub_module->chromatix_module_enable && ltm->effect_applied) {
      isp_sub_module_util_configure_from_chromatix_bit(isp_sub_module);
 ...
 *
 *   If adrc is enabled, update the master scale and curve based
 *   on adrc algo
 *
 * Return:   0 - Success
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  if (ret == FALSE) {
    ...
    for (i = 0; i < CURVE_ENTRY_NUM ; i++) {
      mod->ltm_adrc_algo.adrc_hw_master_curve[i] =
        (double)adrc_master_curve_default[i];
      mod->ltm_adrc_algo.adrc_hw_master_scale[i] =
        (double)adrc_master_scale_curve_default[i];
    }
  }
  /* Round off master scale and master curve */
  for (i = 0; i < CURVE_ENTRY_NUM + 1; i++) {
    mod->unpacked_master_scale[i] = Round(
        mod->ltm_adrc_algo.adrc_hw_master_scale[i]);
    mod->ltm_adrc_algo.adrc_unpacked_master_curve[i] =
        Round(mod->ltm_adrc_algo.adrc_hw_master_curve[i]);
  }
  ...
  ltm47_setup_dark_bright_region_index(ltm, mod, mod->unpacked_master_scale);
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  if (isp_sub_module->manual_ctrls.manual_update &&
    ...
    module_enable = ((isp_sub_module->manual_ctrls.tonemap_mode ==
                     ...
                     CAM_TONEMAP_MODE_FAST)) ? FALSE : TRUE;
