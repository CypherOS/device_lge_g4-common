...
boolean gamma44_init_config(isp_sub_module_t *isp_sub_module,
  gamma44_t *gamma);
static boolean gamma44_store_hw_update(isp_sub_module_t *isp_sub_module,
  gamma44_t *gamma);
 ...
 **/
static int16_t gamma44_apply_contrast_sigmoid(double sigmoid, double multiplier,
 ...
 **/
static boolean gamma44_compute_contrast_table(double sigmoid, int lut_size,
 ...
 **/
static uint16_t gamma44_update_tab_on_contrast(int contrast,
  gamma_rgb_ds_t  *input_table_rgb, gamma_rgb_ds_t  *output_table_rgb)
{
  int             i = 0;
 ...
 */
void gamma44_get_downsampled_table(uint16_t *ispTable,
  uint16_t *table,
  int skipRatio)
{
  int i;
  ...
  if (skipRatio == 1){
    ...
    for (i = 0; i < ISP_GAMMA_NUM_ENTRIES; i++) {
      ispTable[i] = tmp[i] >> (2 + GAMMA_CHROMATIX_TBL_SIZE - ISP_PIPELINE_WIDTH );
     }
 }
}
 ...
 *    contains the current GammaTable value
 */
static int32_t gamma44_get_hi_lo_gamma_bits(uint16_t *table,
  int i)
{
  ...
  entry_clamp = MIN(table[i], MAX_GAMMA_VALUE);
  entry_next_clamp = MIN(table[i + 1], MAX_GAMMA_VALUE);
 ...
 *
 */
static int32_t gamma44_get_last_gamma_value(uint16_t *table)
{
  int32_t       hw_lut_entry = 0;
  uint16_t      delta_lut = 0;
  ...
  last_entry_clamp = MIN(table[ISP_GAMMA_NUM_ENTRIES - 1],
    MAX_GAMMA_VALUE);
...
static void gamma44_update_vfe_table(volatile ISP_GammaConfigCmdType *
  ...
  gamma44_t* gamma, gamma_table_type gamma_table_type)
{
  ...
  case GAMMA_TABLE_OUTDOOR:
    gamma44_copy_gamma(&(gamma->gamma_table_rgb), &gamma->outdoor_gamma_tbl,
                                          ISP_GAMMA_NUM_ENTRIES);

      break;

  case GAMMA_TABLE_LOWLIGHT:
    gamma44_copy_gamma(&(gamma->gamma_table_rgb), &gamma->lowlight_gamma_tbl,
                                          ISP_GAMMA_NUM_ENTRIES);
    ...
    case GAMMA_TABLE_WHITE_BOARD:
      gamma->gamma_lut_size = sizeof(WHITEBOARD_GAMMA_TABLE)/sizeof(uint16_t);
       gamma44_get_downsampled_table(&gamma->gamma_table_rgb.gamma_r[0],
        (uint16_t *)WHITEBOARD_GAMMA_TABLE,
          gamma->gamma_lut_size/ISP_GAMMA_NUM_ENTRIES);
      gamma44_get_downsampled_table(&gamma->gamma_table_rgb.gamma_g[0],
        (uint16_t *)WHITEBOARD_GAMMA_TABLE,
           gamma->gamma_lut_size/ISP_GAMMA_NUM_ENTRIES);
      gamma44_get_downsampled_table(&gamma->gamma_table_rgb.gamma_b[0],
        (uint16_t *)WHITEBOARD_GAMMA_TABLE,
           gamma->gamma_lut_size/ISP_GAMMA_NUM_ENTRIES);
      break;
    ...
    case GAMMA_TABLE_BLACK_BOARD:
      ...
      gamma44_get_downsampled_table(&gamma->gamma_table_rgb.gamma_r[0],
        (uint16_t *)BLACKBOARD_GAMMA_TABLE,
           gamma->gamma_lut_size/ISP_GAMMA_NUM_ENTRIES);
      gamma44_get_downsampled_table(&gamma->gamma_table_rgb.gamma_g[0],
        (uint16_t *)BLACKBOARD_GAMMA_TABLE,
           gamma->gamma_lut_size/ISP_GAMMA_NUM_ENTRIES);
    ...
    default: {
      ...
      if (ret == FALSE) {
        ...
        gamma44_copy_gamma(&(gamma->gamma_table_rgb),
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  if (ret == TRUE && isp_sub_module->submod_trigger_enable == FALSE) {
    ...
    gamma44_update_vfe_table(&gamma->ISP_GammaCfgCmd, &gamma->gamma_table_rgb);
    isp_sub_module->trigger_update_pending = TRUE;
  }
 ...
 *  Return:   None
 **/
static void gamma44_set_solarize_table(gamma44_t *gamma,
  chromatix_parms_type *chromatix_ptr)
{
  ...
  uint16_t reflect_gam_value_R, reflect_gam_value_G, reflect_gam_value_B;
  ...
  gamma44_t *gamma)
{
  ...
  gamma44_chromatix_to_isp_gamma(&gammaChromatixTable,
    &(GAMMA_LOWLIGHT_TABLE(pchromatix_gamma)));

  gamma44_get_downsampled_table((&gamma->lowlight_gamma_tbl.gamma_r[0]),
    &(gammaChromatixTable.gamma_r[0]), skipRatio);
  gamma44_get_downsampled_table((&gamma->lowlight_gamma_tbl.gamma_g[0]),
    &(gammaChromatixTable.gamma_g[0]), skipRatio);
  gamma44_get_downsampled_table((&gamma->lowlight_gamma_tbl.gamma_b[0]),
  ...
  gamma44_get_downsampled_table((&gamma->outdoor_gamma_tbl.gamma_r[0]),
    &(gammaChromatixTable.gamma_r[0]), skipRatio);
  gamma44_get_downsampled_table((&gamma->outdoor_gamma_tbl.gamma_g[0]),
    &(gammaChromatixTable.gamma_g[0]), skipRatio);
  gamma44_get_downsampled_table((&gamma->outdoor_gamma_tbl.gamma_b[0]),
    &(gammaChromatixTable.gamma_b[0]), skipRatio);
  ...
  gamma44_get_downsampled_table(&(gamma->default_gamma_tbl.gamma_r[0]),
    &(gammaChromatixTable.gamma_r[0]), skipRatio);
  gamma44_get_downsampled_table(&(gamma->default_gamma_tbl.gamma_g[0]),
    &(gammaChromatixTable.gamma_g[0]), skipRatio);
  gamma44_get_downsampled_table(&(gamma->default_gamma_tbl.gamma_b[0]),
    &(gammaChromatixTable.gamma_b[0]), skipRatio);

  gamma->gamma_lut_size = GAMMA_CHROMATIX_LUT_SIZE;

  /*load chromatix to gamma*/
  gamma44_copy_gamma(&(gamma->gamma_table_rgb), &gamma->default_gamma_tbl,
    ISP_GAMMA_NUM_ENTRIES);
  ...
  if (gamma->bestshot_mode == CAM_SCENE_MODE_OFF) {
    ...
    switch(gamma->gamma_table_type) {
      ...
      case GAMMA_TABLE_BACKLIGHT:
        ...
        gamma44_update_vfe_table(&gamma->ISP_GammaCfgCmd,
      ...
      default:
        ...
        if (gamma->effects.contrast != DEFAULT_CONTRAST &&
            gamma->effects.spl_effect == CAM_EFFECT_MODE_OFF) {
          gamma44_update_tab_on_contrast(gamma->effects.contrast,
 ...
 *  Return:   0 - Success
 **/
static boolean gamma44_reset_dmi_cfg(
    gamma44_t        *gamma,
 ...
 *  Return:   0 - Success
 **/
static boolean gamma44_store_dmi_info(
    void*            cmd_offset,
    uint32_t         cmd_len,
 ...
 **/
static boolean gamma44_dmi_hw_update(gamma44_t *gamma,
 ...
 *  Return:   None
 **/
static void gamma44_interp_gamma_table(uint16_t *dest,
...
FILL_METADATA:
  if (output->meta_dump_params &&
    ...
    /*fill in DMI info*/
    output->meta_dump_params->frame_meta.lux_idx =
    ...
    void *data3 __unused)
{
  ...
  if (gamma->gamma_ratio.lighting == TRIGGER_OUTDOOR)
    gamma44_interpolate(&gamma->default_gamma_tbl, &gamma->outdoor_gamma_tbl,
      &(gamma->gamma_table_rgb), gamma->gamma_ratio.ratio);
  else if (gamma->gamma_ratio.lighting == TRIGGER_LOWLIGHT)
    gamma44_interpolate(&gamma->default_gamma_tbl, &gamma->lowlight_gamma_tbl,
      &(gamma->gamma_table_rgb), gamma->gamma_ratio.ratio);
  else
    gamma44_copy_gamma(&(gamma->gamma_table_rgb), &gamma->default_gamma_tbl,
      ISP_GAMMA_NUM_ENTRIES);
  ...
  void *data2, void *data3)
{
  ...
  ret = isp_sub_module_util_get_aec_ratio_bright_low(
   ...
   lowlight, new_aec_update, &trigger_ratio);
