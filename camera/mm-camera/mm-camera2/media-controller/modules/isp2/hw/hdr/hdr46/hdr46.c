  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  hdr->wb_gr_ratio = 1.0;
  hdr->wb_gb_ratio = 1.0;
  hdr->aec_update.hdr_exp_ratio = 1.0;
  ...
  isp_sub_module_t *isp_sub_module, mct_event_t *event)
{
  ...
  if (isp_sub_module->trigger_update_pending == TRUE &&
    ...
    if (!F_EQUAL(hdr_mod->aec_update.hdr_exp_ratio, 0.0)) {
       hdr_mod->exp_ratio = hdr_mod->aec_update.hdr_exp_ratio;
    } else {
      hdr_mod->exp_ratio = 1.0;
    }
    hdr_mod->analog_gain = hdr_mod->aec_update.real_gain;
