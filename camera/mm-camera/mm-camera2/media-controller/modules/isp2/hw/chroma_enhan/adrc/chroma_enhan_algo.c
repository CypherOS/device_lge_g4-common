...
void chroma_enhan_algo_adjust_cv_by_adrc(
  ...
  chroma_enhancement_type *cv, aec_update_t aec_update)
{
  ...
  if ((adrc_adj_data->control_adrc_ace == CONTROL_AEC_EXP_SENSITIVITY_RATIO) ||
      (adrc_adj_data->control_adrc_ace == CONTROL_DRC_GAIN)) {
    ...
    if (RegionIdxStrt_lux != RegionIdxEnd_lux) {
      for (i = 0; i < MAX_SETS_FOR_TONE_NOISE_ADJ; i++) {
        if (adrc_adj_data->control_adrc_ace ==
            CONTROL_AEC_EXP_SENSITIVITY_RATIO) {
          trigger = aec_update.hdr_sensitivity_ratio;
          trigger_start[i] =
            (float)adrc_ace_adj_data_start[i].aec_sensitivity_ratio.start;
          trigger_end[i] =
            (float)adrc_ace_adj_data_start[i].aec_sensitivity_ratio.end;
        } else {
      ...
      for (i = 0; i < MAX_SETS_FOR_TONE_NOISE_ADJ; i++) {
        if (adrc_adj_data->control_adrc_ace ==
          ...
          trigger = aec_update.hdr_sensitivity_ratio;
          trigger_start[i] =
            (float)adrc_ace_adj_data_end[i].aec_sensitivity_ratio.start;
          trigger_end[i] =
            (float)adrc_ace_adj_data_end[i].aec_sensitivity_ratio.end;
        } else {
          trigger = aec_update.color_drc_gain;
          trigger_start[i] =
