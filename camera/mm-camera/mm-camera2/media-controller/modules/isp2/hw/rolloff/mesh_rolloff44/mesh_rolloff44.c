 ...
 *  Return void
 **/
static void mesh_rolloff44_calc_flash_trigger(isp_sub_module_t *isp_sub_module,
  ...
  void *data)
{
  ...
  int                            i, rc = 0;
  float                          new_real_gain = 0;
  float                          new_lux_idx = 0;
  float                          new_mired_color_temp = 0;
  cam_flash_mode_t               new_flash_mode = 0;
  ...
  if (isp_sub_module->manual_ctrls.manual_update && !mesh_rolloff->module_streamon) {
  ...
  if (isp_sub_module->tintless_enabled == TRUE) {
    ...
    new_real_gain = mesh_rolloff->aec_update.real_gain;
    new_lux_idx = mesh_rolloff->aec_update.lux_idx;
    new_flash_mode = mesh_rolloff->cur_flash_mode;
    new_mired_color_temp = mesh_rolloff->cur_mired_color_temp;

    ISP_DBG("gain %f, flash %d, lux %f", new_real_gain,
      mesh_rolloff->cur_flash_mode, new_lux_idx);
  ...
  void *data)
{
  ...
  if (isp_sub_module->trigger_update_pending == FALSE) {
    if (!F_EQUAL(mesh_rolloff->aec_update.lux_idx, aec_update->lux_idx) ||
      !F_EQUAL(mesh_rolloff->aec_update.real_gain, aec_update->real_gain)) {
  ...
  void *data)
{
  ...
  mesh_rolloff->cur_mired_color_temp = new_mired_color_temp;
  mesh_rolloff->dual_led_setting = stats_update->awb_update.dual_led_setting;
 ...
 *  Return void
 **/
static void mesh_rolloff44_normalize_table(isp_sub_module_t *isp_sub_module,
  void *mesh_mod_in)
{
  int                        i = 0, j = 0, k = 0;
    ...
    MESH_RollOffTable_V4 *tblOut)
{
  ...
  aec_ratio = isp_sub_module_util_get_aec_ratio_lowlight(
    chromatix_rolloff_trigger->control_rolloff,
    &(chromatix_rolloff_trigger->rolloff_lowlight_trigger), &mod->aec_update);
