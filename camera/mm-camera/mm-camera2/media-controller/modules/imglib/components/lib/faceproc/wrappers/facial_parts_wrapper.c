  ...
  faceproc_info_t *p_old)
{
  ...
  rc = p_lib->FACEPROC_SetDtStep(p_hnd->hdt,
    p_hnd->config.sw_face_search_dens - dens_offset, p_hnd->config.sw_face_search_dens - dens_offset);
  if (rc != FACEPROC_NORMAL) {
...
int facial_parts_wrap_process_result(void *handle, img_frame_t *p_frame,
  faceproc_info_t *p_output, int verify_face, int face_tracked, faceproc_info_t *p_old)
{
  facial_parts_wrap_t *p_hnd;
  facial_parts_wrap_lib_t *p_lib;
