 ...
 *
 **/
int img_thread_mgr_create_pool(int num_of_threads)
{
  int i = 0;

  if ((num_of_threads <= 0) || (num_of_threads > MAX_THREAD_COUNT)) {
    IDBG_ERROR("%s %d: Invalid number of threads. Cannot create threadpool",
      __func__, __LINE__);
  ...
  if (g_thread_pool != NULL) {
    ...
    IDBG_HIGH("%s %d: Thread pool already created ref_count %d",
      __func__, __LINE__, g_thread_pool->ref_count);
  ...
  if (NULL == g_thread_pool) {
    IDBG_ERROR("%s %d: No memory to create new threadpool",
      __func__, __LINE__);
    pthread_mutex_unlock(&g_thread_pool_lock);
  ...
  if (NULL == g_thread_pool->thread) {
    IDBG_ERROR("%s %d: No memory to create threads", __func__, __LINE__);
  ...
  g_thread_pool->ref_count++;
  IDBG_HIGH("%s %d: ref count %d",  __func__, __LINE__,
    g_thread_pool->ref_count);
  pthread_mutex_unlock(&g_thread_pool_lock);
  return IMG_SUCCESS;
...
uint32_t img_thread_mgr_schedule_job(img_thread_job_params_t *job_params)
{
  ...
  g_job_count++;
  p_job->job_id = CREATE_JOB_ID(job_params->client_id, g_job_count);
