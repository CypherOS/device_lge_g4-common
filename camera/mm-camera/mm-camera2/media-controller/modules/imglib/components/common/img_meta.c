...
void *img_get_meta(void *p_meta_data, uint32_t meta_type)
{
  ...
  if ((type < IMG_META_MAX) && p_meta->valid[type]) {
    ...
    case IMG_META_PAAF_RESULT:
      ...
      break;
    default:
      IDBG_ERROR("%s:%d] Not supported meta type %d", __func__, __LINE__, type);
    }
  }
  return val;
}
...
int32_t img_set_meta(void *p_meta_data, uint32_t meta_type, void* val)
{
  ...
  case IMG_META_SW2D_OPS:
    ...
    break;
  case IMG_META_OPAQUE_DATA:
    memcpy(&p_meta->opaque_data, val, sizeof(p_meta->opaque_data));
    break;
  case IMG_META_SEEMORE_CFG:
...
static int32_t img_dump_meta_dcrf_runtime_param(img_meta_t *p_meta, FILE* fd)
{
  ...
  if (runtime_param) {
    ...
    img_dcrf_fov_t *fov_params_main = runtime_param->fov_params_main;
    img_dcrf_fov_t *fov_params_aux = runtime_param->fov_params_aux;
 ...
 * Returns standard image lib codes
 **/
static int32_t __unused img_dump_meta_seemore_cfg(img_meta_t *p_meta, FILE* fd)
{
 ...
 * Returns standard image lib codes
 **/
static int32_t __unused img_dump_meta_stillmore_cfg(img_meta_t *p_meta, FILE* fd)
{
