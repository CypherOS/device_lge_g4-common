  ...
  while (TRUE) {
    ...
    case IMG_MSG_BUNDLE:
      if (p_comp->p_lib->img_algo_process) {
        ...
        if (IMG_ERROR(status)) {
          IDBG_ERROR("%s %d: IMGLIB_BASE algo error %d",
      ...
      frameproc_send_buffers(p_comp, &p_msg->bundle, status);
...
static int frameproc_comp_load_fn_ptrs(frameproc_lib_info_t *p_frameproc_lib)
{
  ...
  *(void **)&(p_frameproc_lib->img_algo_post_process) =
    dlsym(p_frameproc_lib->ptr, "img_algo_post_process");
  if (!p_frameproc_lib->img_algo_post_process) {
    IDBG_ERROR("img_algo_post_process can be NULL for few AOST features");
  }
  ...
  const char *name,  void *p_userdata)
{
  ...
  if (!p_frameproc_lib->ptr) {
    IDBG_ERROR("%s:%d] Error opening frameproc library %s error %s\n",
  ...
  if ((p_ssr_flag != NULL) && (*p_ssr_flag)) {
    if (p_frameproc_lib->img_algo_preload) {
      p_frameproc_lib->img_algo_preload(&p_frameproc_lib->base_ops);
    }
  }
