...
static int module_dcrf_handle_module_event(mct_event_module_t *p_mod_event,
 ...
 * Notes: none
 **/
boolean module_dcrf_handle_module_event(mct_event_module_t *p_mod_event,
  imgbase_client_t *p_client, img_core_ops_t *p_core_ops)
{
  int rc = IMG_SUCCESS;
  ...
  case MCT_EVENT_MODULE_ISP_OUTPUT_DIM: {
    ...
    if (stream_info) {
      ...
      IDBG_MED("%s:%d] MCT_EVENT_MODULE_ISP_OUTPUT_DIM "
        ...
        p_client->dual_cam_sensor_info, p_client->p_intra_port);

      /* send intra port event to master camera */
      if (p_client->peer_session_id &&
        CAM_TYPE_AUX == p_client->dual_cam_sensor_info) {
        mct_event_t event;
        boolean status = FALSE;
        mct_port_t *p_intra_port = p_client->p_intra_port;
        /* Pack into an mct_event object */
        event.direction = MCT_EVENT_INTRA_MOD;
        event.identity =  p_client->peer_session_id;
      ...
      else if (CAM_TYPE_MAIN == p_client->dual_cam_sensor_info &&
