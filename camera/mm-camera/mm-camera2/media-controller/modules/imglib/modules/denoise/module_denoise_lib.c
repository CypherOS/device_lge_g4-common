 ...
 *    @cpp_chromatix: cpp_chromatics table
...
#if defined (CHROMATIX_304) || defined (CHROMATIX_306) || defined (CHROMATIX_307) || \
  defined (CHROMATIX_308) || defined (CHROMATIX_309) || defined(CHROMATIX_308E)
  chromatix_cpp_type *cpp_chromatix;
#endif
  img_gamma_t gamma;
  ...
  cam_denoise_param_t *cam_denoise_param)
{
...
#if defined (CHROMATIX_309)
        chromatix_version =
          chromatix->chromatix_version_info.chromatix_version;
#else
  ...
  defined (CHROMATIX_308) || defined (CHROMATIX_309)
            uint32_t gamma_down_sample_ratio =
              GAMMA_TABLE_SIZE/GAMMA_TABLE_ENTRIES;
            for (i=0; i<IMGLIB_ARRAY_SIZE(wd_lib->low_gamma.table)-1; i++)
              wd_lib->low_gamma.table[i] =
                (uint16_t)((chromatix_gamma->lowlight_gamma_table.gamma_G[i *
                gamma_down_sample_ratio]) +
                ((chromatix_gamma->lowlight_gamma_table.gamma_G[i *
                gamma_down_sample_ratio + 1] -
                chromatix_gamma->lowlight_gamma_table.gamma_G[i *
                gamma_down_sample_ratio]) << 8));
            wd_lib->low_gamma.table[i] =
              (uint16_t)(chromatix_gamma->lowlight_gamma_table.gamma_G
              [GAMMA_TABLE_SIZE - 1] + 0xff00);
#elif defined(CHROMATIX_308E)
            uint32_t gamma_down_sample_ratio =
              GAMMA_TABLE_SIZE/GAMMA_TABLE_ENTRIES;
            for (i=0; i<IMGLIB_ARRAY_SIZE(wd_lib->low_gamma.table)-1; i++)
              wd_lib->low_gamma.table[i] =
                (uint16_t)((chromatix_gamma->gamma_table[LOWLIGHT_GAMMA_IDX].gamma_G[i *
                gamma_down_sample_ratio]) +
                ((chromatix_gamma->gamma_table[LOWLIGHT_GAMMA_IDX].gamma_G[i *
                gamma_down_sample_ratio + 1] -
                chromatix_gamma->gamma_table[LOWLIGHT_GAMMA_IDX].gamma_G[i *
                gamma_down_sample_ratio]) << 8));
            wd_lib->low_gamma.table[i] =
              (uint16_t)(chromatix_gamma->gamma_table[LOWLIGHT_GAMMA_IDX].gamma_G
              [GAMMA_TABLE_SIZE - 1] + 0xff00);
#else
            for (i=0; i<IMGLIB_ARRAY_SIZE(wd_lib->low_gamma.table)-1; i++)
              wd_lib->low_gamma.table[i] =
...
#if defined (CHROMATIX_304) || defined (CHROMATIX_306) || defined (CHROMATIX_307) || \
  defined (CHROMATIX_308) || defined (CHROMATIX_309) || defined(CHROMATIX_308E)
                rc = IMG_COMP_SET_PARAM(&wd_lib->comp, QWD_CPP_CHROMATIX,
                  wd_lib->cpp_chromatix);
                if (rc == IMG_SUCCESS) {
#endif
                sum = 0;
  ...
  defined (CHROMATIX_308) || defined (CHROMATIX_309) || defined(CHROMATIX_308E)
                  ...
                  CDBG_ERROR("Cannot set QWD_CPP_CHROMATIX in %s\n", __func__);
                }
#endif
              } else {
          ...
          CDBG_ERROR("Wrong chromatix version 0x%x is not 0x%x in %s\n",
            chromatix_version, MODULE_DENOISE_LIB_CHROMATIX_VERSION, __func__);
        }
    } else {
  ...
  if (lib_handle) {
    if (MODULE_DENOISE_LIB_SINGLE_INSTANCE) {
      if (pthread_mutex_destroy(&wd_lib_handle->mutex)) {
        CDBG_ERROR("Cannot destroy mutex IN %S\n", __func__);
      }
    }
