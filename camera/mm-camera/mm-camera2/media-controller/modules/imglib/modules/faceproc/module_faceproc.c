  ...
  faceproc_client_t *p_client, uint32_t identity)
{
  ...
  if (s_idx < 0) {
    IDBG_ERROR("%s:%d] Cannot find stream idx for stream identity 0x%x",
      __func__, __LINE__, identity);
    return IMG_ERR_GENERAL;
  }

  if (!p_client->fp_stream[s_idx].buff_configured) {
    IDBG_ERROR("%s:%d] Stream 0x%x not ready",
      __func__, __LINE__, identity);
    return IMG_ERR_GENERAL;
  }
  ...
  if (IMG_SUCCEEDED(rc)) {
    ...
    IDBG_ERROR("%s:%d] Error cannot map buffers %d", __func__, __LINE__, rc);
  }
  return rc;
}
  ...
  mct_event_t *event)
{
  ...
  IDBG_LOW("%s:%d] port %p id %x E", __func__, __LINE__,
    port, event->identity);
  ...
  IDBG_LOW("%s:%d] type %d s_idx %d", __func__, __LINE__, event->type, s_idx);
    ...
    case MCT_EVENT_CONTROL_STREAMON: {
      ...
      if (p_client->fp_stream[s_idx].processing_stream) {
        ...
        if (!p_mod->active) {
          ...
          p_mod->active = TRUE;
          IDBG_MED("%s:%d] p mod active now", __func__, __LINE__);
          if (p_client->fp_stream[s_idx].streamon) {
            IDBG_HIGH("%s:%d] FD_STREAMON already done for stream %x", __func__, __LINE__,
              p_client->fp_stream[s_idx].identity);
          } else {
            IDBG_HIGH("%s:%d] FD_STREAMON for stream %x", __func__, __LINE__,
              p_client->fp_stream[s_idx].identity);
          ...
          IDBG_HIGH("%s:%d] p mod was active already", __func__, __LINE__);
        }
        pthread_mutex_unlock(&p_client->mutex);
