...
/** iface_axi_process_reg_update_mask
 *
 *  Description:
 ...
 **/
static int iface_axi_process_reg_update_fail(
    iface_axi_hw_t       *axi_hw,
    uint16_t              error_mask,
...
/** iface_axi_process_buffer_fail_mask
 *
 *  Description:
 *  this funciton is called to create the framedrop report due
 ...
 *  Returns 0 for success and negative error on failure
 **/
static int iface_axi_process_buffer_fail_mask(
    iface_axi_hw_t       *axi_hw,
    uint32_t              error_mask,
    uint32_t              error_frame_id,
    iface_axi_framedrop  *frame_drop)
    ...
    enum msm_vfe_input_src input_src)
{
  ...
  rc = iface_axi_process_buffer_fail_mask(
      axi_hw,
  ...
  rc = iface_axi_process_reg_update_fail(
      axi_hw,
  ...
  iface_thread_t *thread_data __unused)
{
  ...
  if (rc >= 0) {
    ...
    case ISP_EVENT_IOMMU_P_FAULT: {
      ...
      switch (v4l2_event.type) {
      case ISP_EVENT_HW_FATAL_ERROR:
        IFACE_ERR("HW Error occured at Frame Id %d\n", error_event->frame_id);
        break;

      case ISP_EVENT_PING_PONG_MISMATCH:
        IFACE_ERR("Ping Pong mismatch occured at Frame Id %d\n", error_event->frame_id);
        break;

      case ISP_EVENT_REG_UPDATE_MISSING:
        IFACE_ERR("reg_update_missing occured at Frame Id %d\n", error_event->frame_id);
        break;

      case ISP_EVENT_BUF_FATAL_ERROR:
        IFACE_ERR("Buf Fatal Error occured at Frame Id %d\n", error_event->frame_id);
        break;

      case ISP_EVENT_IOMMU_P_FAULT:
        IFACE_ERR("IOMMU page fault occured at Frame Id %d\n", error_event->frame_id);
        break;
      ...
      rc = axi_hw->notify_ops->notify(
        ...
        IFACE_AXI_NOTIFY_PAGE_FAULT, &bus_msg,
