...
int iface_util_fe_cfg(int fd, cam_dimension_t dim,
  uint32_t op_pixel_clk, cam_format_t fmt);
  ...
  sensor_src_port_cap_t *sensor_cap)
{
  ...
  if((i == sensor_cap->num_cid_ch) ||
     (i >= SENSOR_CID_CH_MAX)) {
    CDBG_ERROR("%s:%d error cannot find primary sensor format cids %d",
...
void iface_util_dump_sensor_cfg(iface_sink_port_t *sink_port)
{
  ...
  CDBG("%s: sensor dim: width = %d, heght = %d, fmt = %d, is_bayer = %d, init_skip = %d\n",
 ...
 *    return int
 **/
void iface_util_fill_sensor_csid_info_to_hw_by_format(iface_hw_stream_t *hw_stream,
  ...
  uint8_t             initial_frame_skip)
{
 ...
 iface_util_decide_zsl_mode(session, reserving_pix_stream,
  ...
  iface_util_fill_sensor_csid_info_to_hw_by_format(reserving_pix_stream,
  ...
  iface_resource_request_t *isp_resource)
{
  ...
  for (i = 0; i < isp_resource->num_pix_stream
    && i < ISP_NUM_PIX_STREAM_MAX; i++) {
    IFACE_ERR("pix stream %d\n", i);
    IFACE_ERR("num mct stream mapped = %d(Mapped stream ID[0] = %d)\n",
 ...
 *
 **/
int32_t iface_util_reset_used_resource_mask(int isp_id,
  iface_session_t *iface_session)
{
  int32_t  rc = 0;
  ...
  enum msm_vfe_axi_stream_src axi_path)
{
  ...
  camif_hw_stream->camif_output_format = CAMIF_QCOM_FORMAT;
  ...
  iface_util_fill_sensor_csid_info_to_hw_by_format(camif_hw_stream,
  ...
  uint32_t          *isp_id)
{
  ...
  if ((session->session_resource.isp_id_mask & (1 << VFE0)) &&
    ...
    *isp_id = VFE0;
    rc = iface_get_rdi_resource(VFE0, session, reserved_rdi_intf);
  } else if ((session->session_resource.isp_id_mask & (1 << VFE1)) &&
    session->session_resource.used_wm[VFE1] < NUM_AXI_WM) {
    *isp_id = VFE1;
    rc = iface_get_rdi_resource(VFE1, session, reserved_rdi_intf);
  } else {
  ...
  if (rc < 0) {
    ...
    *isp_id = (*isp_id == VFE0)? VFE1: VFE0;
    rc = iface_get_rdi_resource(*isp_id, session, reserved_rdi_intf);
  }
  ...
  iface_bundle_pdaf_info_t *bundle_pdaf_info)
{
  ...
  memset(camif_hw_stream->cids, 0, sizeof(camif_hw_stream->cids));
  camif_hw_stream->csid = 0;
  ...
  sensor_meta_t     *meta_ch_info)
{
  ...
  memset(rdi_hw_stream->cids, 0, sizeof(rdi_hw_stream->cids));
  rdi_hw_stream->csid = 0;
 ...
 * Return: 0 - success and negative value - failure
 **/
static int iface_util_process_page_fault(iface_t *iface,
  ...
  int                  axi_idx __unused)
{
  ...
  for (i = 0; i < axi_framedrop->framedrop.num_streams
    ...
    if (hw_stream != NULL) {
      ...
      CDBG("%s: Error! Could not find user_stream for session %d\n",
        __func__, session_id);
      continue;
    }
  }
...

{
  ...
  CDBG_ERROR("%s:%d DIS enable flag = %d\n", __func__, __LINE__,
        session->dis_enabled);
  return 0;
}
 ...
 * Return: 0- success and negative value - failure
 **/
int iface_util_set_sensor_hdr(iface_t *iface, uint32_t session_id,
                ...
                cam_stream_size_info_t *streams_desc)
{
  ...
  if (session->dis_enabled) {
    CDBG_ERROR("%s:%d DIS enabled, no native buf opt\n", __func__, __LINE__);
    return 0;
  }
  ...
  int32_t *hfr_mode)
{
  ...
  if (hw_stream == NULL) {
     CDBG_ERROR("%s: can't find hw stream, only save hfr mode\n", __func__);
     return 0;
  }
  ...
  iface_offline_isp_info_t *offline_info)
{
  ...
  if (offline_stream_ready == FALSE) {
    for (i = 0; i< IFACE_MAX_STREAMS; i++) {
      ...
      for (isp_id = 0; isp_id < VFE_MAX; isp_id++) {
        if (offline_hw_stream->interface_mask & (1 << (16 * isp_id +
          ...
          rc = iface_util_fe_cfg(iface->isp_axi_data.axi_data[isp_id].fd,
    ...
    for (i= 0; i < IFACE_MAX_STATS_STREAMS; i++) {
  ...
  iface_frame_divert_notify_t *divert_event)
{
  ...
  if (!hw_stream->divert_to_3a) {
    ...
    if ((buf_divert->frame_id <= hw_stream->last_frame_id &&
      ...
      bus_msg.sessionid =  user_stream->session_id;
  ...
  uint32_t             session_id)
{
  ...
  for (i=0; i<axi_frame_drop->framedrop.num_streams
    ...
    for (j = 0; j < hw_stream->num_mct_stream_mapped
      ...
      CDBG("%s: Send buf drop to peer (0x%x, fid: 0x%x)",
  ...
  int                  axi_idx)
{
  ...
  if (session->overflow_recovery_state ==
    ...
    if (sof_event->frame_id <= session->sof_frame_id) {
      CDBG("%s: after overflow recovery, skip all posted SOF %d",
        __func__, sof_event->frame_id);
      return 0;
    } else {
      CDBG_HIGH("%s: complete recovery, resume SOF %d post",
        __func__, sof_event->frame_id);
      session->overflow_recovery_state =
  ...
  uint32_t type, void *notify_data, uint32_t notify_data_size)
{
  ...
  case IFACE_AXI_NOTIFY_PAGE_FAULT: {
    rc = iface_util_process_page_fault(iface, (mct_bus_msg_t*)notify_data);
  }
    break;
...
#ifdef _ANDROID_
    /* use testgen instead of sensor data*/
    property_get("persist.camera.isp.testgen", value, "0");
    testgen_enable = atoi(value);
#endif
    if (testgen_enable || sink_port->sensor_out_info.is_dummy) {
 ...
 *
 **/
int iface_util_reset_resource_mask(iface_session_t *iface_session,
  uint32_t interface_mask, uint32_t isp_id_mask)
{
  ...
  for (i = 0; i < VFE_MAX; i++) {
    if (isp_id_mask & (1 << i) && (interface_mask & (0xffff << (16 * i)))) {
      ...
      iface_util_reset_used_resource_mask(i, iface_session);
  ...
  iface_session_t *session, iface_hw_stream_t *hw_stream)
{
  ...
  iface_util_reset_resource_mask(session, hw_stream->interface_mask,
                ...
                iface_session_t *session, iface_hw_stream_t *hw_stream)
{
 ...
 #ifdef _ANDROID_
   ret = property_get("persist.camera.isp.buf_cnt", value, NULL);
   if ( (ret > 0 ) && atoi(value)){
     max_native_buf_num = atoi(value);
   }
 #endif
  ...
  iface_t *iface)
{
  ...
  if (session->ms_type == MS_TYPE_MASTER) {
    ...
    for (intf = 0; intf < IFACE_INTF_MAX; intf++) {
      if ((session->session_resource.used_resource_mask &
        (1 << (16 * vfe_intf + intf))) && intf != (iface_intf_type_t)ms_cmd.primary_intf) {
 ...
 uint32_t user_streamon_id, uint32_t is_streamon)
{
   ...
   if (is_streamon) {
     if (hw_stream->state == IFACE_HW_STREAM_STATE_HW_CFG_DONE) {
       ...
       CDBG_ERROR("%s:skip, hw stream state = %d, hw stream id = %x, mct streamon id = %x\n",
  ...
  uint32_t user_stream_id)
{
  ...
  if (!hw_stream->shared_hw_stream) {
    CDBG_ERROR("%s: hw stream already streaming. hw stream id %x, is_share_hw_stream?%d\n",
