  ...
  mct_queue_t **parm_q, pthread_mutex_t *parm_q_lock)
{
  ...
  if (!parm_q_lock) {
    CDBG_ERROR("%s: parm_q_lock parm_q\n", __func__);
    return -1;
  }
...
int iface_stop_session(iface_t *iface, uint32_t session_id)
{
  ...
  for (i = 0; i < IFACE_MAX_SESSIONS; i++) {
	...
	/*
      ...
      rc = iface_session_sem_thread_stop(
        &session->session_thread);
      if (rc < 0) {
        CDBG_ERROR("%s: stop session thread failed, rc = %d\n", __func__, rc);
        return rc;
      }
 ...
 *          -1 - Session not found
 **/
int iface_set_pipeline_delay(iface_t *iface, uint32_t session_id,
  ...
  mct_event_t     *event)
{
  ...
  if (peer && iface_sink_port->port != peer) {
    ...
    iface_session->ms_type =
      iface_util_map_cam_sync_type_to_iface(related_session_info->type);
#endif
  ...
  uint32_t  halt_frame_id)
{
  ...
  CDBG_HIGH("%s:%d overflow_dbg HALT AXI... \n", __func__, __LINE__);
  ...
  CDBG_HIGH("%s:%d overflow_dbg RESET VFE HW ... \n", __func__, __LINE__);
  ...
  if (session->hvx.enabled == TRUE) {
    ...
    if (rc < 0) {
      CDBG_ERROR("failed: iface_hvx_streamon rc %d\n", rc);
      return rc;
    }
  }

  CDBG_HIGH("%s:%d overflow_dbg RESTART AXI ... \n", __func__, __LINE__);
  for (i = 0; i< VFE_MAX; i++) {
    if (session->session_resource.isp_id_mask & (1 << i)) {
        CDBG("%s:%d VFE%d \n", __func__, __LINE__, i);
        iface->isp_axi_data.axi_data[i].axi_hw_ops->action(
...
#if 0
 /* only needed when STOP IMMEDIATELY,, if STOP at FRAME BOUND, then we dont need */
  /* HW reset ispif*/
  CDBG_ERROR("%s: overflow_dbg  reset ISPIF\n", __func__);
  ...
  CDBG("%s: ispif_hw_reset done\n", __func__);
  session->overflow_recovery_state = IFACE_AXI_RECOVERY_STATE_ISPIF_RESET_DONE;
#endif

  CDBG_HIGH("%s:%d overflow_dbg RESTART ISPIF ... \n", __func__, __LINE__);
  ...
  CDBG_HIGH("%s: overflow recovery done! skip 1st SOF and prepare missed buf\n",
    __func__);
  ...
  uint32_t frame_id)
{
  ...
  case CAM_INTF_PARM_SENSOR_HDR:{
      rc = iface_util_set_sensor_hdr(iface, session->session_id,
