 ...
 *
 **/
static uint8_t *iface_do_mmap_ion(int ion_fd, struct ion_allocation_data *alloc,
...
static boolean iface_init_hal_buffer(void *data, void *user_data)
{
  ...
  for (i = 0; i < IFACE_MAX_IMG_BUF; i++) {
    if (!map_buf->iface_map_bufs[i].vaddr) {
      idx = i;
      break;
    }
  }
  ...
  /*put the MCT buffer onto iface buf list*/
  map_buf->iface_map_bufs[idx].vaddr = img_buf->buf_planes[0].buf;
  ...
  for (i = 0; i < IFACE_MAX_IMG_BUF; i++) {
    ...
    if (map_buf->iface_map_bufs[i].vaddr &&
      ...
      if (map_buf->iface_map_bufs[i].is_reg) {
        ...
        map_buf->iface_map_bufs[i].vaddr = NULL;
        map_buf->cnt--;
      } else {
  ...
  int vfe_fd)
{
  ...
  for (i = 0; i < IFACE_MAX_IMG_BUF; i++) {
    ...
    if (!bufq->image_bufs[i].is_reg && bufq->image_bufs[i].vaddr) {
  ...
  iface_bufq_t *bufq, int vfe_fd)
{
  ...
  for (i = 0; i < bufq->current_num_buffer; i++) {
    rc = iface_queue_buf_int(buf_mgr, bufq->user_bufq_handle, i, dirty_buf, vfe_fd);
    if (rc < 0) {
