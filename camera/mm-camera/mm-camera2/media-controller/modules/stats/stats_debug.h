...
#if (STATS_DEBUG)
#define CDBG_ASD(fmt, args...) \
  CLOGL(CAM_STATS_ASD_MODULE,  fmt, ##args)

#define CDBG_AFD(fmt, args...) \
  CLOGL(CAM_STATS_AFD_MODULE,  fmt, ##args)

#else

#define CDBG_ASD(fmt, args...) do {STATS_UNUSED(asd); PPRINT (fmt, ##args)} while(0)
#define CDBG_AFD(fmt, args...) do {STATS_UNUSED(afd); PPRINT (fmt, ##args)} while(0)
