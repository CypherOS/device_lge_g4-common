...
static boolean awb_biz_set_init(awb_biz_t *awb,awb_set_parameter_init_t* init_param)
{
  ...
  awb_dual_led_calibaration dual_led_calibration = {0};
...
static int awb_biz_set_current_wb(awb_biz_t *awb, uint32_t new_wb)
{
  ...
  if (rc == 0 && update_hw) {
    ...
    awb->curr_gains.b_gain = rgb_gains.b_gain * gain_adjust.red_gain_adj;
  ...
  awb_set_aec_parms  *aec_parms)
{
  ...
  set_param_info.u.aec_parms.is_hdr_drc_enabled = aec_parms->is_hdr_drc_enabled;
  if (awb->awb_algo_ops.set_parameters) {
  ...
  if (Q3A_STATS_STREAM_OFFLINE == stats->isp_stream_type) {
    ...
    if (Q3A_OPERATION_MODE_PREVIEW == awb->op_mode ||
      ...
      if (awb->current_wb_type == AWB_AUTO && !awb->awb_locked){
        ...
        if (FALSE == awb->flash_on) {
          awb_biz_estimate_ccm((void*)awb, awb->color_temp, &(awb->core_output.ccm));
          awb->core_output.snapshot_ccm = awb->core_output.ccm;
        }
      }
    } else {
