 ...
 * @bayer_gb_hist: Array containing the green-blue histogram values smaller than the max storage allocated
 * @hdr_mode: HDR mode
 * @array_size: array size of stats
 **/
typedef struct
{
  const uint32*            bayer_r_hist;
  const uint32*            bayer_b_hist;
  const uint32*            bayer_gr_hist;
  const uint32*            bayer_gb_hist;
  uint32                   num_bins;
