...
#define AF_FIRSTRUN_SPOTLIGHT_DETECT_CNT_TH 1
 ...
 * Note: This enum is to be moved to chromatix in next chromatix
 * revision. Right now we are going to use reserved section.
 */
typedef enum {
  AF_CHROMATIX_FOCUS_MODE_DEFAULT,
  AF_CHROMATIX_FOCUS_MODE_MACRO,
  AF_CHROMATIX_FOCUS_MODE_NORMAL,
  AF_CHROMATIX_FOCUS_MODE_CAF,
  AF_CHROMATIX_FOCUS_MODE_MANUAL,
  AF_CHROMATIX_FOCUS_MODE_MAX
} af_chromatix_focus_mode_type;;
 ...
 *   @haf_debug_size: HAF debug data size.
 *   @tof_debug_size: TOF debug data size.
 *   @dciaf_debug_size: DCIAF debug data size.
 *   @pdaf_debug_size: PDAF debug data size.
 *   @debug_data: Array store the AF debug data.
 *   @stats_buffer_size: AF debug stats size.
 *   @stats_buffer: Array store to AF debug stats.
 *
 *   AF debug data
 **/
typedef struct _af_debug_info_t {
  boolean enable;
  uint32_t af_debug_size;
  uint32_t haf_debug_size;
  uint32_t tof_debug_size;
  uint32_t dciaf_debug_size;
  uint32_t pdaf_debug_size;
  char debug_data[AF_DEBUG_DATA_SIZE];
  uint32_t stats_buffer_size;
  char stats_buffer[AF_STATS_DEBUG_DATA_SIZE];
} af_debug_info_t;
...
typedef enum {
  ...
  AF_OUTPUT_DCIAF_CONFIG   = (1 << 15),
  AF_OUTPUT_CUSTOM         = (1 << 16),
  AF_OUTPUT_DEBUG_DATA     = (1 << 17),
} af_output_type;
