...
void af_biz_util_update_output_data(af_biz_internal *af,
  ...
  case AF_OUTPUT_MOBICAT_METADATA:
    af->af_out.type = (af_output_type)(AF_OUTPUT_MOBICAT_METADATA | af->af_out.type);
    memcpy(&af->af_out.mobicat, (af_mobicat_t*)data,
      sizeof(af_mobicat_t));
    break;
  ...
  case AF_OUTPUT_DEBUG_DATA: {
    if (AF_DEBUG_DATA_LEVEL_VERBOSE == af->af_input.exif_dbg_level) {
      ...
      debug_info->af_debug_size = core_output->af_debug_size;
      debug_info->haf_debug_size = core_output->haf_debug_size;
      debug_info->tof_debug_size = core_output->tof_debug_size;
      debug_info->dciaf_debug_size = core_output->dciaf_debug_size;
      debug_info->pdaf_debug_size = core_output->pdaf_debug_size;
      debug_info->stats_buffer_size = core_output->stats_buffer_size;
    ...
    /* Copy the AF debug data pointer to output struct */
      ...
      debug_info->enable = TRUE;
      debug_info->af_debug_size = core_output->af_debug_size;
      debug_info->haf_debug_size = 0;
      debug_info->tof_debug_size = 0;
      debug_info->dciaf_debug_size = 0;
      debug_info->pdaf_debug_size = 0;
      debug_info->stats_buffer_size = 0;
      af->af_out.type = (af_output_type)(AF_OUTPUT_DEBUG_DATA | af->af_out.type);
    }
  }
    break;
...
int af_biz_util_get_focus_distance(af_biz_internal *af,
  ...
  af_algo_tune_parms_t *fptr = af->af_input.tuning_info;
...
inline void print_step_table(const char *str, step_size_t *light) {
...
inline void print_bv_threshold(const char *str, BV_threshold_t *BV_thres) {
#if !defined(LOG_DEBUG)
  CAM_UNUSED_PARAM(str);
...
{
  af_header_info_t *info = &af->af_input.tuning_info->af_header_info;
...
{
  af_tuning_single_t *single = &af->af_input.tuning_info->af_algo.af_single;
  uint8_t i;
  step_size_table_t *table;
  step_size_t *light;
  single_threshold_t *thres;
  BV_threshold_t *BV_thres;
...
static void af_biz_debug_print_algo_info(af_biz_internal *af) {
  af_tuning_algo_t *info = &af->af_input.tuning_info->af_algo;
  af_tuning_continuous_t *cont = &info->af_cont;
  af_tuning_exhaustive_t *exh = &info->af_exh;
  af_tuning_fullsweep_t *full = &info->af_full;
  af_tuning_sp_t *sp = &info->af_sp;
  af_tuning_single_t *single = &info->af_single;
  af_shake_resistant_t *shake = &info->af_shake_resistant;
  af_motion_sensor_t *motion = &info->af_motion_sensor;
  af_fd_priority_caf_t *fd = &info->fd_prio;

  af_tuning_sad_t *sad = &cont->af_sad;
  af_tuning_gyro_t *gyro = &cont->af_gyro;
...
{
  af_tuning_vfe_t *info = &af->af_input.tuning_info->af_vfe[0];
  int i = 0;
...
static int af_biz_focus_mode_internal(af_biz_internal *af,
  ...
  int rc = 0;
  af_tuning_algo_t *algo_tuning = NULL;
  ...
  AF_LOW("Setting focus mode to: %d", af_mode);

  switch (af_mode) {

  case AF_MODE_AUTO:
    af->af_input.search_range.near_end_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_DEFAULT].near_end;
    af->af_input.search_range.far_end_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_DEFAULT].far_end;
    af->af_input.search_range.default_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_DEFAULT].default_pos;
  ...
  case AF_MODE_CAF:
    af->af_input.search_range.near_end_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_CAF].near_end;
    af->af_input.search_range.far_end_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_CAF].far_end;
    af->af_input.search_range.default_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_CAF].default_pos;
    af->af_input.search_range.hyperfocal_position =
      algo_tuning->position_normal_hyperfocal;
    break;
  ...
  case AF_MODE_CAF_NORMAL:
    af->af_input.search_range.near_end_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_NORMAL].near_end;
    af->af_input.search_range.far_end_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_NORMAL].far_end;
    af->af_input.search_range.default_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_NORMAL].default_pos;
    af->af_input.search_range.hyperfocal_position =
      algo_tuning->position_normal_hyperfocal;
    break;
  ...
  case AF_MODE_CAF_MACRO:
    af->af_input.search_range.near_end_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_MACRO].near_end;
    af->af_input.search_range.far_end_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_MACRO].far_end;
    af->af_input.search_range.default_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_MACRO].default_pos;
    af->af_input.search_range.hyperfocal_position =
      algo_tuning->position_normal_hyperfocal;
    break;
  ...
  case AF_MODE_MANUAL:
    af->af_input.search_range.near_end_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_MANUAL].near_end;
    af->af_input.search_range.far_end_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_MANUAL].far_end;
    af->af_input.search_range.default_position =
      af->search_limit[AF_CHROMATIX_FOCUS_MODE_MANUAL].default_pos;
    af->af_input.search_range.hyperfocal_position =
      algo_tuning->position_normal_hyperfocal;
    break;
  default:
    ...
    if (af->af_mode_changed == TRUE) {
      ...
      if (!IS_CAF_ENABLED(af_mode)) {
        if (af_mode == AF_MODE_INFINITY) {
          AF_LOW(" Focus mode change to Infinity, Lens Reset");
          af_biz_move_lens_to(af, af->af_mode.default_pos);
        }
      } // else condition looks wrong, it wont reset lens on manual mode
      else if(af->af_mode.prev_mode == AF_MODE_MANUAL ||
        af_mode == AF_MODE_MANUAL ||
        af->af_mode.prev_mode == AF_MODE_MANUAL_HAL1 ||
        af_mode == AF_MODE_MANUAL_HAL1){
        // also reset the lens when set to manual mode, as af_port will control
        // the lens directly in manual mode, should reset it to a default postion
        // so the af_port will know where it is
        AF_LOW(" Focus mode change, previous mode Manual, Lens Reset");
...
void af_biz_initialize_internal_data(af_biz_internal *af) {
  ...
  af->af_input.debug_stats = af->af_out.debug_info.stats_buffer;
  af->af_input.debug_stats_max_size = AF_STATS_DEBUG_DATA_SIZE;
...
void af_biz_handle_stats_config_paaf(af_biz_internal *af,
  ...
  af_tuning_sw_stats_t *tuning = &af->af_input.tuning_info->af_swaf_config;
  ...
  bf_fw_roi_cfg_t *af_fw_roi_cfg)
{
  ...
  for (row = 0; row < config.total_row &&
    ...
    for (col = 0; col < config.total_col &&
      ...
      rej_row = abs(col - config.total_row/2);
      rej_col = abs(row - config.total_col/2);
...
void af_biz_handle_stats_config_vfe_fw(af_biz_internal *af,
  ...
  fw_cfg->bf_input_cfg.bf_input_g_sel = core_stats_info->bf_input_cfg.bf_g_sel;
  fw_cfg->bf_input_cfg.bf_channel_sel =
    core_stats_info->bf_input_cfg.bf_input_sel;
...
void af_biz_handle_stats_config_vfe_grid(af_biz_internal *af,
  ...
  stats_config->bf.r_min =
    af->af_input.tuning_info->af_vfe[AF_KERNEL_PRIMARY].config.fv_min;
  ...
  stats_config->bf_scale.r_min =
    af->af_input.tuning_info->af_vfe[AF_KERNEL_SCALE].config.fv_min;
  ...
  stats_config->bf_scale.scale =
    af->af_input.tuning_info->af_vfe[AF_KERNEL_SCALE].config.bf_scale_factor;
...
void af_biz_handle_stats_config(af_biz_internal *af,
  ...
  if (core_stats_info->mask & AF_CORE_STATS_MASK_DEPTH_ASSISTED) {
    if (core_stats_info->daaf_config.daaf_type == AF_DAAF_DCIAF) {
      af_biz_util_update_output_data(af,
...
void af_biz_handle_core_output(af_biz_internal *af) {
  ...
  if (core_out->mask & AF_CORE_OUTPUT_MOBICAT_METADATA) {
    af_biz_util_update_output_data(af, AF_OUTPUT_MOBICAT_METADATA,
      &core_out->mobicat_output_data);
  }
...
{
  af_tuning_algo_t *tuning_ptr = &af->af_input.tuning_info->af_algo;
  int i = 0;
  int j = 0;

  for (i = 0; i < AF_CHROMATIX_FOCUS_MODE_MAX; i++) {
    j = i*3; // three parameters inside each limit structure

    af->search_limit[i].near_end = tuning_ptr->reserve[j];
    af->search_limit[i].far_end = tuning_ptr->reserve[j+1];
    af->search_limit[i].default_pos = tuning_ptr->reserve[j+2];
