...
/** af_ctof_rebase_reference: Notification of focus search end &
*   reset any reference parameters if required.
*
*  Return: void
**/
boolean af_ctof_rebase_reference(af_alg_interface_t *p_alg_intf)
{
  ...
  if(p_alg->p_tof_alg->rebase_reference){
    result = p_alg->p_tof_alg->rebase_reference(p_alg->p_tof_alg);
  }
  return result;
}
...
af_alg_interface_t* af_alg_CTOF_create(af_alg_util_interface_t *p_alg_util_instance)
{
  ...
  p_alg->af_alg.rebase_reference        = af_ctof_rebase_reference;
