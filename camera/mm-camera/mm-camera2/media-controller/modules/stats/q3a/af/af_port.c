...
boolean af_port_update_af_state(mct_port_t  *port,
  ...
  AF_LOW("Af State changed new_state=%d, old_state=%d!, cause=%d, af_mode=%d",
  ...
  uint32_t regular_sof_id)
{
  ...
  pthread_mutex_unlock(&(af_port->focus_state_mutex));
  af_immediate.af_trigger.trigger_id = af_port->af_trans.trigger_id;
  af_immediate.focus_mode = af_port->bus.focus_mode;
  af_immediate.f_distance = af_port->focus_distance_info;
...
boolean af_port_process_status_update(
  ...
  AF_LOW("Status Update from algo! %d",
    af_status->status);
  ...
  af_output_data_t *output)
{
  mct_event_t             event;
  mct_bus_msg_t           bus_msg;
  uint8_t                 af_info[MAX_AF_STATS_DATA_SIZE];
  af_port_private_t       *private;
  int                     size;
  ...
  if (output->mobicat.mobicat_data_size > MAX_AF_STATS_DATA_SIZE) {
    AF_ERR("BUF size error");
    return;
  }
  ...
  bus_msg.msg = (void *)&af_info;
  size = output->mobicat.mobicat_data_size;
  bus_msg.size = size;
...
static void af_port_send_exif_debug_data(mct_port_t *port)
{
  ...
  if (debug_info->enable) {
    ...
    af_info->af_debug_data_size = debug_info->af_debug_size;
    af_info->haf_debug_data_size = debug_info->haf_debug_size;
    af_info->tof_debug_data_size = debug_info->tof_debug_size;
    af_info->dciaf_debug_data_size = debug_info->dciaf_debug_size;
    af_info->pdaf_debug_data_size = debug_info->pdaf_debug_size;
    af_info->af_stats_buffer_size = debug_info->stats_buffer_size;
    AF_LOW("debug data size: af: %d, haf: %d, tof: %d, dciaf: %d,"
      "pdaf: %d, af stats: %d",
      debug_info->af_debug_size, debug_info->haf_debug_size,
      debug_info->tof_debug_size, debug_info->dciaf_debug_size,
      debug_info->pdaf_debug_size, debug_info->stats_buffer_size);
    memcpy(af_info->af_private_debug_data,
      debug_info->debug_data, AF_DEBUG_DATA_SIZE);
    memcpy(af_info->af_stats_private_debug_data,
      debug_info->stats_buffer, AF_STATS_DEBUG_DATA_SIZE);
    event.direction = MCT_EVENT_UPSTREAM;
    event.identity = private->reserved_id;
    event.type = MCT_EVENT_MODULE_EVENT;
    event.u.module_event.type = MCT_EVENT_MODULE_STATS_POST_TO_BUS;
...
static void af_port_populate_reserve_tuning_params(
  ...
  *core_tuning, af_algo_tune_parms_t *tuning_info){

  int i,j;
  int resv_enabled = (int)tuning_info->af_algo.af_haf.tof.reserve[0];
  if (resv_enabled) {
    ...
    core_tuning->frames_to_wait_stable =
      (unsigned short)tuning_info->af_algo.af_haf.tof.reserve[6];
    core_tuning->num_of_samples =
      (int)tuning_info->af_algo.af_haf.tof.reserve[7];

    core_tuning->af_actuator_hysteresis_comp =
      (int)tuning_info->af_algo.af_haf.tof.reserve[8];
    core_tuning->af_actuator_shift_comp =
      (int)tuning_info->af_algo.af_haf.tof.reserve[9];
    core_tuning->af_sensitiviy =
      (float)tuning_info->af_algo.af_haf.tof.reserve[10];
    ...
    core_tuning->af_sad.gain_max =
      (float)tuning_info->af_algo.af_haf.tof.reserve[19];
    core_tuning->af_sad.gain_min =
      (float)tuning_info->af_algo.af_haf.tof.reserve[20];
    core_tuning->af_sad.ref_gain_max =
      (float)tuning_info->af_algo.af_haf.tof.reserve[21];
    core_tuning->af_sad.ref_gain_min =
      (float)tuning_info->af_algo.af_haf.tof.reserve[22];
    core_tuning->af_sad.ref_threshold_max =
      (unsigned short)tuning_info->af_algo.af_haf.tof.reserve[23];
    core_tuning->af_sad.ref_threshold_min =
      (unsigned short)tuning_info->af_algo.af_haf.tof.reserve[24];
    core_tuning->af_sad.threshold_max =
      (unsigned short)tuning_info->af_algo.af_haf.tof.reserve[25];
    core_tuning->af_sad.threshold_min =
      (unsigned short)tuning_info->af_algo.af_haf.tof.reserve[26];
    ...
    core_tuning->af_actuator_hysteresis_comp = AF_HYSTERESIS_COMP;
    core_tuning->af_actuator_shift_comp = AF_ACTUATOR_SHIFT_COMP;
  ...
  /* some TOF related parameters */
  /* Note: Needs to be read from reserved parameters after
   * reserved parameter cleanup */
  ...
  core_tuning->spot_light_filter.enable =
  ...
  for(i = 0, j = 71; i< 10 ; i++, j++) {
    core_tuning->spot_light_filter.iir_2tap_filter[i]  =
  ...
  for(i = 0, j = 81; i< 10 ; i++, j++) {
    core_tuning->spot_light_filter.iir_4tap_filter[i]  =
    (float) tuning_info->af_algo.reserve[j];
  }

  core_tuning->face_extension_tuning.af_roi_face_extension =
   (int)tuning_info->af_algo.reserve[91];
  core_tuning->face_extension_tuning.af_roi_face_scale_x =
    (float)tuning_info->af_algo.reserve[92];
  core_tuning->face_extension_tuning.af_roi_face_scale_y =
    (float)tuning_info->af_algo.reserve[93];
  core_tuning->face_extension_tuning.af_roi_face_symmetric_x =
    (float)tuning_info->af_algo.reserve[94];
  core_tuning->face_extension_tuning.af_roi_face_symmetric_y =
    (float)tuning_info->af_algo.reserve[95];
  core_tuning->firstrun_spotlight_detect_cnt_th =
    AF_FIRSTRUN_SPOTLIGHT_DETECT_CNT_TH;
}
...
void af_port_set_haf_enable(af_port_private_t *af_port)
{
  ...
  af_tuning_haf_t *p_haf_tuning = af_port->af_haf_tuning;
  ...
  mct_event_module_t *mod_evt)
{
  ...
  if (af_msg != NULL) {
    ...
    af_port->af_haf_tuning = &tuning_info->af_algo.af_haf;
    ...
    af_port_populate_reserve_tuning_params(af_port,
      &af_msg->u.af_set_parm.u.af_init_param.core_tuning_params, tuning_info);
...
/** af_port_handle_send_fake_stats_on_sof
 * Send Fake Stats on SOF event with Frame ID
 *
 * @af_port: private port number
 *
 * @mod_event: module event
 *
 **/
static boolean af_port_handle_send_fake_stats_on_sof(mct_port_t * port,
...
static boolean af_port_handle_sof_event(mct_port_t * port,
  ...
  af_port_handle_send_fake_stats_on_sof(port,ctrl_evt);
  ...
  mct_event_module_t *mod_evt)
{
  ...
  switch (mod_evt->type) {
    case MCT_EVENT_MODULE_STATS_PEER_AF_FOCUS_UPDATE:
        AF_HIGH("Received MCT_EVENT_MODULE_STATS_PEER_AF_FOCUS_UPDATE");
  ...
  af_set_parameter_t * set_parm, int mode)
{
  ...
  AF_LOW("Set Focus mode: %d old_mode: %d", mode, af_port->af_mode);
...
static float af_port_get_optimal_focus_distance(af_port_private_t *af_port, int32_t cur_pos)
{
  ...
  af_algo_tune_parms_t *fptr = af_port->fd_prio_data.tuning_info;
  af_tuning_single_t* af_single = &fptr->af_algo.af_single;
  ...
  af_set_parameter_t * parm)
{
  ...
  case AF_SET_PARAM_START: { // HAL3
    ...
    AF_LOW("TRIGGER_START ID: %d, focus mode: %d, af_led_assist: %d",
      af_port->af_trans.trigger_id, af_port->af_mode, af_port->af_led_assist);
    if ((af_port->af_mode != CAM_FOCUS_MODE_CONTINOUS_PICTURE) &&
...
static boolean af_port_intra_event(mct_port_t * port, mct_event_t * event)
{
  ...
  /* sanity check */
  if (!port || !event) {
    AF_ERR("%s: NULL parameter", __func__);
    return FALSE;
  }

  AF_LOW("%s: Received AF port event type: %d, dir: %d", __func__,
    event->type, MCT_EVENT_DIRECTION(event));

  private = (af_port_private_t *)(port->port_private);
  if (!private){
    AF_ERR("%s: NULL private port", __func__);
    return FALSE;
  }
  ...
  if ((private->reserved_id & 0xFFFF0000) != (event->identity & 0xFFFF0000)) {
    AF_LOW("%s:Identity does not match", __func__);
    return FALSE;
  }
  mct_event_module_t *mod_evt =
