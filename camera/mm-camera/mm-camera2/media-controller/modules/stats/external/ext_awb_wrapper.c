  ...
  case AWB_GAINS:
    {
      ...
      if (awb->ops.get_gains(awb->opsptr, &args)) {
        ...
        memcpy(&awb->normal.ccm.ccm, &args.ccm, _CCM_TOTAL * sizeof(float));
    ...
    memcpy(&param->u.awb_gains.ccm.ccm,
      &awb->normal.ccm.ccm,
      _CCM_TOTAL * sizeof(float));
    AWB_HIGH("gain: r:g:b[%f:%f:%f] ctemp[%d] "
      "ccm[%f:%f:%f:%f:%f:%f:%f:%f:%f]",
    ...
    awb_set_parameter_init_t* init_param)
{
  ...
  const awb_algo_struct_type *bayer_awb_param;
  ...
  bayer_awb_param = &chromatix->AWB_bayer_algo_data;
  if (!bayer_awb_param) {
    return FALSE;
  }
    ...
    awb_output_data_t *output)
{
  ...
  if (awb->exif && awb->exif_size) {
    memcpy(output->awb_debug_data_array, awb->exif, awb->exif_size);
...
boolean ext_awb_algo_set_param(awb_set_parameter_t *param,
    awb_output_data_t *output, void *obj)
{
  boolean ret = TRUE;
  ext_awb_algo_control_t *awb = (ext_awb_algo_control_t *)obj;
...
void awb_convert_stats(ext_awb_algo_process_inargs_t *args, stats_t *stats)
{
  ...
  /* R */
  memcpy(&args->r[0], &stats->bayer_stats.p_q3a_bg_stats->bg_r_sum[0],
      auiBg_BlkH * auiBg_BlkV * sizeof(uint32_t));
  ...
  /* B data */
  memcpy(&args->b[0], &stats->bayer_stats.p_q3a_bg_stats->bg_b_sum[0],
      auiBg_BlkH * auiBg_BlkV * sizeof(uint32_t));

  /* Count data */
  memcpy(&args->c[0], &stats->bayer_stats.p_q3a_bg_stats->bg_gr_num[0],
      auiBg_BlkH * auiBg_BlkV * sizeof(uint32_t));
    ...
    ext_awb_algo_process_outargs_t *args)
{
  ...
  memcpy(&awb->normal.ccm.ccm, &args->result.ccm, _CCM_TOTAL * sizeof(float));
...
static boolean is_ledgain_available(ext_awb_algo_control_t *awb)
{
  if ((awb->led.gains.r_gain == 0) ||
    ...
    memcpy(&awb->led.ccm.ccm, &awb->normal.ccm.ccm, _CCM_TOTAL * sizeof(float));
  ...
  case Q3A_OPERATION_MODE_CAMCORDER:
    ...
    if (awb->is_ledgain) {
      if (awb->ops.get_pre_led_gains(awb->opsptr, &args)) {
        ...
        memcpy(&awb->led.ccm.ccm, &args.ccm, _CCM_TOTAL * sizeof(float));
      }
...
static void *ext_awb_algo_init(void *obj, cam_position_t position)
{
  ...
  memset(awb, 0, sizeof(ext_awb_algo_control_t));
