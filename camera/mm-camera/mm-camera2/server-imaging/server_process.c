...
boolean server_process_module_init(void)
{
  ...
  CLOGH(CAM_MCT_MODULE, "Begin init mods");
  for (i = 1;
       i < (int)(sizeof(modules_list)/sizeof(mct_module_init_name_t)); i++) {
  ...
  CLOGH(CAM_MCT_MODULE, "CAMERA_DAEMON: init mods done");
...
serv_proc_ret_t server_process_hal_event(struct v4l2_event *event)
{
  ...
  mct_status_ret_type mct_ret_type;
  ...
  case MSM_CAMERA_NEW_SESSION: {
    ...
    mct_ret_type = mct_controller_new (modules, data->session_id, pipe_fd[1]);
    if (MCT_STATUS_RET_SUCCESS == mct_ret_type) {
      ret.new_session    = TRUE;
      ret.new_session_info.session_idx = data->session_id;
      goto process_done;
    } else if (MCT_STATUS_RET_BUSY == mct_ret_type) {
      close(pipe_fd[0]);
      close(pipe_fd[1]);
      goto return_busy;
    } else {
