...
public:
    ...
    void RunThread()
    {
        ...
        while (true) {
            ...
            MMCAM_LOGI("Server: Connected to new client..., port: %d", m_port);
            ...
            PrintByteStream(m_response.data(), response_size, "Server: Sending connection response: ");
            ...
            while (true) {
                ...
                */
                if (rc == true) {
                    //for debug
                    PrintByteStream(m_data.data(), size, "Server: New message: ");
                    ...
                    if (pipeflag == true) {
                        ...
                        InterfaceThreadMessage *pipe_msg = (InterfaceThreadMessage *)m_data.data();
    ...
    bool Run()
    {
        ...
        pthread_setname_np(m_server_thread, "CAM_eztuneSrvr");

        return rc;
    }

    static EztuneInterface *GetInstance(uint32_t port, void *client)
    {
        ...
        if (kControlPort == port) {
            if (!m_control_instance)
                m_control_instance = new EztuneInterface(kControlPort, client);

            instance = m_control_instance;
        } else {
            if (!m_preview_instance)
                m_preview_instance = new EztuneInterface(kImagePort, client);
...
private:
    ...
    EztuneInterface(uint32_t port, void *client):
        m_client(client),
        m_data(eztune::kMaxProtocolBufferSize),
        ...
        m_engine(NULL)
        {
            ...
            if(port == kControlPort) {
                m_engine = new eztune::ProcessLayer(EZTUNE_SERVER_CONTROL, client);
            }
            else {
                m_engine = new eztune::ProcessLayer(EZTUNE_SERVER_PREVIEW, client);
            }
        }

    //avoid accidental copies
    EztuneInterface(EztuneInterface const &); // Don't Implement
    EztuneInterface& operator=(EztuneInterface const &); // Don't implement
};
...
extern "C" void *eztune_create_server(void *client, eztune_server_t type)
{
    ...
    if (type == EZTUNE_SERVER_CONTROL)
        obj = eztune::EztuneInterface::GetInstance(eztune::kControlPort, client);
    else
        obj = eztune::EztuneInterface::GetInstance(eztune::kImagePort, client);
