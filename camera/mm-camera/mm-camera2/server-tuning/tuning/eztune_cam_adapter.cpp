...
bool CamAdapter::SetupTuningTables(bool reset_ptr)
{
    ...
    // get chromatix and aftune data from sensor
    if(!m_init_done || reset_ptr) {
        m_init_done  = FetchChromatixData();
        m_init_done &= FetchAFTuneData();
        MMCAM_LOGI("EZTune: fetch chromatix/af (success %d)",
            m_init_done);
    }
...
{
    MMCAM_ASSERT(m_init_done, "Error: update before initial fetch");
    return SendEvent(MCT_EVENT_MODULE_EZTUNE_SET_CHROMATIX, m_chromatix);
}

bool CamAdapter::UpdateCamAFTuneData()
{
    MMCAM_ASSERT(m_init_done, "Error: update before initial fetch");
    return SendEvent(MCT_EVENT_MODULE_EZTUNE_SET_AFTUNE, &m_af_driver);
}
