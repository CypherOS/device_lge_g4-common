...
static const char * camscope_filenames[CAMSCOPE_SECTION_SIZE] = {
    "/data/misc/camera/camscope_mct.bin",
...
pthread_mutex_t camscope_mutex[CAMSCOPE_SECTION_SIZE];
 ...
 */
static uint32_t camscope_reserve(camscope_section_type camscope_section,
 ...
 *  Return: N/A
 */
static void store_data(camscope_section_type camscope_section,
 ...
 *
 *  @camscope_section: camscope section where this function is occurring
 *  @packet_type:      the type of camscope packet being stored
 *  @va_args:          the data to be stored in the data packet
 *
 *  Logs CameraScope data to memstore and ultimately the file system
 *
 *  Return: N/A
 */
void camscope_log(camscope_section_type camscope_section,
    ...
    if (camscope_fd[camscope_section] != NULL) {
        va_list args;
        va_start(args, packet_type);
        struct timeval camscope_timestamp;
        gettimeofday(&camscope_timestamp, NULL);

        pthread_mutex_lock(&(camscope_mutex[camscope_section]));
        switch (packet_type) {
            case CAMSCOPE_ASYNC_BEGIN: /* fall through */
            case CAMSCOPE_ASYNC_END: {
                camscope_timing scope_struct;
                uint32_t size = sizeof(scope_struct);
                uint32_t total_size =
                    camscope_reserve(camscope_section, size);
                if (size == total_size) {
                    scope_struct.sw_base.base.packet_type = packet_type;
                    scope_struct.sw_base.base.size = size;
                    scope_struct.sw_base.timestamp = camscope_timestamp;
                    scope_struct.sw_base.process_id = getpid();
                    scope_struct.sw_base.event_name = va_arg(args, uint32_t);
                    scope_struct.frame_id = va_arg(args, uint32_t);
                    store_data(camscope_section, &scope_struct, size);
                }
                break;
            }
            default: {
                CLOGE(CAM_NO_MODULE, "Unknown camscope packet type %d used.");
                break;
            }
        }
        pthread_mutex_unlock(&(camscope_mutex[camscope_section]));
